...
# ─────────────────────────────────────────────────────────────────────────────
# Path + env setup — dynamic so the script runs from any working directory.
# ─────────────────────────────────────────────────────────────────────────────

# script is at <repo>/Model_competition/scripts/generic_model.py
...
# Alias OpenRouter key — production code reads QUALIFICATION_OPENROUTER_API_KEY
_master_key = (
...
# Make production-scoring imports work from any cwd
...
# Press-wire + tech-news domains for site-restricted Exa queries. Single list
# across all intents — no per-industry hardcoding.
_NEWS_DOMAINS = (
    "businesswire.com", "prnewswire.com", "globenewswire.com",
    "techcrunch.com", "reuters.com", "bloomberg.com",
    "venturebeat.com", "axios.com",
)
...
async def sd_google_search(
    client: httpx.AsyncClient,
    query: str,
    n: int = 10,
) -> list[dict]:
    """ScrapingDog Google search → list of {url, title}. Disabled in current
    pipeline; kept as a hook for future expansion alongside Exa.
    """
    ...
    try:
        resp = await client.get(
            ...
            params={"api_key": api_key, "query": query, "results": n, "country": "us"},
            timeout=30,
        )
        return [
            {"url": r.get("link"), "title": (r.get("title") or "")[:300], "date": ""}
...
async def discover_events(client: httpx.AsyncClient, icp: dict) -> list[dict]:
    """Top-level discovery: Exa news first (highest precision), then Sonar to
    fill gaps. Returns deduped events with normalized company names.
    """
    events: list[dict] = []
    seen: set[str] = set()
