...
# now would collapse fulfillment volume and miner payouts. With gate off, emails are
# still verified and ``email_verified`` is recorded accurately (invalids are visible,
# not silently trusted) — turn the gate on only once email SOURCING produces real,
# mostly-deliverable addresses.
FULFILLMENT_EMAIL_GATE = os.getenv("FULFILLMENT_EMAIL_GATE", "false").lower() == "true"
