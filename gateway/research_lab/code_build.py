...
class _CandidateBuildDeadline:
    def __init__(self, timeout_seconds: int):
        self.deadline = time.monotonic() + max(1, int(timeout_seconds))
        self.previous: float | None = None

    def __enter__(self) -> None:
        self.previous = getattr(_BUILD_DEADLINE, "value", None)
        _BUILD_DEADLINE.value = (
            min(self.previous, self.deadline)
    ...
    def __exit__(self, _exc_type: Any, _exc: Any, _traceback: Any) -> None:
        if self.previous is None:
            try:
                del _BUILD_DEADLINE.value
            except AttributeError:
                return
        else:
            _BUILD_DEADLINE.value = self.previous


def _bounded_command_timeout(timeout_seconds: int) -> int:
    requested = max(1, int(timeout_seconds))
    deadline = getattr(_BUILD_DEADLINE, "value", None)
    if deadline is None:
        return requested
    remaining = int(deadline - time.monotonic())
    if remaining <= 0:
        raise CodeEditBuildError("candidate build deadline exhausted")
    return min(requested, remaining)


def _bounded_retry_sleep(delay_seconds: float) -> None:
...
def _copy_source_tree(source_root: Path, destination: Path) -> None:
    """Copy one measured source tree without carrying mutable Git state."""
...
def attach_git_tree_lineage(
    *,
    ...
    """Bind measured Git-tree lineage into an already validated build."""
    ...
    build_doc["git_tree"] = dict(lineage)
        ...
        *,
        ...
        total_timeout_seconds = max(
            ...
            int(
                ...
                else self.config.code_edit_build_timeout_seconds
            ),
        )
        with _CandidateBuildDeadline(total_timeout_seconds):
            return self._build_under_deadline(
                draft=draft,
