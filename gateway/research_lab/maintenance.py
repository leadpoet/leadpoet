"""Maintenance controls for scoring and retained reward settlement."""
...
from .store import create_gateway_control_event, select_all, select_one
...
async def resume_gateway_restart_owned_maintenance() -> dict[str, Any]:
    """Report retained maintenance controls without changing them."""

    scoring_state = await get_scoring_maintenance_state()
    try:
        source_add_state = await select_one(
            "research_lab_source_add_control",
            filters=(("singleton", True),),
        )
        source_add = {
            "status": "preserved" if source_add_state else "already_inactive",
            **({"state": dict(source_add_state)} if source_add_state else {}),
        }
    except Exception as exc:
        logger.warning(
            "research_lab_source_add_maintenance_state_unavailable: %s",
            str(exc)[:240],
        )
        source_add = {
            "status": "unavailable_fail_closed",
    ...
    if scoring_state.get("paused"):
        scoring = {
            "status": (
                "preserved_restart_pause"
                if is_gateway_restart_owned_pause(
                    scoring_state,
                    operator_action="pause-scoring",
                )
                else "preserved_non_restart_pause"
            ),
            "state": scoring_state,
        }
    return {
        "ok": source_add.get("status") != "unavailable_fail_closed",
