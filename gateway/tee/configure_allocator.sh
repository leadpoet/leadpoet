#!/bin/bash
# Configure Nitro's parent allocator for the measured coordinator topology.
...
if set(roles or {}) != {"gateway_coordinator"}:
    raise SystemExit("full V2 topology must define only gateway_coordinator")
print(
...
if [ "$REQUIRED_CPUS" -ne 2 ] || [ "$REQUIRED_MEMORY_MIB" -ne 8192 ]; then
