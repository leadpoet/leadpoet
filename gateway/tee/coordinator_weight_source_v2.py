...
class CoordinatorWeightSourceV2:
    ...
    def _champion_document(
        ...
        With Arena rewards disabled, an omitted reward basis requires an empty
        champion slot. With rewards enabled, the coordinator also reads the
        database so a publication that raced the validator's read cannot be
        omitted as zero. A basis the snapshot names must be the governing row,
        its signature must verify against the Arena key pinned by
        ``LAB_ARENA_SIGNING_PUBLIC_KEY_HASH``, and the triple must be what the
        shared kernel derives from it on the snapshot's metagraph.
        ...
        if "lab_arena_reward_basis" not in value:
            ...
            if lab_arena_rewards.rewards_enabled_from_environment(os.environ):
                ...
                if rows:
                    raise CoordinatorWeightSourceV2Error(
                        "champions calculation snapshot omits the governing reward basis"
                    )
            return dict(proposed)
