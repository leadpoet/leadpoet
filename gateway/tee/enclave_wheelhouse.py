"""Verify the offline CPython 3.9 enclave wheelhouse and installed runtime."""
...
CRITICAL_IMPORTS = ("cbor2", "cryptography")
_PIN_RE = re.compile(r"^([A-Za-z0-9_.-]+)==([^\s\\]+)")
_HASH_RE = re.compile(r"--hash=sha256:([0-9a-f]{64})")


class EnclaveWheelhouseError(RuntimeError):
    """The enclave dependency closure differs from the reviewed lock."""
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        if digest and pending_name and pending_version:
            if pending_name in records:
                raise EnclaveWheelhouseError("duplicate enclave lock package")
            records[pending_name] = (pending_version, digest.group(1))
            pending_name = None
            pending_version = None
    if pending_name is not None or not records:
        raise EnclaveWheelhouseError("enclave lock is incomplete")
    return records
...
def _input_records(path: Path) -> Dict[str, str]:
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not pin:
            raise EnclaveWheelhouseError("enclave input contains a non-exact pin")
        name = _normalized_name(pin.group(1))
        if name in records:
            raise EnclaveWheelhouseError("duplicate enclave input package")
...
def _wheel_metadata(path: Path) -> Tuple[str, str, Tuple[str, ...]]:
    with zipfile.ZipFile(path) as archive:
        ...
        if len(metadata_names) != 1 or len(wheel_names) != 1:
            raise EnclaveWheelhouseError("wheel metadata layout is invalid")
    ...
    if not name or not package_version or not tags:
        raise EnclaveWheelhouseError("wheel name, version, or tags are missing")
...
def verify_wheelhouse(*, input_path: Path, lock_path: Path, wheelhouse: Path) -> Dict[str, str]:
    ...
    if inputs != {name: record[0] for name, record in locked.items()}:
        raise EnclaveWheelhouseError("enclave input and lock pins differ")
    observed = {}
    wheel_paths = sorted(wheelhouse.glob("*.whl"))
    if len(wheel_paths) != len(locked):
        raise EnclaveWheelhouseError("enclave wheelhouse file count differs from lock")
    for wheel_path in wheel_paths:
        name, package_version, tags = _wheel_metadata(wheel_path)
        if name in observed:
            raise EnclaveWheelhouseError("enclave wheelhouse contains duplicate package")
        expected = locked.get(name)
        if expected is None or expected[0] != package_version:
            raise EnclaveWheelhouseError("scoring wheel package or version is not locked")
        digest = hashlib.sha256(wheel_path.read_bytes()).hexdigest()
        if digest != expected[1]:
            raise EnclaveWheelhouseError("scoring wheel hash differs from lock")
        if not any(_tag_is_compatible(tag) for tag in tags):
            raise EnclaveWheelhouseError("scoring wheel has no CPython 3.9 x86_64-compatible tag")
        observed[name] = package_version
    if set(observed) != set(locked):
        raise EnclaveWheelhouseError("enclave wheelhouse package set differs from lock")
    return observed


def verify_installed(*, lock_path: Path) -> Dict[str, str]:
    if tuple(sys.version_info[:3]) != EXPECTED_PYTHON:
        raise EnclaveWheelhouseError(
            "identity enclave runtime requires Python %s" % ".".join(str(item) for item in EXPECTED_PYTHON)
        )
    locked = _lock_records(lock_path)
    observed = {}
    for name, (expected_version, _digest) in sorted(locked.items()):
        try:
            ...
            raise EnclaveWheelhouseError("locked enclave package is not installed") from exc
        if installed_version != expected_version:
            raise EnclaveWheelhouseError("installed enclave package version differs from lock")
...
def main(argv: Iterable[str] = None) -> int:
    ...
    print("verified enclave packages: %s" % len(records))
    return 0
