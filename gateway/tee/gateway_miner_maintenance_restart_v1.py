"""Bind shared miner maintenance to one exact locked restart.

Preserve the global admission setting, secret readback, sealed invocation proof,
and immutable controller authority. Arena owns its separate lease drain guard.
"""
...
SCHEMA_VERSION = "leadpoet.gateway_miner_maintenance_restart.v6"
CANONICAL_COORDINATION_DEADLINE_SECONDS = 9_300
...
def _require_production_parity_clone_service_role_key(token: str) -> None:
    ...
    try:
        ...
        header = json.loads(base64.b64decode(encoded_header + '=' * (-len(encoded_header) % 4), altchars=b'-_', validate=True))
        payload = json.loads(base64.b64decode(encoded_payload + '=' * (-len(encoded_payload) % 4), altchars=b'-_', validate=True))
        signature = base64.b64decode(encoded_signature + '=' * (-len(encoded_signature) % 4), altchars=b'-_', validate=True)
        if not isinstance(header, Mapping) or not isinstance(payload, Mapping) or isinstance(payload.get('iat'), bool) or isinstance(payload.get('exp'), bool):
        ...
        raise GatewayMinerMaintenanceRestartError('production-parity clone credential is invalid') from exc
    now = int(time.time())
    if header != {'alg': 'HS256', 'typ': 'JWT'} or payload.get('aud') != 'authenticated' or payload.get('iss') != 'leadpoet-production-parity' or (payload.get('role') != 'service_role') or (expires_at - issued_at != 172805) or (issued_at > now) or (expires_at <= now) or (len(signature) != 32):
        raise GatewayMinerMaintenanceRestartError('production-parity clone credential identity differs')


def _production_parity_clone_authority(environment: Mapping[str, str], *, deploy_commit: str) -> Optional[dict[str, str]]:
    """Return a clone-only authority after every isolation check succeeds."""
    try:
        boundary = validate_production_parity_boundary_document_v2(environment, network=str(environment.get('BITTENSOR_NETWORK') or ''), netuid=int(environment.get('BITTENSOR_NETUID') or 0))
    except (ProductionParityBoundaryV2Error, TypeError, ValueError) as exc:
        raise GatewayMinerMaintenanceRestartError('production-parity boundary is invalid') from exc
    ...
    expected_environment_path = PRODUCTION_PARITY_WORK_ROOT / run_id / 'runtime' / 'gateway.env'
    configured_environment_path = Path(str(environment.get('GATEWAY_ENV_FILE') or ''))
    service_role_key = str(environment.get('SUPABASE_SERVICE_ROLE_KEY') or '')
    if str(environment.get('GATEWAY_ACTIVE_RELEASE_FALLBACK_CONTEXT') or '') != 'full-parity' or str(environment.get('LEADPOET_GATEWAY_ENV_SECRET_ID') or '') != expected_secret_id or configured_environment_path != expected_environment_path or (str(environment.get('SUPABASE_URL') or '').rstrip('/') != origin) or (origin == PRODUCTION_SUPABASE_ORIGIN) or (str(environment.get('LEADPOET_PARITY_CANDIDATE_SHA') or '').lower() != str(deploy_commit).lower()) or (not service_role_key) or (len(service_role_key.encode('utf-8')) > 64 * 1024) or any((ord(character) < 32 or ord(character) == 127 for character in service_role_key)) or (str(environment.get('LEADPOET_AWS_INSTANCE_ROLE_ONLY') or 'true').lower() != 'true') or any((str(environment.get(name) or '') for name in _FORBIDDEN_AWS_ENV_NAMES)):
        raise GatewayMinerMaintenanceRestartError('production-parity isolation authority differs')
    marker_descriptor: Optional[int] = None
    try:
        marker_parent = PRODUCTION_PARITY_ISOLATION_MARKER.parent.lstat()
        marker_descriptor = os.open(PRODUCTION_PARITY_ISOLATION_MARKER, os.O_RDONLY | getattr(os, 'O_CLOEXEC', 0) | getattr(os, 'O_NOFOLLOW', 0))
        marker = os.fstat(marker_descriptor)
        marker_value = os.read(marker_descriptor, 32)
    except OSError as exc:
        raise GatewayMinerMaintenanceRestartError('production-parity isolation proof is unavailable') from exc
    finally:
        if marker_descriptor is not None:
            os.close(marker_descriptor)
    if not stat.S_ISDIR(marker_parent.st_mode) or stat.S_IMODE(marker_parent.st_mode) != 448 or marker_parent.st_uid != os.geteuid() or (not stat.S_ISREG(marker.st_mode)) or (marker.st_uid != os.geteuid()) or stat.S_IMODE(marker.st_mode) & 18 or (marker_value != b'isolated\n') or (marker.st_size != len(marker_value)):
        raise GatewayMinerMaintenanceRestartError('production-parity isolation proof differs')
    environment_payload = _read_hydrated_gateway_environment(configured_environment_path)
    try:
        (hydrated_environment, _format) = _parse_environment(environment_payload.decode('utf-8'))
    except (UnicodeDecodeError, GatewayMinerSubmissionsDisableError) as exc:
        raise GatewayMinerMaintenanceRestartError('production-parity hydrated environment is invalid') from exc
    bound_names = {'BITTENSOR_NETWORK', 'BITTENSOR_NETUID', 'LEADPOET_AWS_INSTANCE_ROLE_ONLY', 'LEADPOET_PARITY_CANDIDATE_SHA', 'LEADPOET_PRODUCTION_PARITY_BENCHMARK_DATE', 'LEADPOET_PRODUCTION_PARITY_MODE', 'LEADPOET_PRODUCTION_PARITY_RUN_ID', 'LEADPOET_PRODUCTION_PARITY_SUPABASE_ORIGIN', 'RESEARCH_LAB_MINER_SUBMISSIONS_ENABLED', 'SUPABASE_SERVICE_ROLE_KEY', 'SUPABASE_URL'}
    if any((not isinstance(hydrated_environment.get(name), str) or hydrated_environment.get(name) != environment.get(name) for name in bound_names)):
        raise GatewayMinerMaintenanceRestartError('production-parity hydrated environment differs')
    _require_production_parity_clone_service_role_key(service_role_key)
    return {'run_id': run_id, 'supabase_origin': origin, 'service_role_key': service_role_key}
...
def _verify_proof_against_state(*, proof: Mapping[str, str], deploy_commit: str, candidate_tree_hash: str, client: Any, tree_evidence: Optional[Mapping[str, Any]]=None, restart_invocation_id: Optional[str]=None, live_process_commitment: Optional[str]=None, hydrated_environment_path: Optional[Path]=None) -> dict[str, str]:
    validated = _validate_proof_document(proof)
    if validated['candidate_commit'] != str(deploy_commit).lower() or validated['candidate_tree_hash'] != str(candidate_tree_hash).lower():
        raise GatewayMinerMaintenanceRestartError('miner-maintenance invocation proof differs from the candidate')
    if restart_invocation_id is not None and validated['restart_invocation_id'] != str(restart_invocation_id):
        raise GatewayMinerMaintenanceRestartError('miner-maintenance invocation proof differs from the restart')
    if live_process_commitment is not None and validated['pre_hydration_live_process_commitment'] != str(live_process_commitment):
        raise GatewayMinerMaintenanceRestartError('running gateway process differs from the invocation proof')
    current = _require_disabled_secret_readback(client=client, expected_current_version_id=validated['current_secret_version_id'])
    if current.get('current_document_commitment') != validated['current_document_commitment'] or current.get('current_hydrated_environment_commitment') != validated['current_hydrated_environment_commitment'] or current.get('current_stage_topology_commitment') != validated['current_stage_topology_commitment']:
        raise GatewayMinerMaintenanceRestartError('durable miner-maintenance state differs from the invocation proof')
    if hydrated_environment_path is not None:
        _require_hydrated_environment_commitment(path=hydrated_environment_path, expected_commitment=validated['current_hydrated_environment_commitment'])
    final_current = _require_disabled_secret_readback(client=client, expected_current_version_id=validated['current_secret_version_id'])
    if final_current.get('current_document_commitment') != validated['current_document_commitment'] or final_current.get('current_hydrated_environment_commitment') != validated['current_hydrated_environment_commitment'] or final_current.get('current_stage_topology_commitment') != validated['current_stage_topology_commitment']:
        raise GatewayMinerMaintenanceRestartError('durable miner-maintenance state changed during hydration verification')
...
def prepare_gateway_miner_maintenance_restart(*, repo_root: Path, candidate_root: Path, plan_file: Path, expected_commit: str, controller_current: Path, host_restart_path: Path, restart_invocation_id: str, recovery_journal_path: Path=DEFAULT_RECOVERY_JOURNAL_PATH, secrets_client: Any=None) -> dict[str, Any]:
    ...
    if not re.fullmatch('[A-Za-z0-9][A-Za-z0-9._:-]{0,199}', str(restart_invocation_id)):
        raise GatewayMinerMaintenanceRestartError('gateway restart invocation identity is invalid')
    secrets_client = _resolve_bootstrap_secrets_client(secrets_client)
    tree_evidence = _validate_candidate_identity(repo_root=repo_root, candidate_root=candidate_root, plan_file=plan_file, expected_commit=expected_commit, controller_current=controller_current, host_restart_path=host_restart_path, reconcile_host_wrapper=True)
    _verify_protected_source()
    live_process_commitment = _pre_hydration_live_process_commitment(tree_evidence)
    with _open_recovery_journal_parent_fd(recovery_journal_path) as recovery_authority:
        _recover_orphan_transaction(secrets_client, recovery_journal_path=recovery_journal_path, recovery_journal_authority=recovery_authority)
        observed = disable_gateway_miner_submissions_secret(secrets_client=secrets_client)
        version_id = str(observed['current_version_id'])
        if observed['status'] == 'already_disabled':
            applied = observed
        elif observed['status'] == 'verified':
            applied = _apply_gateway_miner_submissions_secret(secrets_client=secrets_client, expected_current_version_id=version_id, recovery_journal_path=recovery_journal_path, recovery_journal_authority=recovery_authority)
        else:
            raise GatewayMinerMaintenanceRestartError('miner-submission disable verification returned an invalid status')
        final_version = str(applied['current_version_id'])
        final_result = disable_gateway_miner_submissions_secret(secrets_client=secrets_client, expected_current_version_id=final_version)
        if final_result.get('status') != 'already_disabled':
            raise GatewayMinerMaintenanceRestartError('miner submissions were not disabled after exact readback')
    proof = _proof_body(candidate_commit=expected_commit, tree_evidence=tree_evidence, final_secret_result=final_result, restart_invocation_id=restart_invocation_id, live_process_commitment=live_process_commitment)
    _validate_proof_document(proof)
    return {'status': 'prepared', 'proof': proof, 'tree_evidence': tree_evidence}


def verify_gateway_miner_maintenance_state(*, deploy_commit: str, candidate_tree_hash: str, parent_environment: Mapping[str, str], secrets_client: Any=None, bind_live_process_to_proof: bool=True, hydrated_environment_path: Path=CANONICAL_GATEWAY_ENV_PATH) -> dict[str, str]:
    ...
    if proof_fd is not None:
        ...
        if bind_live_process_to_proof:
            proof_runtime_commit = validated_proof['pre_hydration_runtime_commit']
            proof_controller_commit = validated_proof['n_minus_one_controller_commit']
            live_process_commitment = _live_gateway_restart_authority_commitment(expected_runtime_commit=proof_runtime_commit, verified_controller_commit=proof_controller_commit, allow_legacy_n_minus_one_git_helper=proof_runtime_commit == LEGACY_N_MINUS_ONE_CONTROLLER_COMMIT and proof_controller_commit == LEGACY_N_MINUS_ONE_CONTROLLER_COMMIT)
        else:
            _live_gateway_restart_authority_commitment()
            live_process_commitment = None
        return _verify_proof_against_state(proof=validated_proof, deploy_commit=deploy_commit, candidate_tree_hash=candidate_tree_hash, client=secrets_client, restart_invocation_id=parent_environment.get('GATEWAY_RESTART_INVOCATION_ID'), live_process_commitment=live_process_commitment, hydrated_environment_path=hydrated_environment_path)
    _live_gateway_restart_authority_commitment()
    current = _require_disabled_secret_readback(client=secrets_client)
    restart_invocation_id = str(parent_environment.get('GATEWAY_RESTART_INVOCATION_ID') or '')
    _require_hydrated_environment_commitment(path=hydrated_environment_path, expected_commitment=str(current['current_hydrated_environment_commitment']))
    final_current = _require_disabled_secret_readback(client=secrets_client, expected_current_version_id=str(current['current_version_id']))
    if final_current.get('current_document_commitment') != current.get('current_document_commitment') or final_current.get('current_hydrated_environment_commitment') != current.get('current_hydrated_environment_commitment') or final_current.get('current_stage_topology_commitment') != current.get('current_stage_topology_commitment'):
        raise GatewayMinerMaintenanceRestartError('durable miner-maintenance state changed during hydration verification')
    return {'status': 'durable_false_verified', 'current_secret_version_id': str(current['current_version_id'])}
...
def _wait_for_handoff_marker(*, path: Path, expected_commit: str, nonce: str, timeout_seconds: int) -> None:
    if not re.fullmatch('/tmp/leadpoet-gateway-miner-maintenance-handoff\\.[A-Za-z0-9._-]+', str(path)) or not re.fullmatch('[0-9a-f]{64}', str(nonce)) or (not 1 <= int(timeout_seconds) <= CANONICAL_COORDINATION_DEADLINE_SECONDS):
        raise GatewayMinerMaintenanceRestartError('miner-maintenance handoff request is invalid')
    ...
    while time.monotonic() < deadline:
        try:
            ...
            time.sleep(0.1)
            continue
        action = _read_handoff_marker(path=path, expected_commit=expected_commit, nonce=nonce)
        if action == 'cancel':
            raise GatewayMinerMaintenanceRestartError('paired operator cancelled the miner-maintenance handoff')
        if action != 'continue':
            raise GatewayMinerMaintenanceRestartError('paired operator did not authorize the miner-maintenance handoff')
        return
    raise GatewayMinerMaintenanceRestartError('paired operator did not provide a bounded miner-maintenance handoff')
...
def bootstrap_gateway_miner_maintenance_restart(*, repo_root: Path, candidate_root: Path, bootstrap_root: Path, plan_file: Path, expected_commit: str, controller_current: Path, host_restart_path: Path, handoff_file: Path, handoff_nonce: str, restart_invocation_id: str, secrets_client: Any=None) -> None:
    ...
    try:
        ...
        _require_fixed_bootstrap_authority(os.environ)
        secrets_client = _resolve_bootstrap_secrets_client(secrets_client)
        prepared = prepare_gateway_miner_maintenance_restart(repo_root=repo_root, candidate_root=candidate_root, plan_file=plan_file, expected_commit=expected_commit, controller_current=controller_current, host_restart_path=host_restart_path, restart_invocation_id=restart_invocation_id, secrets_client=secrets_client)
        proof = prepared['proof']
        _seal_payload_at_fd_number(payload=_serialized_proof(proof), fd_number=PROOF_FD_NUMBER, name='leadpoet-miner-maintenance-proof', max_bytes=MAX_PROOF_BYTES)
        proof_fd_open = True
        if _proof_from_fd(PROOF_FD_NUMBER) != proof:
            raise GatewayMinerMaintenanceRestartError('sealed miner-maintenance invocation proof differs')
        print('Prepared exact-candidate miner maintenance under the canonical restart lock', flush=True)
        _wait_for_handoff_marker(path=handoff_file, expected_commit=expected_commit, nonce=handoff_nonce, timeout_seconds=CANONICAL_COORDINATION_DEADLINE_SECONDS)
        _require_canonical_restart_lock_fd()
        final_tree = _validate_candidate_identity(repo_root=repo_root, candidate_root=candidate_root, plan_file=plan_file, expected_commit=expected_commit, controller_current=controller_current, host_restart_path=host_restart_path, reconcile_host_wrapper=True)
        _verify_protected_source()
        secrets_client = _resolve_bootstrap_secrets_client(secrets_client)
        final_live_process_commitment = _pre_hydration_live_process_commitment(final_tree)
...
def verify_gateway_miner_maintenance_shutdown_quiescence(
    *, deploy_commit: str, parent_environment: Mapping[str, str],
    secrets_client: Any = None, connection_factory: Any = None,
) -> dict[str, str]:
    """Verify the shared maintenance authority before destructive shutdown.

    Arena has its own separately fenced drain and destructive authorization.
    """
    _require_canonical_restart_lock_fd()
    commit = str(deploy_commit).lower()
    if not _COMMIT_RE.fullmatch(commit):
        raise GatewayMinerMaintenanceRestartError("candidate commit is invalid")
    parity = _production_parity_clone_authority(parent_environment, deploy_commit=commit)
    _require_disabled_parent_environment(parent_environment)
    if parity is not None:
        return {"status": "shutdown_quiescence_verified", "authority": "production_parity_clone"}
    _require_fixed_bootstrap_authority(parent_environment)
    client = _resolve_bootstrap_secrets_client(secrets_client)
    proof_fd = _proof_fd_from_environment(parent_environment)
    proof = _validate_proof_document(_proof_from_fd(proof_fd)) if proof_fd is not None else None
    if proof is not None and (proof["candidate_commit"] != commit or proof["restart_invocation_id"] != str(parent_environment.get("GATEWAY_RESTART_INVOCATION_ID") or "")):
        raise GatewayMinerMaintenanceRestartError("miner-maintenance invocation proof differs at shutdown")
    current = _require_disabled_secret_readback(client=client, expected_current_version_id=proof["current_secret_version_id"] if proof else None)
    if proof is not None and any(current.get(field) != proof[field] for field in ("current_document_commitment", "current_hydrated_environment_commitment", "current_stage_topology_commitment")):
        raise GatewayMinerMaintenanceRestartError("durable miner-maintenance state differs at shutdown")
    return {"status": "shutdown_quiescence_verified", "current_secret_version_id": str(current["current_version_id"])}
...
def verify_gateway_miner_maintenance_runtime_state(
    *, deploy_commit: str, candidate_tree_hash: str,
    runtime_environment: Mapping[str, str], runtime_status: Mapping[str, Any],
    secrets_client: Any = None, hydrated_environment_path: Path = CANONICAL_GATEWAY_ENV_PATH,
    connection_factory: Any = None, runtime_status_provider: Any = None,
) -> dict[str, str]:
    """Verify shared miner-maintenance controls against the exact runtime."""
    commit = str(deploy_commit).lower()
    ...
    result = verify_gateway_miner_maintenance_state(
        deploy_commit=commit, candidate_tree_hash=candidate_tree_hash,
        parent_environment=runtime_environment, secrets_client=secrets_client,
        bind_live_process_to_proof=False, hydrated_environment_path=hydrated_environment_path,
    )
