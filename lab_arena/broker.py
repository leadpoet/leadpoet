"""Closed provider operations with reservation, dispatch, and settlement
...
def validate_price_table(document: Any) -> Dict[str, Any]:
    ...
    contracts.require_only_keys(document, ("schema_version", "fetched_at", "source", "models", "price_table_hash"))
    ...
    if "price_table_hash" in document:
        contracts.verify_hashed_document(dict(document), "price_table_hash")
    return contracts.hashed_document(table, "price_table_hash")


def price_table_from_models_response(response: Mapping[str, Any], allowed_models: Sequence[str], *, fetched_at: str) -> Dict[str, Any]:
    """Pin every priced component of the allowed models from the models endpoint."""
    ...
    wanted = {str(model) for model in allowed_models}
    if not wanted:
        raise ArenaContractError("at least one allowed model is required")
    ...
    for item in response["data"]:
        ...
        if model_id in wanted:
            ...
            if not isinstance(pricing, Mapping):
                raise ArenaContractError("model %s has no pricing" % model_id)
            found[str(model_id)] = {component: pricing.get(component, "0") for component in PRICED_COMPONENTS}
...
def fetch_openrouter_price_table(
    ...
    *,
    ...
    return price_table_from_models_response(decoded, allowed_models, fetched_at=moment.strftime("%Y-%m-%dT%H:%M:%SZ"))
...
def bounded_input_tokens(messages: Sequence[Mapping[str, Any]]) -> int:
    total = REQUEST_TOKEN_OVERHEAD
    for message in messages:
        content = message.get("content") if isinstance(message, Mapping) else None
        total += MESSAGE_TOKEN_OVERHEAD + TOKENS_PER_CHAR_BOUND * len(str(content or ""))
    return total
...
def max_openrouter_cost_microusd(price_table: Mapping[str, Any], model: str, parameters: Mapping[str, Any], *, max_output_tokens: int) -> int:
    ...
    input_tokens = bounded_input_tokens(parameters.get("messages") or ())
...
class HttpxProviderTransport:
    ...
    def __init__(self, *, client: Optional[httpx.Client] = None, max_response_bytes: int = 4 * 1024 * 1024) -> None:
        self._client = client or httpx.Client(http1=True, http2=False, follow_redirects=False, timeout=httpx.Timeout(30.0))
    ...
    def send(self, *, method: str, url: str, headers: Mapping[str, str], body: bytes, timeout_seconds: float) -> ProviderResponse:
        ...
        try:
            response = self._client.request(method, url, headers=dict(headers), content=body, timeout=httpx.Timeout(float(timeout_seconds)))
        ...
        return ProviderResponse(int(response.status_code), {k.lower(): v for k, v in response.headers.items()}, content)
...
class Broker:
    ...
    def price_table_hash(self) -> str:
        return str(self._price_table["price_table_hash"])
    ...
    def _event(self, run_state: Mapping[str, Any], payload: Mapping[str, Any]) -> Dict[str, Any]:
        return contracts.build_private_event(
            ...
            sequence=int(run_state["event_cursor"]),
            prev_hash=str(run_state["event_head_hash"] or ""),
            timestamp=self._timestamp(),
            payload=payload,
        )

    def _terminal_with_event(self, action: Callable[..., Dict[str, Any]], run_state: Mapping[str, Any], payload: Mapping[str, Any], **kwargs: Any) -> Dict[str, Any]:
        """Call settle/uncertain with a summary event; refresh the cursor on a race."""

        state = dict(run_state)
        for _attempt in range(3):
            try:
                return action(event=self._event(state, payload), **kwargs)
            except Exception as exc:  # the store raises ArenaStoreError on a sequence race
                if "lab_arena_event_sequence" not in str(exc):
                    raise
                run = self._store.get_run(kwargs["run_id"])
                if run is None:
                    raise
                state = {"event_cursor": run["event_cursor"], "event_head_hash": run["event_head_hash"]}
        raise BrokerError("broker_unavailable")
    ...
    def execute(
        ...
        *,
        ...
        summary["event_cursor"] = reserved.get("event_cursor")
        summary["event_head_hash"] = reserved.get("event_head_hash")
        if status == "stale":
        ...
        if dispatched.get("status") != "dispatched":
            summary["outcome"] = "uncertain"
            return _error_result("call_uncertain", summary)
        run_state = {"event_cursor": dispatched.get("event_cursor"), "event_head_hash": dispatched.get("event_head_hash")}
