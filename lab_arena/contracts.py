...
# rounds. New rounds use a separate 60-minute profile; the 90-minute profile
# remains available only for explicit recovery.
CHECKPOINT_DEADLINE_POLICY = "atomic_checkpoint_45m_v1"
CHECKPOINT_WALL_CLOCK_SECONDS = 45 * 60
CHECKPOINT_LEASE_TTL_SECONDS = 3600
CHECKPOINT_60M_DEADLINE_POLICY = "atomic_checkpoint_60m_v1"
CHECKPOINT_60M_WALL_CLOCK_SECONDS = 60 * 60
CHECKPOINT_60M_LEASE_TTL_SECONDS = CHECKPOINT_60M_WALL_CLOCK_SECONDS + 15 * 60
DEFAULT_CHECKPOINT_DEADLINE_POLICY = CHECKPOINT_60M_DEADLINE_POLICY
...
CHECKPOINT_DEADLINE_PROFILES = {
    ...
    CHECKPOINT_60M_DEADLINE_POLICY: (
        CHECKPOINT_60M_WALL_CLOCK_SECONDS,
        CHECKPOINT_60M_LEASE_TTL_SECONDS,
    ),
    CHECKPOINT_90M_DEADLINE_POLICY: (
        CHECKPOINT_90M_WALL_CLOCK_SECONDS,
        CHECKPOINT_90M_LEASE_TTL_SECONDS,
    ),
}
...
ROUND_CONFIGURATION_FIELDS = (
    ...
    F(
        ...
        "str",
        required=False,
        choices=CHECKPOINT_DEADLINE_POLICIES,
    ),
...
def validate_round_configuration(document: Any) -> Dict[str, Any]:
    ...
    checkpoint_profile = CHECKPOINT_DEADLINE_PROFILES.get(
        config.get("checkpoint_deadline_policy")
    )
    if checkpoint_profile is not None:
        expected_wall, expected_lease = checkpoint_profile
        if config["icp_wall_clock_seconds"] != expected_wall:
            raise ArenaContractError(
                "checkpoint deadline round must freeze its exact wall clock"
            )
        if config["lease_ttl_seconds"] != expected_lease:
            raise ArenaContractError(
                "checkpoint deadline round must freeze its full lease"
            )
    ...
    if has_intent_details and not is_integrity:
        raise ArenaContractError("intent details policy requires integrity policy")
    if has_intent_details != intent_details_policy.scorer_enabled(
        config["scorer_policy"]
    ):
        raise ArenaContractError(
            "intent details policy requires matching scorer policy"
        )
...
def validate_scorer_policy(document: Any) -> Dict[str, Any]:
    ...
    if (
        policy.get("intent_details_policy")
        and policy["scoring_adapter_version"]
        not in ("qualification_integrity_v2", "qualification_contacts_v3")
    ):
        raise ArenaContractError("intent details requires integrity scoring")
