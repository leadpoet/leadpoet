...
def _candidate_host(value: ResolverInput) -> Optional[str]:
    for candidate in (
...
def _website_for_host(
    value: ResolverInput,
    host: str,
) -> Optional[WebsiteObservation]:
...
def _is_usable_site(observation: Optional[WebsiteObservation]) -> bool:
    return bool(
        observation
...
def _site_links_linkedin(
    observation: Optional[WebsiteObservation],
...
def _prior_anchor_matches(
    value: ResolverInput,
    linkedin: Optional[LinkedInCompanyIdentity],
) -> bool:
...
def _decision(
    ...
    *,
    ...
    identity_id = uuid5(
        NAMESPACE_URL,
        "https://leadpoet.com/company-identity/v1/"
        + canonical_sha256(identity_material),
    )
    identity_material["company_identity_id"] = str(identity_id)
