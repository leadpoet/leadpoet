...
KEYWORDS = {
    ...
    ["logistics", "shipping", "transport", "automotive"],
    "media & entertainment":
    ["media", "gaming", "music", "streaming", "content"],
}
...
def classify_industry(description: str) -> Optional[str]:
    ...
    prompt_system = ("You are an industry classifier. "
...
def _intent_score(business: str, website: str,
                  industry: Optional[str]) -> float:
    if not industry:
        return 0.0
...
async def _score_batch(leads: list[dict], description: str) -> list[float]:
    ...
    try:
        ...
        try:
            import time
            time.sleep(1)
            r = await asyncio.to_thread(_call_batch, FALLBACK_MODEL,
                                        prompt_user)
            r.raise_for_status()
            raw = r.json()["choices"][0]["message"]["content"].strip()
...
async def rank_leads(leads: list[dict], description: str) -> list[dict]:
    ...
    if desired_roles:
        ...
        if not filt:  # nothing matched → fall back to all leads
            ...
            print(
                f"✅ Filtered to {len(filt)} leads matching roles: {desired_roles}"
            )
    else:
        filt = leads
    ...
    sorted_leads = sorted(filt,
                          key=lambda x: x["miner_intent_score"],
                          reverse=True)
...
ROLE_KEYWORDS: dict[str, list[str]] = {
    ...
    # Engineering & data
    "software engineer": [
        "software engineer", "engineer", "developer", "dev", "full-stack",
        "backend", "front-end"
    ],
    "data scientist": [
    ...
    ["business development", "bd", "partnerships", "growth"],
    "sdr": ["sdr", "sales development representative", "sales development"],
...
# ---------------------------------------------------------------------
ROLES_CANON = [
    "Founder", "CEO", "CTO", "CMO", "Software Engineer", "Data Scientist",
    "Product Manager", "Business Development", "SDR", "Sales", "Marketing",
    "Operations", "Finance"
]
    ...
    def _try_model(model_name: str) -> list[str]:
        ...
        if start != -1 and end != -1:
            try:
                ...
                if valid_roles:
                    print(
                        f"✅ ROLE-LLM succeeded → {valid_roles}  (model: {model_name})"
                    )
                    return valid_roles
            except Exception:
                pass
