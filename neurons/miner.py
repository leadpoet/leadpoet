...
def run_qualification_submission_flow(wallet, config, netuid: int):
    ...
    print("   └── your-model/")
    print("       ├── qualify.py      # REQUIRED: Must define `qualify(icp)` (takes ICP dict, returns one output dict)")
    print("       ├── requirements.txt  # Optional: Dependencies")
    print("       └── ...             # Any other files your model needs")
    print("")
    print("   Output schema depends on icp['mode']:")
    print("     • mode == 'lead'    (default, legacy) → return a LeadOutput dict")
    print("                                              (company + contact + intent signals)")
    print("     • mode == 'company' (new)             → return a CompanyOutput dict")
    print("                                              (company + intent signals only, NO contact fields)")
    print("   Schemas live in gateway/qualification/models.py.  Extra/unknown fields")
    print("   cause instant score-0 validation failure (gaming prevention).")
    print("")
