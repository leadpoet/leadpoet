...
def build_loop_direction_planner_messages(
    *,
    ...
    user = (
        ...
        "- Context JSON approved_provider_capabilities.routerverse_source_incorporation.requests is the only authority for adding a SOURCE_ADD provider to Routerverse. Each request is already approval-, readiness-, stage-, and manifest-bound.\n"
        "- When that requests list is non-empty, select source_routing, inspect the exact registration_symbol, and plan an exact SourceAddRoutingRegistration using the supplied provider_id, stage, manifest, bounds, and tool metadata. A model registration is required; changing only a query, prompt, provider client, or fallback does not implement the request.\n"
...
def build_code_edit_auto_research_messages(
    *,
    ...
    user = (
        ...
        "- For a source-incorporation plan, add the exact SourceAddRoutingRegistration to sourcing_model/routing/runtime.py::SOURCE_ADD_ROUTING_REGISTRATIONS using only the request's provider_id, stage, revision, manifest_sha256, priority, capabilities, idempotency, cost/budget bounds, and evidence types.\n"
