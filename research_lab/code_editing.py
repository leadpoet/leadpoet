...
def build_loop_direction_planner_messages(
    *,
    ...
    user = (
        ...
        "- When that requests list is non-empty, select source_routing, inspect the exact registration_symbol and its current v7 or v8 contract, and plan an exact SourceAddRoutingRegistration using the supplied provider_id, stage, execution mode, manifest, bounds, tool metadata, intent_categories, evidence types, category contracts, binding requirements, best_for, avoid_when, best_for_description, and avoid_when_description. A model registration is required; changing only a query, prompt, provider client, or fallback does not implement the request.\n"
