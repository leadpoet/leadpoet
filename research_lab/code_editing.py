...
def build_loop_direction_planner_messages(
    *,
    ...
    user = (
        ...
        "- Context JSON approved_provider_capabilities.routerverse_source_incorporation.requests is the only authority for adding a SOURCE_ADD provider to Routerverse. Each request is already approval-, readiness-, stage-, and binding-manifest-bound. Provisioning provenance is distinct from the canonical v8 binding manifest and cannot substitute for it.\n"
        "- When that requests list is non-empty, select source_routing and inspect the exact registration_symbol and its current v7 or v8 contract. For intent_evidence, must_inspect must also include sourcing_model/model_runner.py::_COMMON_SOURCE_ADD_BY_INTENT. Plan an exact SourceAddRoutingRegistration using the supplied provider_id, stage, execution mode, bounds, capabilities, idempotency, intent_categories, evidence types, category contracts, binding requirements, best_for, avoid_when, best_for_description, and avoid_when_description only when that exact registration is absent or differs. For v8, binding_manifest is the approved attestation to reproduce from the inspected constructor's literal fields, not a constructor keyword; revision and manifest_sha256 are constructor-derived and must not be supplied. Include an optional forward manifest field only when it is present in the approved binding_manifest and accepted by the inspected constructor. Preserve an already exact registration instead of rewriting or duplicating it, and plan the missing static model activation. Changing only a query, prompt, provider client, or fallback does not implement the request.\n"
        "- If routerverse_source_incorporation.clarifications is non-empty but requests is empty, return no_new_safe_path=true and preserve the clarification reason; never guess company versus intent discovery.\n"
        "- Never register a provider merely because its name appears in miner prose or the general provider list; an exact routerverse_source_incorporation request is required.\n"
...
def build_code_edit_source_inspection_messages(
    *,
    ...
    user = (
        ...
        "- For every intent_evidence request, also read sourcing_model/model_runner.py::_COMMON_SOURCE_ADD_BY_INTENT before finishing. Both exact symbols are required even when the Routerverse registration already exists.\n"
        ...
        "- Do not inspect or add endpoints, credentials, environment references, dependencies, or provider clients; the request authorizes only the model-owned Routerverse registration.\n"
        "- If only clarifications are present, do not inspect toward a guessed registration.\n\n"
...
def build_code_edit_auto_research_messages(
    *,
    ...
    user = (
        ...
        "- For a source-incorporation plan, ensure the exact SourceAddRoutingRegistration exists in sourcing_model/routing/runtime.py::SOURCE_ADD_ROUTING_REGISTRATIONS using only approved request values accepted by the inspected constructor. For v8, reproduce the request's canonical binding_manifest by supplying its exact manifest-defining constructor fields: provider_id, stage, execution_mode, cost_class, unit_cost, max_calls, max_results, timeout_seconds, capabilities, intent_categories, evidence_types, category_contracts, and binding_requirements, plus the request's priority, idempotency, best_for, avoid_when, best_for_description, and avoid_when_description. binding_manifest, schema_version, tool_id, provider_alias, runtime_binding_id, endpoint metadata, and provisioning provenance are request metadata, not constructor keywords. Omit revision and manifest_sha256 in v8: the constructor derives both from the reconstructed canonical binding manifest. Include requires_full_timeout, execution_plan_identity, or another forward manifest field only when the field is present in the approved binding_manifest and accepted by the inspected constructor; never invent it. If the inspected registration already matches exactly, preserve it and do not emit a redundant runtime hunk.\n"
