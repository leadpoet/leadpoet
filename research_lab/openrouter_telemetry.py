...
Capture stays inert when the dedicated raw-trace destination is unset or the
KMS key is absent.
"""
...
def resolved_raw_trace_s3_prefix() -> str:
    """Return the explicit raw-trace destination, if configured."""

    return str(os.getenv(RAW_TRACE_S3_PREFIX_ENV, "") or "").strip().rstrip("/")
