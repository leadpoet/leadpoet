-- Serialize measured provider-outcome checkpoints across coordinator contexts.
--
-- The encrypted state remains append-only and service-role-only. The RPC
-- accepts a checkpoint only when it extends the current head for the exact
-- artifact-key lineage and UTC day. This prevents concurrent enclave request
-- contexts from creating sibling checkpoints from one stale predecessor.
...
COMMENT ON FUNCTION public.append_research_lab_provider_outcome_checkpoint_v2(JSONB) IS
    'Atomically extends one encrypted provider-outcome checkpoint lineage or returns the exact existing checkpoint.';
