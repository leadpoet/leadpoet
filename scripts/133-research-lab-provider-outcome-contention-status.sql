-- Return expected provider-outcome append contention as measured RPC state.
--
-- Migration 131 made lock acquisition nonblocking, but surfaced ordinary
-- concurrent-writer contention as SQLSTATE 40001. PostgREST counted every
-- bounded retry as a failed API call. Keep malformed and inconsistent inputs
-- fail-closed while returning explicit busy/conflict states for the two
-- expected compare-and-set outcomes.
...
CREATE OR REPLACE FUNCTION public.research_lab_provider_outcome_contention_contract_v2()
RETURNS JSONB
LANGUAGE sql
STABLE
PARALLEL SAFE
...
AS $$
    SELECT jsonb_build_object(
        'schema_version', 'leadpoet.provider_outcome_contention_contract.v2',
...
REVOKE ALL ON FUNCTION public.research_lab_provider_outcome_contention_contract_v2()
    FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.research_lab_provider_outcome_contention_contract_v2()
    TO service_role;

COMMENT ON FUNCTION public.append_research_lab_provider_outcome_checkpoint_v2(JSONB) IS
    'Atomically extends one encrypted provider-outcome lineage and returns measured busy/conflict states without failed PostgREST calls.';
COMMENT ON FUNCTION public.research_lab_provider_outcome_contention_contract_v2() IS
    'Declares the measured non-error statuses used for expected provider-outcome append contention.';
