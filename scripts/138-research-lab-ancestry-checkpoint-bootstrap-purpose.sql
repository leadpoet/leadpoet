-- Admit the measured coordinator job that converts durable legacy ancestry
-- into enclave-signed recursive checkpoints.  The operation remains bound to
-- the existing append-only receipt/checkpoint stores; this migration only
-- widens the canonical role/purpose allowlist.
-- Apply after migration 136.
...
public.research_lab_ancestry_checkpoint_bootstrap_contract_v2()
RETURNS JSONB
LANGUAGE SQL
STABLE
SECURITY DEFINER
