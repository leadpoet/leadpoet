...
-- The database independently recomputes the frozen score at publication.
-- Fixed decimal roots and half-even rounding match lab_arena.verify.stage_score.
CREATE OR REPLACE FUNCTION public.lab_arena__quality_round_even(
  p_value NUMERIC, p_places INTEGER
) RETURNS NUMERIC
LANGUAGE plpgsql IMMUTABLE STRICT
SET search_path = pg_catalog, public
AS $quality_round_even$
DECLARE
  v_scale NUMERIC := pg_catalog.power(10::NUMERIC, p_places);
  v_scaled NUMERIC := p_value * v_scale;
  v_lower NUMERIC := pg_catalog.floor(v_scaled);
BEGIN
  IF v_scaled - v_lower = 0.5 AND pg_catalog.mod(v_lower, 2) = 0 THEN
    RETURN v_lower::NUMERIC(160,80) / v_scale;
  END IF;
  RETURN pg_catalog.floor(v_scaled + 0.5)::NUMERIC(160,80) / v_scale;
END;
$quality_round_even$;
ALTER FUNCTION public.lab_arena__quality_round_even(NUMERIC, INTEGER)
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena__quality_round_even(NUMERIC, INTEGER)
  FROM PUBLIC, lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena__company_quality_stage_score(
  p_scores DOUBLE PRECISION[]
) RETURNS DOUBLE PRECISION
LANGUAGE plpgsql IMMUTABLE STRICT
SET search_path = pg_catalog, public
AS $company_quality_stage_score$
DECLARE
  v_count INTEGER := pg_catalog.cardinality(p_scores);
  v_score DOUBLE PRECISION;
  v_root NUMERIC;
  v_total NUMERIC := 0;
  v_result NUMERIC;
BEGIN
  -- Submission summaries also inspect partial attempts and all 25 positions
  -- for cost eligibility. Their existing valid flag guards completeness.
  IF v_count NOT BETWEEN 1 AND 25 THEN
    RAISE EXCEPTION 'lab_arena_company_quality_score_count_invalid'
      USING ERRCODE = '22023';
  END IF;
  FOR v_score IN SELECT value FROM pg_catalog.unnest(p_scores) AS scores(value)
                 ORDER BY value LOOP
    IF v_score IS NULL OR v_score < 0 OR v_score > 100 THEN
      RAISE EXCEPTION 'lab_arena_company_quality_score_invalid'
        USING ERRCODE = '22023';
    END IF;
    v_root := public.lab_arena__quality_round_even(
      pg_catalog.sqrt((v_score::TEXT)::NUMERIC(80,50)), 40
    );
    v_total := v_total + v_root;
  END LOOP;
  v_result := (v_total / v_count) * (v_total / v_count);
  RETURN public.lab_arena__quality_round_even(v_result, 12)::DOUBLE PRECISION;
END;
$company_quality_stage_score$;
ALTER FUNCTION public.lab_arena__company_quality_stage_score(DOUBLE PRECISION[])
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena__company_quality_stage_score(DOUBLE PRECISION[])
  FROM PUBLIC, lab_arena_service;
  ...
  SELECT
    ...
    (SELECT CASE WHEN pg_catalog.count(*) FILTER (WHERE status = 'accepted') > 0
            THEN CASE WHEN EXISTS (
              SELECT 1 FROM public.lab_arena_rounds AS quality_round
              WHERE quality_round.round_id = p_round_id
                AND quality_round.configuration_doc ->> 'company_quality_policy'
                  = 'company_quality_v1'
            ) THEN public.lab_arena__company_quality_stage_score(
              pg_catalog.array_agg(per_icp_score ORDER BY per_icp_score)
            ) ELSE pg_catalog.avg(per_icp_score)::DOUBLE PRECISION END END
       FROM selected),
