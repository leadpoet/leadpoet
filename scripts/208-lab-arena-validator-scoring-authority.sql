...
-- PostgreSQL still accepts claims only through the service-role RPC and keeps
-- all scheduling, lease, retry, review, and miner self-dealing constraints.

BEGIN;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '30s';

DO $lab_arena_208_requires_207$
BEGIN
  IF pg_catalog.to_regprocedure(
       'public.lab_arena_code_review_schema_v1()'
     ) IS NULL THEN
    RAISE EXCEPTION 'apply 207-lab-arena-code-review.sql first';
  END IF;
END;
$lab_arena_208_requires_207$;
...
BEGIN
  ...
  END IF;
  -- Only lab_arena_service can call this RPC. The gateway verifies the signed
  -- hotkey against its canonical subnet registry before it reaches this lock.
...
CREATE OR REPLACE FUNCTION public.lab_arena_validator_scoring_authority_schema_v1()
RETURNS JSONB
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = pg_catalog
AS $lab_arena_validator_scoring_authority_schema$
  SELECT pg_catalog.jsonb_build_object(
    'schema_version', 'leadpoet.lab_arena.validator_scoring_authority.v1',
    'version', 208,
    'authority', 'gateway_subnet_validator_role'
  );
$lab_arena_validator_scoring_authority_schema$;
ALTER FUNCTION public.lab_arena_validator_scoring_authority_schema_v1()
  OWNER TO lab_arena_owner;

DO $lab_arena_validator_scoring_authority_acl$
DECLARE
  signature TEXT;
  role_name TEXT;
BEGIN
  ...
  END LOOP;
END;
$lab_arena_validator_scoring_authority_acl$;

COMMENT ON FUNCTION public.lab_arena_claim_assignment(
  TEXT, TEXT, INTEGER, INTEGER, TEXT[], TEXT, TEXT, TEXT, INTEGER
) IS
  'Claims Arena work after the gateway verifies the signed hotkey as a current subnet validator; preserves lease, scheduling, review, and self-dealing controls.';
