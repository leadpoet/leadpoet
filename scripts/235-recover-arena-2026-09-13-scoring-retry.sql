...
-- company/source-local verifier-failure isolation and prompt-budget fixes.
-- Preserve every accepted execution and score. Requeue only the 84 incomplete
-- score assignments under a fresh provider-call namespace and bind those
-- claims to the fixed scorer image.
...
DO $lab_arena_235_recover_20260913_scoring$
DECLARE
  v_round_id CONSTANT TEXT := 'arena-2026-09-13';
  v_suffix CONSTANT TEXT := ':recovery235';
...
BEGIN
  IF v_new_digest !~ '^sha256:[0-9a-f]{64}$'
     OR v_new_digest IS NOT DISTINCT FROM v_old_digest
     OR v_new_reference IS DISTINCT FROM
       '493765492819.dkr.ecr.us-east-1.amazonaws.com/leadpoet/'
       || 'sourcing-model@' || v_new_digest THEN
    RAISE EXCEPTION 'fill migration 235 scorer image constants';
  END IF;
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION 'apply the current Arena schema before migration 235';
  END IF;
  ...
  IF v_round.status <> 'cancelled'
     ...
     OR v_round.status_generation <> 7
     OR v_round.stage_generation <> 6
  ...
  IF (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
     ...
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
         WHERE round_id = v_round_id AND stage = 1 AND kind = 'score'
           AND status = 'accepted' AND terminal_cause = 'accepted') <> 26
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
         WHERE round_id = v_round_id AND stage = 1 AND kind = 'score'
           AND status = 'failed' AND terminal_cause = 'judge_error') <> 8
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
         WHERE round_id = v_round_id AND stage = 1 AND kind = 'score'
           AND status = 'failed' AND terminal_cause = 'stage_closed') <> 82
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_judgment_cache
         WHERE scope_doc ->> 'round_id' = v_round_id) <> 15
     OR EXISTS (
       SELECT 1 FROM public.lab_arena_runs
       WHERE round_id = v_round_id AND stage = 1 AND kind = 'score'
  ...
  IF (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE assignment_id IN (
        'arena-2026-09-13:sub-00ee4222188195476cdcf63fdb94171f:1:0:score:recovery234',
        'arena-2026-09-13:sub-5c1f20eb379bcd4e11c29912b5251b0d:1:1:score:recovery234'
      ) AND attempt = 2 AND status = 'failed'
  ...
  IF pg_catalog.array_length(v_incomplete_assignments, 1) <> 84
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
         WHERE round_id = v_round_id AND stage = 1 AND kind = 'score'
           AND assignment_id = ANY(v_incomplete_assignments)) <> 86
     OR EXISTS (
       SELECT 1 FROM pg_catalog.unnest(v_incomplete_assignments) AS assignment(id)
       WHERE assignment.id NOT LIKE '%:recovery234'
     )
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
     ...
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
         WHERE run_id = ANY(v_latest_run_ids)
           AND terminal_cause = 'stage_closed') <> 82
     OR EXISTS (
       SELECT 1
  ...
  -- arena/arena-2026-09-13/recovery/company-verification-retry-20260913.json
  -- (SHA-256 470ea98aaeeb2f089c32a5599a7fbdb75b75e50621264a4855292fddc19192ba).
  ...
  WHERE round_id = v_round_id AND status = 'cancelled'
    ...
    AND status_generation = 7 AND stage_generation = 6;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  ALTER TABLE public.lab_arena_rounds
  ...
  IF (SELECT pg_catalog.count(DISTINCT assignment_id)
      FROM public.lab_arena_runs
      WHERE round_id = v_round_id AND stage = 1 AND kind = 'score'
        AND assignment_id LIKE '%' || v_suffix) <> 84
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
     ...
     OR NOT EXISTS (
       ...
       WHERE round_id = v_round_id AND status = 'stage1_scoring'
         AND status_generation = 8 AND stage_generation = 7
  ...
  END IF;
END;
$lab_arena_235_recover_20260913_scoring$;
