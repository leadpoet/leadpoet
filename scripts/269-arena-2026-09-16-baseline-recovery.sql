-- Exact second baseline rerun recovery for arena-2026-09-16.
-- Render only after the first native rerun is terminal and all active provider
...
BEGIN
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION 'apply current Arena migrations through 268 before 269';
  END IF;
END;
...
CREATE TABLE IF NOT EXISTS public.lab_arena_sep16_baseline_recovery_authority (
  round_id TEXT PRIMARY KEY CHECK (round_id = 'arena-2026-09-16'),
  archive_round_id TEXT NOT NULL UNIQUE CHECK (
    archive_round_id = 'arena-2026-09-16-rerun265archive'
  ),
  archive_submission_id TEXT NOT NULL UNIQUE CHECK (
    archive_submission_id = 'baseline-2026-09-16-native-rerun265-archive'
  ),
  execute_namespace TEXT NOT NULL CHECK (execute_namespace = 'rerun269'),
  terminal_source_ref TEXT NOT NULL CHECK (
    terminal_source_ref = 'arena/arena-2026-09-16/sources/baseline-2026-09-16-native-rerun268.tar.gz'
  ),
  terminal_source_size_bytes BIGINT NOT NULL CHECK (
    terminal_source_size_bytes = 524266
  ),
  terminal_source_sha256 TEXT NOT NULL CHECK (
    terminal_source_sha256 = 'c8d188c4766008ac949c5ff794536fc905589f46657d4b1ed27f3a22385f11b0'
  ),
  terminal_source_commit TEXT NOT NULL CHECK (
    terminal_source_commit = '3afe71508636254eb31a172d94f5a63f4f84a5c2'
  ),
  recovery_source_ref TEXT NOT NULL CHECK (
    recovery_source_ref = 'arena/arena-2026-09-16/sources/baseline-2026-09-16-recovery269.tar.gz'
  ),
  recovery_source_size_bytes BIGINT NOT NULL CHECK (
    recovery_source_size_bytes = 525059
  ),
  recovery_source_sha256 TEXT NOT NULL CHECK (
    recovery_source_sha256 = 'd5a132191ff9fa246ab348fcb91c3111db11d2175c5ed3375313134c411378d8'
  ),
  ...
  terminal_round_hash TEXT NOT NULL CHECK (
    terminal_round_hash = 'sha256:a1d551e501097ef099a10d0b387341ec3dd2d9bb3308111b2130eed06f514078'
  ),
  terminal_baseline_submission_hash TEXT NOT NULL CHECK (
    terminal_baseline_submission_hash = 'sha256:40d6d413816532c6a84bda3782efcbb5a4f98aa2a397db98e5d4b3c268a84330'
  ),
  terminal_baseline_runs_hash TEXT NOT NULL CHECK (
    terminal_baseline_runs_hash = 'sha256:84115f7707da90d800c5991386230a5a02858e2563ef2ebf5ca660386e5c25a4'
  ),
  terminal_baseline_ledger_hash TEXT NOT NULL CHECK (
    terminal_baseline_ledger_hash = 'sha256:0ef50e5bd9146cefb635759f5577d63ed34c53584dde14bd2c43428f5defdbff'
  ),
  terminal_baseline_run_count BIGINT NOT NULL CHECK (
    terminal_baseline_run_count = 40
  ),
  terminal_baseline_ledger_count BIGINT NOT NULL CHECK (
    terminal_baseline_ledger_count = 516
  ),
  terminal_baseline_ledger_max_entry_id BIGINT NOT NULL CHECK (
    terminal_baseline_ledger_max_entry_id = 444521
  ),
  terminal_baseline_settled_microusd BIGINT NOT NULL CHECK (
    terminal_baseline_settled_microusd = 303547
  ),
  terminal_baseline_uncertain_microusd BIGINT NOT NULL CHECK (
    terminal_baseline_uncertain_microusd = 7000
  ),
  ...
  terminal_status_generation BIGINT NOT NULL CHECK (
    terminal_status_generation = 14
  ),
  terminal_stage_generation BIGINT NOT NULL CHECK (
    terminal_stage_generation = 10
  ),
  terminal_cancel_reason TEXT NOT NULL CHECK (
    terminal_cancel_reason = 'execution_incomplete:stage1:20'
  ),
  old_schedule JSONB NOT NULL CHECK (
    old_schedule = $old_schedule${"benchmark_deadline":"2026-09-16T17:45:00Z","final_scoring_close":"2026-09-17T02:15:00Z","publication_deadline":"2026-09-17T02:45:00Z","stage_1_close":"2026-09-16T23:00:00Z","stage_1_scoring_close":"2026-09-17T00:30:00Z","stage_1_start":"2026-09-16T17:45:01Z","stage_2_close":"2026-09-17T00:45:00Z","stage_2_start":"2026-09-17T00:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$old_schedule$::JSONB
  ),
  forward_schedule JSONB NOT NULL CHECK (
    forward_schedule = $new_schedule${"benchmark_deadline":"2026-09-16T19:25:00Z","final_scoring_close":"2026-09-17T02:15:00Z","publication_deadline":"2026-09-17T02:45:00Z","stage_1_close":"2026-09-16T23:00:00Z","stage_1_scoring_close":"2026-09-17T00:30:00Z","stage_1_start":"2026-09-16T19:25:01Z","stage_2_close":"2026-09-17T00:45:00Z","stage_2_start":"2026-09-17T00:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$new_schedule$::JSONB
  ),
  authorized_at TIMESTAMPTZ NOT NULL DEFAULT pg_catalog.clock_timestamp()
);
ALTER TABLE public.lab_arena_sep16_baseline_recovery_authority
  OWNER TO lab_arena_owner;
ALTER TABLE public.lab_arena_sep16_baseline_recovery_authority
  ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.lab_arena_sep16_baseline_recovery_authority
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

CREATE TABLE IF NOT EXISTS public.lab_arena_sep16_baseline_recovery_audit (
  round_id TEXT PRIMARY KEY CHECK (round_id = 'arena-2026-09-16'),
...
ALTER TABLE public.lab_arena_sep16_baseline_recovery_audit
  OWNER TO lab_arena_owner;
ALTER TABLE public.lab_arena_sep16_baseline_recovery_audit ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.lab_arena_sep16_baseline_recovery_audit
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

INSERT INTO public.lab_arena_sep16_baseline_recovery_authority (
  ...
  release_authority_hash, terminal_status_generation,
  terminal_stage_generation, terminal_cancel_reason, old_schedule,
  forward_schedule
) VALUES (
  'arena-2026-09-16', 'arena-2026-09-16-rerun265archive',
  'baseline-2026-09-16-native-rerun265-archive', 'rerun269',
  'arena/arena-2026-09-16/sources/baseline-2026-09-16-native-rerun268.tar.gz',
  524266, 'c8d188c4766008ac949c5ff794536fc905589f46657d4b1ed27f3a22385f11b0',
  ...
  'sha256:a1d551e501097ef099a10d0b387341ec3dd2d9bb3308111b2130eed06f514078',
  'sha256:40d6d413816532c6a84bda3782efcbb5a4f98aa2a397db98e5d4b3c268a84330',
  'sha256:84115f7707da90d800c5991386230a5a02858e2563ef2ebf5ca660386e5c25a4',
  'sha256:0ef50e5bd9146cefb635759f5577d63ed34c53584dde14bd2c43428f5defdbff',
  40,
  516,
  444521,
  303547,
  ...
  14,
  10,
  'execution_incomplete:stage1:20',
  $old_schedule${"benchmark_deadline":"2026-09-16T17:45:00Z","final_scoring_close":"2026-09-17T02:15:00Z","publication_deadline":"2026-09-17T02:45:00Z","stage_1_close":"2026-09-16T23:00:00Z","stage_1_scoring_close":"2026-09-17T00:30:00Z","stage_1_start":"2026-09-16T17:45:01Z","stage_2_close":"2026-09-17T00:45:00Z","stage_2_start":"2026-09-17T00:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$old_schedule$::JSONB,
  $new_schedule${"benchmark_deadline":"2026-09-16T19:25:00Z","final_scoring_close":"2026-09-17T02:15:00Z","publication_deadline":"2026-09-17T02:45:00Z","stage_1_close":"2026-09-16T23:00:00Z","stage_1_scoring_close":"2026-09-17T00:30:00Z","stage_1_start":"2026-09-16T19:25:01Z","stage_2_close":"2026-09-17T00:45:00Z","stage_2_start":"2026-09-17T00:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$new_schedule$::JSONB
) ON CONFLICT (round_id) DO NOTHING;

DO $seal_terminal_sep16_rerun$
DECLARE
  v_authority public.lab_arena_sep16_baseline_recovery_authority%ROWTYPE;
...
BEGIN
  ...
  FROM public.lab_arena_sep16_baseline_recovery_authority
  WHERE round_id = 'arena-2026-09-16' FOR UPDATE;
  IF EXISTS (
    SELECT 1 FROM public.lab_arena_sep16_baseline_recovery_audit
     ...
     OR EXISTS (
       ...
       WHERE round_id = v_authority.round_id
         ...
         AND (kind <> 'execute' OR attempt NOT IN (1, 2)
              OR assignment_id NOT LIKE '%:rerun265')
     ) THEN
    RAISE EXCEPTION 'Sep16 terminal native rerun seal differs before recovery269'
      USING ERRCODE = '55000';
  END IF;
END;
$seal_terminal_sep16_rerun$;

CREATE OR REPLACE FUNCTION public.lab_arena_sep16_recovery_archive_valid_v1()
RETURNS BOOLEAN
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $sep16_recovery_archive_valid$
DECLARE
  v_audit public.lab_arena_sep16_baseline_recovery_audit%ROWTYPE;
  v_runs_hash TEXT;
  v_ledger_hash TEXT;
  v_submission_doc JSONB;
BEGIN
  SELECT * INTO v_audit
  FROM public.lab_arena_sep16_baseline_recovery_audit
...
ALTER FUNCTION public.lab_arena_sep16_recovery_archive_valid_v1()
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep16_recovery_archive_valid_v1()
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_prepare_sep16_baseline_recovery_v1(
...
BEGIN
  ...
  IF p_forward_schedule IS DISTINCT FROM v_authority.forward_schedule THEN
    RAISE EXCEPTION 'Sep16 recovery schedule differs from authority'
      USING ERRCODE = '22023';
  END IF;
  LOCK TABLE public.lab_arena_sep16_baseline_recovery_audit
    ...
    IF v_audit.archive_round_id <> v_authority.archive_round_id
       ...
       OR (SELECT pg_catalog.count(DISTINCT assignment_id)
           ...
           WHERE round_id = v_round_id AND submission_id = v_baseline_id
             AND kind = 'execute' AND assignment_id LIKE '%:rerun269') <> 20 THEN
  ...
  IF v_round_hash <> v_authority.terminal_round_hash
    ...
    RAISE EXCEPTION 'Sep16 terminal rerun changed before recovery'
      USING ERRCODE = '55000';
  END IF;
  INSERT INTO public.lab_arena_sep16_baseline_recovery_audit (
    round_id, archive_round_id, archive_submission_id, execute_namespace,
    ...
    challenger_ledger_max_entry_id, prior_rerun_audit_hash,
    release_authority_hash, bank_sha256,
  ...
  FOR v_position IN 0 .. 19 LOOP
    ...
    v_assignment := v_round_id || ':' || v_baseline_id || ':' ||
      v_stage::TEXT || ':' || v_position::TEXT || ':rerun269';
  ...
  IF NOT public.lab_arena_sep16_recovery_archive_valid_v1()
     OR NOT public.lab_arena_sep16_challenger_seals_valid_v1()
     OR NOT EXISTS (
       SELECT 1 FROM public.lab_arena_submissions
       WHERE submission_id = v_baseline_id AND round_id = v_round_id
     ...
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
         WHERE round_id = v_round_id AND submission_id = v_baseline_id
           ...
           AND assignment_id LIKE '%:rerun269') <> 20
     OR (SELECT pg_catalog.count(DISTINCT assignment_id)
         FROM public.lab_arena_runs
    ...
    RAISE EXCEPTION 'Sep16 recovery post-archive verification differs: %',
      pg_catalog.jsonb_build_object(
        'archive_valid', public.lab_arena_sep16_recovery_archive_valid_v1(),
        ...
        'pending_recovery', (SELECT pg_catalog.count(*)
          FROM public.lab_arena_runs WHERE round_id = v_round_id
            ...
            AND status = 'pending' AND assignment_id LIKE '%:rerun269'),
...
ALTER FUNCTION public.lab_arena_prepare_sep16_baseline_recovery_v1(
  BIGINT, TEXT, TEXT, JSONB
)
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_prepare_sep16_baseline_recovery_v1(
  BIGINT, TEXT, TEXT, JSONB
)
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;
GRANT EXECUTE ON FUNCTION public.lab_arena_prepare_sep16_baseline_recovery_v1(
  BIGINT, TEXT, TEXT, JSONB
)
...
BEGIN
  FOREACH v_proc IN ARRAY ARRAY[
    ...
    v_old_count := (
      ...
      pg_catalog.length(pg_catalog.replace(v_definition, ':rerun265', ''))
    ) / pg_catalog.length(':rerun265');
    v_new_count := (
      pg_catalog.length(v_definition) -
    ...
    IF v_old_count > 0 AND v_new_count = 0 THEN
      EXECUTE pg_catalog.replace(v_definition, ':rerun265', ':rerun269');
    ELSIF NOT (v_old_count = 0 AND v_new_count > 0) THEN
...
DECLARE
  v_definition TEXT;
  v_old TEXT := $old$OR NOT public.lab_arena_sep16_challenger_seals_valid_v1()$old$;
  v_new TEXT := $new$OR NOT public.lab_arena_sep16_challenger_seals_valid_v1()
       OR NOT public.lab_arena_sep16_recovery_archive_valid_v1()$new$;
BEGIN
