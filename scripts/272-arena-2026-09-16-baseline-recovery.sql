-- Exact third baseline rerun recovery for arena-2026-09-16.
-- Render only after the second native rerun is terminal and all active provider
...
BEGIN
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION 'apply current Arena migrations through 271 before 272';
  END IF;
END;
...
CREATE TABLE IF NOT EXISTS public.lab_arena_sep16_baseline_recovery272_authority (
  round_id TEXT PRIMARY KEY CHECK (round_id = 'arena-2026-09-16'),
  archive_round_id TEXT NOT NULL UNIQUE CHECK (
    archive_round_id = 'arena-2026-09-16-rerun269archive'
  ),
  archive_submission_id TEXT NOT NULL UNIQUE CHECK (
    archive_submission_id = 'baseline-2026-09-16-native-rerun269-archive'
  ),
  execute_namespace TEXT NOT NULL CHECK (execute_namespace = 'rerun272'),
  terminal_source_ref TEXT NOT NULL CHECK (
    terminal_source_ref = 'arena/arena-2026-09-16/sources/baseline-2026-09-16-recovery269.tar.gz'
  ),
  terminal_source_size_bytes BIGINT NOT NULL CHECK (
    terminal_source_size_bytes = 525059
  ),
  terminal_source_sha256 TEXT NOT NULL CHECK (
    terminal_source_sha256 = 'd5a132191ff9fa246ab348fcb91c3111db11d2175c5ed3375313134c411378d8'
  ),
  terminal_source_commit TEXT NOT NULL CHECK (
    terminal_source_commit = '851038141112868051dd5223c839682810042eea'
  ),
  recovery_source_ref TEXT NOT NULL CHECK (
    recovery_source_ref = 'arena/arena-2026-09-16/sources/baseline-2026-09-16-recovery272.tar.gz'
  ),
  recovery_source_size_bytes BIGINT NOT NULL CHECK (
    recovery_source_size_bytes = 533090
  ),
  recovery_source_sha256 TEXT NOT NULL CHECK (
    recovery_source_sha256 = '72a008e2cacde52921c4be94951839edccc321d90de07741010a3100463eef32'
  ),
  ...
  terminal_round_hash TEXT NOT NULL CHECK (
    terminal_round_hash = 'sha256:c9cc8a7371f2a99ac539b329abac99ec27d5a3e8569b932ad39a63cdb9f84777'
  ),
  terminal_baseline_submission_hash TEXT NOT NULL CHECK (
    terminal_baseline_submission_hash = 'sha256:76dfb679f887f70b4b6c5a2991dda79ef47470eabf1b4f42276df2c90e9821eb'
  ),
  terminal_baseline_runs_hash TEXT NOT NULL CHECK (
    terminal_baseline_runs_hash = 'sha256:b3210233c8498771e1b8b78bf8c137748f3ee452985c7bb10bd611b251bf8f0f'
  ),
  terminal_baseline_ledger_hash TEXT NOT NULL CHECK (
    terminal_baseline_ledger_hash = 'sha256:c646af6fa020de2c3273b03f079245d94dc345d45b5413149f2e4a4a27b73176'
  ),
  terminal_baseline_run_count BIGINT NOT NULL CHECK (
    terminal_baseline_run_count = 29
  ),
  terminal_baseline_ledger_count BIGINT NOT NULL CHECK (
    terminal_baseline_ledger_count = 3864
  ),
  terminal_baseline_ledger_max_entry_id BIGINT NOT NULL CHECK (
    terminal_baseline_ledger_max_entry_id = 448385
  ),
  terminal_baseline_settled_microusd BIGINT NOT NULL CHECK (
    terminal_baseline_settled_microusd = 10013700
  ),
  terminal_baseline_uncertain_microusd BIGINT NOT NULL CHECK (
    terminal_baseline_uncertain_microusd = 1532902
  ),
  ...
  prior_recovery_authority_hash TEXT NOT NULL CHECK (
    prior_recovery_authority_hash = 'sha256:12396acac34ace966d5a2d97d3c4723701df7333fb3341e85da5a5d0104b733c'
  ),
  prior_recovery_audit_hash TEXT NOT NULL CHECK (
    prior_recovery_audit_hash = 'sha256:5b1e941d7ffc9e0eba7d1e9b479e5876bb92f1f6bcaf8aa923240840f1745856'
  ),
  terminal_status_generation BIGINT NOT NULL CHECK (
    terminal_status_generation = 16
  ),
  terminal_stage_generation BIGINT NOT NULL CHECK (
    terminal_stage_generation = 12
  ),
  terminal_cancel_reason TEXT NOT NULL CHECK (
    terminal_cancel_reason = 'operator'
  ),
  old_schedule JSONB NOT NULL CHECK (
    old_schedule = $old_schedule${"benchmark_deadline":"2026-09-16T19:25:00Z","final_scoring_close":"2026-09-17T02:15:00Z","publication_deadline":"2026-09-17T02:45:00Z","stage_1_close":"2026-09-16T23:00:00Z","stage_1_scoring_close":"2026-09-17T00:30:00Z","stage_1_start":"2026-09-16T19:25:01Z","stage_2_close":"2026-09-17T00:45:00Z","stage_2_start":"2026-09-17T00:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$old_schedule$::JSONB
  ),
  forward_schedule JSONB NOT NULL CHECK (
    forward_schedule = $new_schedule${"benchmark_deadline":"2026-09-16T21:35:00Z","final_scoring_close":"2026-09-17T06:15:00Z","publication_deadline":"2026-09-17T06:45:00Z","stage_1_close":"2026-09-17T03:00:00Z","stage_1_scoring_close":"2026-09-17T04:30:00Z","stage_1_start":"2026-09-16T21:35:01Z","stage_2_close":"2026-09-17T04:45:00Z","stage_2_start":"2026-09-17T04:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$new_schedule$::JSONB
  ),
  authorized_at TIMESTAMPTZ NOT NULL DEFAULT pg_catalog.clock_timestamp()
);
ALTER TABLE public.lab_arena_sep16_baseline_recovery272_authority
  OWNER TO lab_arena_owner;
ALTER TABLE public.lab_arena_sep16_baseline_recovery272_authority
  ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.lab_arena_sep16_baseline_recovery272_authority
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

CREATE TABLE IF NOT EXISTS public.lab_arena_sep16_baseline_recovery272_audit (
  round_id TEXT PRIMARY KEY CHECK (round_id = 'arena-2026-09-16'),
...
ALTER TABLE public.lab_arena_sep16_baseline_recovery272_audit
  OWNER TO lab_arena_owner;
ALTER TABLE public.lab_arena_sep16_baseline_recovery272_audit ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.lab_arena_sep16_baseline_recovery272_audit
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

INSERT INTO public.lab_arena_sep16_baseline_recovery272_authority (
  ...
  'arena-2026-09-16', 'arena-2026-09-16-rerun269archive',
  'baseline-2026-09-16-native-rerun269-archive', 'rerun272',
  'arena/arena-2026-09-16/sources/baseline-2026-09-16-recovery269.tar.gz',
  525059, 'd5a132191ff9fa246ab348fcb91c3111db11d2175c5ed3375313134c411378d8',
  '851038141112868051dd5223c839682810042eea',
  ...
  'sha256:c9cc8a7371f2a99ac539b329abac99ec27d5a3e8569b932ad39a63cdb9f84777',
  'sha256:76dfb679f887f70b4b6c5a2991dda79ef47470eabf1b4f42276df2c90e9821eb',
  'sha256:b3210233c8498771e1b8b78bf8c137748f3ee452985c7bb10bd611b251bf8f0f',
  'sha256:c646af6fa020de2c3273b03f079245d94dc345d45b5413149f2e4a4a27b73176',
  29,
  3864,
  448385,
  10013700,
  ...
  'sha256:12396acac34ace966d5a2d97d3c4723701df7333fb3341e85da5a5d0104b733c',
  'sha256:5b1e941d7ffc9e0eba7d1e9b479e5876bb92f1f6bcaf8aa923240840f1745856',
  16,
  12,
  'operator',
  $old_schedule${"benchmark_deadline":"2026-09-16T19:25:00Z","final_scoring_close":"2026-09-17T02:15:00Z","publication_deadline":"2026-09-17T02:45:00Z","stage_1_close":"2026-09-16T23:00:00Z","stage_1_scoring_close":"2026-09-17T00:30:00Z","stage_1_start":"2026-09-16T19:25:01Z","stage_2_close":"2026-09-17T00:45:00Z","stage_2_start":"2026-09-17T00:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$old_schedule$::JSONB,
  $new_schedule${"benchmark_deadline":"2026-09-16T21:35:00Z","final_scoring_close":"2026-09-17T06:15:00Z","publication_deadline":"2026-09-17T06:45:00Z","stage_1_close":"2026-09-17T03:00:00Z","stage_1_scoring_close":"2026-09-17T04:30:00Z","stage_1_start":"2026-09-16T21:35:01Z","stage_2_close":"2026-09-17T04:45:00Z","stage_2_start":"2026-09-17T04:35:00Z","submission_cutoff":"2026-09-16T00:00:00Z","submission_open":"2026-09-15T00:00:00Z"}$new_schedule$::JSONB
) ON CONFLICT (round_id) DO NOTHING;

DO $seal_terminal_sep16_rerun$
DECLARE
  v_authority public.lab_arena_sep16_baseline_recovery272_authority%ROWTYPE;
...
BEGIN
  ...
  FROM public.lab_arena_sep16_baseline_recovery272_authority
  WHERE round_id = 'arena-2026-09-16' FOR UPDATE;
  IF EXISTS (
    SELECT 1 FROM public.lab_arena_sep16_baseline_recovery272_audit
  ...
  FROM public.lab_arena_sep16_baseline_recovery_authority AS row_value
  WHERE round_id = v_authority.round_id;
  SELECT 'sha256:' || pg_catalog.encode(extensions.digest(
    pg_catalog.to_jsonb(row_value)::TEXT, 'sha256'), 'hex')
  INTO v_prior_recovery_audit_hash
  FROM public.lab_arena_sep16_baseline_recovery_audit AS row_value
  WHERE round_id = v_authority.round_id;
  ...
  IF v_round_hash IS DISTINCT FROM v_authority.terminal_round_hash
     ...
     OR (SELECT pg_catalog.count(DISTINCT assignment_id)
         ...
         WHERE round_id = v_authority.round_id
           AND submission_id = 'baseline-2026-09-16'
           AND kind = 'execute' AND assignment_id LIKE '%:rerun269') <> 20
     OR EXISTS (
       SELECT 1 FROM public.lab_arena_runs
       WHERE round_id = v_authority.round_id
         AND submission_id = 'baseline-2026-09-16'
         AND (kind <> 'execute' OR attempt NOT IN (1, 2)
              OR assignment_id NOT LIKE '%:rerun269')
     ) THEN
    RAISE EXCEPTION 'Sep16 terminal native rerun seal differs before recovery272'
      USING ERRCODE = '55000';
  END IF;
END;
$seal_terminal_sep16_rerun$;

CREATE OR REPLACE FUNCTION public.lab_arena_sep16_recovery272_archive_valid_v1()
RETURNS BOOLEAN
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $sep16_recovery_archive_valid$
DECLARE
  v_audit public.lab_arena_sep16_baseline_recovery272_audit%ROWTYPE;
  v_runs_hash TEXT;
  v_ledger_hash TEXT;
  v_submission_doc JSONB;
BEGIN
  SELECT * INTO v_audit
  FROM public.lab_arena_sep16_baseline_recovery272_audit
...
END;
$sep16_recovery_archive_valid$;
ALTER FUNCTION public.lab_arena_sep16_recovery272_archive_valid_v1()
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep16_recovery272_archive_valid_v1()
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_prepare_sep16_baseline_recovery272_v1(
...
SET search_path = pg_catalog, public, extensions
AS $prepare_sep16_baseline_recovery$
DECLARE
...
BEGIN
  ...
  IF p_source_size_bytes IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'Sep16 recovery source differs from authority'
      USING ERRCODE = '22023';
  END IF;
  IF p_forward_schedule IS DISTINCT FROM v_authority.forward_schedule THEN
    RAISE EXCEPTION 'Sep16 recovery schedule differs from authority'
      USING ERRCODE = '22023';
  END IF;
  LOCK TABLE public.lab_arena_sep16_baseline_recovery272_audit
    ...
    IF v_audit.archive_round_id <> v_authority.archive_round_id
       ...
       OR (SELECT pg_catalog.count(DISTINCT assignment_id)
           ...
           WHERE round_id = v_round_id AND submission_id = v_baseline_id
             AND kind = 'execute' AND assignment_id LIKE '%:rerun272') <> 20 THEN
      RAISE EXCEPTION 'Sep16 recovery replay differs' USING ERRCODE = '55000';
    END IF;
  ...
  IF v_round_hash <> v_authority.terminal_round_hash
    ...
    RAISE EXCEPTION 'Sep16 terminal rerun changed before recovery'
      USING ERRCODE = '55000';
  END IF;
  INSERT INTO public.lab_arena_sep16_baseline_recovery272_audit (
    round_id, archive_round_id, archive_submission_id, execute_namespace,
  ...
  IF v_count <> v_authority.terminal_baseline_run_count THEN
    RAISE EXCEPTION 'Sep16 recovery archive run count differs';
  END IF;
  ...
  IF v_count <> v_authority.terminal_baseline_ledger_count THEN
    RAISE EXCEPTION 'Sep16 recovery archive ledger count differs';
  END IF;
  ...
  WHERE submission_id = v_baseline_id AND round_id = v_round_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Sep16 recovery baseline source switch failed';
  END IF;
  ...
  IF (v_new_configuration - 'schedule') IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'Sep16 recovery changed frozen configuration';
  END IF;
  ...
  FOR v_position IN 0 .. 19 LOOP
    ...
    v_assignment := v_round_id || ':' || v_baseline_id || ':' ||
      v_stage::TEXT || ':' || v_position::TEXT || ':rerun272';
  ...
  IF NOT public.lab_arena_sep16_recovery272_archive_valid_v1()
     OR NOT public.lab_arena_sep16_recovery_archive_valid_v1()
     OR NOT public.lab_arena_sep16_challenger_seals_valid_v1()
     OR (SELECT pg_catalog.count(*)
         FROM public.lab_arena_ledger
     ...
     OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
         WHERE round_id = v_round_id AND submission_id = v_baseline_id
           ...
           AND assignment_id LIKE '%:rerun272') <> 20
     OR (SELECT pg_catalog.count(DISTINCT assignment_id)
         FROM public.lab_arena_runs
    ...
    RAISE EXCEPTION 'Sep16 recovery post-archive verification differs: %',
      pg_catalog.jsonb_build_object(
        'archive_valid', public.lab_arena_sep16_recovery272_archive_valid_v1(),
        ...
        'pending_recovery', (SELECT pg_catalog.count(*)
          FROM public.lab_arena_runs WHERE round_id = v_round_id
            ...
            AND status = 'pending' AND assignment_id LIKE '%:rerun272'),
...
END;
$prepare_sep16_baseline_recovery$;
ALTER FUNCTION public.lab_arena_prepare_sep16_baseline_recovery272_v1(
  BIGINT, TEXT, TEXT, JSONB
)
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_prepare_sep16_baseline_recovery272_v1(
  BIGINT, TEXT, TEXT, JSONB
)
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;
GRANT EXECUTE ON FUNCTION public.lab_arena_prepare_sep16_baseline_recovery272_v1(
  BIGINT, TEXT, TEXT, JSONB
)
...
DO $replace_sep16_recovery_namespaces$
DECLARE
  v_proc REGPROCEDURE;
  v_definition TEXT;
  v_old_count INTEGER;
...
BEGIN
  FOREACH v_proc IN ARRAY ARRAY[
    ...
    v_old_count := (
      ...
      pg_catalog.length(pg_catalog.replace(v_definition, ':rerun269', ''))
    ) / pg_catalog.length(':rerun269');
    v_new_count := (
      pg_catalog.length(v_definition) -
      pg_catalog.length(pg_catalog.replace(v_definition, ':rerun272', ''))
    ) / pg_catalog.length(':rerun272');
    IF v_old_count = v_expected_count AND v_new_count = 0 THEN
      EXECUTE pg_catalog.replace(v_definition, ':rerun269', ':rerun272');
    ELSIF NOT (
  ...
  END LOOP;
END;
$replace_sep16_recovery_namespaces$;

DO $bind_sep16_recovery_archive_guard$
DECLARE
  v_definition TEXT;
  v_old TEXT := $old$OR NOT public.lab_arena_sep16_recovery_archive_valid_v1()$old$;
  v_new TEXT := $new$OR NOT public.lab_arena_sep16_recovery_archive_valid_v1()
       OR NOT public.lab_arena_sep16_recovery272_archive_valid_v1()$new$;
  v_old_count INTEGER;
  v_new_count INTEGER;
...
BEGIN
  ...
  ELSIF NOT (v_new_count = 1 AND v_new_function_count = 1) THEN
    RAISE EXCEPTION 'Sep16 publication recovery272 guard shape differs'
      USING ERRCODE = '55000';
  END IF;
  SELECT pg_catalog.pg_get_functiondef(
    'public.lab_arena_sep16_rerun_publication_guard_v1()'::REGPROCEDURE
  ...
  IF v_old_count <> 1 OR v_new_count <> 1 OR v_new_function_count <> 1 THEN
    RAISE EXCEPTION 'Sep16 publication recovery272 guard binding failed'
      USING ERRCODE = '55000';
  END IF;
END;
$bind_sep16_recovery_archive_guard$;
