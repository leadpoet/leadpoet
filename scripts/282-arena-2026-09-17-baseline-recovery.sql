...
BEGIN
  IF pg_catalog.to_regprocedure(
     ...
     OR pg_catalog.to_regprocedure(
       'public.lab_arena_sep17_recovery278_archive_valid_v1()'
     ) IS NULL
     OR pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION 'apply current Arena schema before recovery282';
  END IF;
END;
$requires_sep17_recovery282$;

GRANT CREATE ON SCHEMA public TO lab_arena_owner;

CREATE TABLE IF NOT EXISTS public.lab_arena_sep17_baseline_recovery282_authority (
  round_id TEXT PRIMARY KEY CHECK (round_id = 'arena-2026-09-17'),
  archive_round_id TEXT NOT NULL UNIQUE CHECK (
    archive_round_id = 'arena-2026-09-17-rerun278archive'
  ),
  archive_submission_id TEXT NOT NULL UNIQUE CHECK (
    archive_submission_id = 'baseline-2026-09-17-rerun278archive'
  ),
  execute_namespace TEXT NOT NULL CHECK (execute_namespace = 'rerun282'),
  terminal_source_ref TEXT NOT NULL CHECK (
    terminal_source_ref =
      'arena/arena-2026-09-17/sources/baseline-2026-09-17-recovery278.tar.gz'
  ),
  terminal_source_size_bytes BIGINT NOT NULL CHECK (
    terminal_source_size_bytes = 563105
  ),
  ...
  recovery_source_size_bytes BIGINT NOT NULL CHECK (
    recovery_source_size_bytes = 580050
  ),
  recovery_source_sha256 TEXT NOT NULL CHECK (
    recovery_source_sha256 = 'cfa4a9bf8093dd230e8643bec5508b675e6f4b0ded75ac3d0f4c2a23616b6e41'
  ),
  recovery_source_commit TEXT NOT NULL CHECK (
    recovery_source_commit = '5a89cee202416419ade064e85ff40499f51d34b0'
  ),
  ...
  terminal_round_hash TEXT NOT NULL CHECK (
    terminal_round_hash =
      'sha256:c33a517002177c375ca644605cf155c747e0a6db43d0a4f832916ab14e9072f1'
  ),
  terminal_baseline_submission_hash TEXT NOT NULL CHECK (
    terminal_baseline_submission_hash =
      'sha256:f66e1c5815202dd7c04ee5532db16e952d190f5b3bc050b8b12fff09012db744'
  ),
  terminal_baseline_runs_hash TEXT NOT NULL CHECK (
    terminal_baseline_runs_hash =
      'sha256:57efd5c815fc4d77821acde40db42529c630c28484b9f3be9d8f28f45e77de3f'
  ),
  terminal_baseline_ledger_hash TEXT NOT NULL CHECK (
    terminal_baseline_ledger_hash =
      'sha256:4bdd23c56c842fa05524317ac99475ad7476895db1fec35383ea83d90614f38a'
  ),
  ...
  terminal_submissions_hash TEXT NOT NULL CHECK (
    terminal_submissions_hash =
      'sha256:e54ca0ba8daf046f4e14fd3ef93682952e0c6ff81214654f1a4aaa9756a67c3b'
  ),
  ...
  terminal_baseline_run_count BIGINT NOT NULL CHECK (
    terminal_baseline_run_count = 35
  ),
  terminal_baseline_ledger_count BIGINT NOT NULL CHECK (
    terminal_baseline_ledger_count = 6640
  ),
  ...
  terminal_baseline_ledger_max_entry_id BIGINT NOT NULL CHECK (
    terminal_baseline_ledger_max_entry_id = 472204
  ),
  terminal_baseline_settled_microusd BIGINT NOT NULL CHECK (
    terminal_baseline_settled_microusd = 18251793
  ),
  terminal_baseline_uncertain_microusd BIGINT NOT NULL CHECK (
    terminal_baseline_uncertain_microusd = 61714426
  ),
  ...
  terminal_execute_cost_doc JSONB NOT NULL CHECK (
    terminal_execute_cost_doc =
      $execute_cost${"call_count":2268,"inflight_calls":0,"refused_calls":82,"reserved_or_uncertain_microusd":61714426,"settled_microusd":18251793,"success_unresolved_calls":0,"success_unresolved_microusd":0,"successful_calls":2150,"successful_microusd":18251793,"uncertain_calls":32}$execute_cost$::JSONB
  ),
  ...
  terminal_status_generation BIGINT NOT NULL CHECK (
    terminal_status_generation = 5
  ),
  terminal_stage_generation BIGINT NOT NULL CHECK (
    terminal_stage_generation = 4
  ),
  terminal_cancel_reason TEXT NOT NULL CHECK (
    terminal_cancel_reason = 'execution_incomplete:stage1:9'
  ),
  old_schedule JSONB NOT NULL CHECK (
    old_schedule = $old_schedule${"benchmark_deadline":"2026-09-17T06:00:00Z","final_scoring_close":"2026-09-17T16:20:00Z","publication_deadline":"2026-09-17T16:50:00Z","stage_1_close":"2026-09-17T12:00:00Z","stage_1_scoring_close":"2026-09-17T14:00:00Z","stage_1_start":"2026-09-17T06:00:01Z","stage_2_close":"2026-09-17T14:20:00Z","stage_2_start":"2026-09-17T14:00:01Z","submission_cutoff":"2026-09-17T00:00:00Z","submission_open":"2026-09-16T00:00:00Z"}$old_schedule$::JSONB
  ),
  forward_schedule JSONB NOT NULL CHECK (
    forward_schedule = $new_schedule${"benchmark_deadline":"2026-09-17T10:00:00Z","final_scoring_close":"2026-09-17T20:20:00Z","publication_deadline":"2026-09-17T20:50:00Z","stage_1_close":"2026-09-17T16:00:00Z","stage_1_scoring_close":"2026-09-17T18:00:00Z","stage_1_start":"2026-09-17T10:00:01Z","stage_2_close":"2026-09-17T18:20:00Z","stage_2_start":"2026-09-17T18:00:01Z","submission_cutoff":"2026-09-17T00:00:00Z","submission_open":"2026-09-16T00:00:00Z"}$new_schedule$::JSONB
  ),
  authorized_at TIMESTAMPTZ NOT NULL DEFAULT pg_catalog.clock_timestamp()
);
ALTER TABLE public.lab_arena_sep17_baseline_recovery282_authority
  OWNER TO lab_arena_owner;
ALTER TABLE public.lab_arena_sep17_baseline_recovery282_authority
  ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.lab_arena_sep17_baseline_recovery282_authority
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

CREATE TABLE IF NOT EXISTS public.lab_arena_sep17_baseline_recovery282_audit (
  round_id TEXT PRIMARY KEY CHECK (round_id = 'arena-2026-09-17'),
...
ALTER TABLE public.lab_arena_sep17_baseline_recovery282_audit
  OWNER TO lab_arena_owner;
ALTER TABLE public.lab_arena_sep17_baseline_recovery282_audit
  ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.lab_arena_sep17_baseline_recovery282_audit
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

INSERT INTO public.lab_arena_sep17_baseline_recovery282_authority (
  ...
  'arena-2026-09-17', 'arena-2026-09-17-rerun278archive',
  'baseline-2026-09-17-rerun278archive', 'rerun282',
  'arena/arena-2026-09-17/sources/baseline-2026-09-17-recovery278.tar.gz',
  563105,
  'f6d8ca05a33907489138b37b2089151d78a84a05e5c73020147a282a29383361',
  '4ceae936b902433432a195f77af3f94559d80378',
  'arena/arena-2026-09-17/sources/baseline-2026-09-17-recovery282.tar.gz',
  580050, 'cfa4a9bf8093dd230e8643bec5508b675e6f4b0ded75ac3d0f4c2a23616b6e41',
  ...
  'sha256:e54ca0ba8daf046f4e14fd3ef93682952e0c6ff81214654f1a4aaa9756a67c3b',
  'sha256:7c0d8883f0984000ea0fa2a30726fb489f959b3ce484b8dd5d40d79596684be6',
  35, 6640, 15, 453599, 472204, 18251793, 61714426, 9, 8,
  $execute_cost${"call_count":2268,"inflight_calls":0,"refused_calls":82,"reserved_or_uncertain_microusd":61714426,"settled_microusd":18251793,"success_unresolved_calls":0,"success_unresolved_microusd":0,"successful_calls":2150,"successful_microusd":18251793,"uncertain_calls":32}$execute_cost$::JSONB,
  $score_cost${"call_count":0,"inflight_calls":0,"refused_calls":0,"reserved_or_uncertain_microusd":0,"settled_microusd":0,"success_unresolved_calls":0,"success_unresolved_microusd":0,"successful_calls":0,"successful_microusd":0,"uncertain_calls":0}$score_cost$::JSONB,
  5, 4, 'execution_incomplete:stage1:9',
  $old_schedule${"benchmark_deadline":"2026-09-17T06:00:00Z","final_scoring_close":"2026-09-17T16:20:00Z","publication_deadline":"2026-09-17T16:50:00Z","stage_1_close":"2026-09-17T12:00:00Z","stage_1_scoring_close":"2026-09-17T14:00:00Z","stage_1_start":"2026-09-17T06:00:01Z","stage_2_close":"2026-09-17T14:20:00Z","stage_2_start":"2026-09-17T14:00:01Z","submission_cutoff":"2026-09-17T00:00:00Z","submission_open":"2026-09-16T00:00:00Z"}$old_schedule$::JSONB,
  $new_schedule${"benchmark_deadline":"2026-09-17T10:00:00Z","final_scoring_close":"2026-09-17T20:20:00Z","publication_deadline":"2026-09-17T20:50:00Z","stage_1_close":"2026-09-17T16:00:00Z","stage_1_scoring_close":"2026-09-17T18:00:00Z","stage_1_start":"2026-09-17T10:00:01Z","stage_2_close":"2026-09-17T18:20:00Z","stage_2_start":"2026-09-17T18:00:01Z","submission_cutoff":"2026-09-17T00:00:00Z","submission_open":"2026-09-16T00:00:00Z"}$new_schedule$::JSONB
) ON CONFLICT (round_id) DO NOTHING;


-- Keep the already archived original attempt and its owner audit immutable.
CREATE OR REPLACE FUNCTION public.lab_arena_sep17_recovery282_prior_valid_v1()
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $sep17_recovery282_prior_valid$
  SELECT public.lab_arena_sep17_recovery278_archive_valid_v1() IS TRUE
    AND public.lab_arena_sep17_recovery278_nonbaseline_ledger_valid_v1() IS TRUE
    AND EXISTS (
      SELECT 1 FROM public.lab_arena_sep17_baseline_recovery278_authority AS prior
      WHERE round_id = 'arena-2026-09-17'
        AND 'sha256:' || pg_catalog.encode(extensions.digest(
          pg_catalog.to_jsonb(prior)::TEXT, 'sha256'), 'hex') =
            'sha256:e83520d50f1b821bddebb85064f1d814307a9b481046e35165f9a93bb41f0026'
    )
    AND EXISTS (
      SELECT 1 FROM public.lab_arena_sep17_baseline_recovery278_audit AS prior
      WHERE round_id = 'arena-2026-09-17'
        AND 'sha256:' || pg_catalog.encode(extensions.digest(
          pg_catalog.to_jsonb(prior)::TEXT, 'sha256'), 'hex') =
            'sha256:082a5617de604c9cdf24f3f6ff60e686a8c97ee953401e3f21aa5e3faf8faff7'
    );
$sep17_recovery282_prior_valid$;
ALTER FUNCTION public.lab_arena_sep17_recovery282_prior_valid_v1()
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep17_recovery282_prior_valid_v1()
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep17_recovery282_terminal_valid_v1()
RETURNS BOOLEAN
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $sep17_recovery282_terminal_valid$
DECLARE
  v_authority public.lab_arena_sep17_baseline_recovery282_authority%ROWTYPE;
...
BEGIN
  ...
  FROM public.lab_arena_sep17_baseline_recovery282_authority
  WHERE round_id = 'arena-2026-09-17';
  ...
  RETURN public.lab_arena_sep17_recovery282_prior_valid_v1() IS TRUE
     ...
     AND (SELECT pg_catalog.count(DISTINCT assignment_id)
          ...
          WHERE round_id = v_round.round_id AND submission_id = v_baseline.submission_id
            AND kind = 'execute' AND assignment_id LIKE '%:rerun278') = 20
     AND NOT EXISTS (
...
END;
$sep17_recovery282_terminal_valid$;
ALTER FUNCTION public.lab_arena_sep17_recovery282_terminal_valid_v1()
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep17_recovery282_terminal_valid_v1()
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;
...
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $sep17_recovery282_nonbaseline_ledger_valid$
DECLARE
  v_authority public.lab_arena_sep17_baseline_recovery282_authority%ROWTYPE;
  v_hash TEXT;
  v_count BIGINT;
...
END;
$sep17_recovery282_nonbaseline_ledger_valid$;
ALTER FUNCTION
public.lab_arena_sep17_recovery282_nonbaseline_ledger_valid_v1()
  OWNER TO lab_arena_owner;
...
CREATE OR REPLACE FUNCTION public.lab_arena_sep17_recovery282_archive_valid_v1()
RETURNS BOOLEAN
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $sep17_recovery282_archive_valid$
DECLARE
  v_audit public.lab_arena_sep17_baseline_recovery282_audit%ROWTYPE;
...
BEGIN
  SELECT * INTO v_audit
  FROM public.lab_arena_sep17_baseline_recovery282_audit
  WHERE round_id = 'arena-2026-09-17';
  ...
  RETURN public.lab_arena_sep17_recovery282_prior_valid_v1() IS TRUE
     ...
     AND v_archive_round.cancel_reason =
          'authorized_sep17_recovery278_baseline_archive'
...
END;
$sep17_recovery282_archive_valid$;
ALTER FUNCTION public.lab_arena_sep17_recovery282_archive_valid_v1()
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep17_recovery282_archive_valid_v1()
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;

DO $verify_sep17_recovery282_terminal$
BEGIN
  IF EXISTS (
       SELECT 1 FROM public.lab_arena_sep17_baseline_recovery282_audit
       WHERE round_id = 'arena-2026-09-17'
     ) THEN
    IF public.lab_arena_sep17_recovery282_archive_valid_v1() IS NOT TRUE
       OR public.lab_arena_sep17_recovery282_nonbaseline_ledger_valid_v1()
            IS NOT TRUE THEN
      RAISE EXCEPTION 'Sep17 recovery282 archive seal differs'
        USING ERRCODE = '55000';
    END IF;
  ELSIF public.lab_arena_sep17_recovery282_terminal_valid_v1() IS NOT TRUE THEN
    RAISE EXCEPTION 'Sep17 cancelled baseline terminal seal differs'
      USING ERRCODE = '55000';
  END IF;
END;
$verify_sep17_recovery282_terminal$;

CREATE OR REPLACE FUNCTION public.lab_arena_prepare_sep17_baseline_recovery282_v1(
...
SET search_path = pg_catalog, public, extensions
AS $prepare_sep17_baseline_recovery282$
DECLARE
...
BEGIN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('arena-2026-09-17-baseline-recovery282', 0)
  );
  ...
  IF p_source_size_bytes IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'Sep17 recovery282 source, bank, or schedule differs'
      USING ERRCODE = '22023';
  END IF;
  LOCK TABLE public.lab_arena_sep17_baseline_recovery282_audit
  ...
  IF FOUND THEN
    ...
    IF public.lab_arena_sep17_recovery282_archive_valid_v1() IS NOT TRUE
       ...
       OR (SELECT pg_catalog.count(DISTINCT assignment_id)
           ...
           WHERE round_id = v_round_id AND submission_id = v_baseline_id
             AND kind = 'execute'
             AND assignment_id LIKE '%:rerun282') <> 20
       OR EXISTS (
         SELECT 1 FROM public.lab_arena_runs
         WHERE round_id = v_round_id AND submission_id = v_baseline_id
       ...
       OR v_nonbaseline_submissions_hash IS DISTINCT FROM
            v_authority.terminal_nonbaseline_submissions_hash
       THEN
      RAISE EXCEPTION 'Sep17 recovery282 replay differs' USING ERRCODE = '55000';
    END IF;
  ...
  IF pg_catalog.jsonb_typeof(p_forward_schedule) IS DISTINCT FROM 'object'
    ...
    RAISE EXCEPTION 'Sep17 recovery282 forward schedule invalid'
      USING ERRCODE = '22023';
  END IF;
  IF public.lab_arena_sep17_recovery282_terminal_valid_v1() IS NOT TRUE THEN
    RAISE EXCEPTION 'Sep17 cancelled baseline terminal seal differs'
      USING ERRCODE = '55000';
  END IF;
  ...
  INSERT INTO public.lab_arena_sep17_baseline_recovery282_audit (
    round_id, archive_round_id, archive_submission_id, execute_namespace,
  ...
  IF (v_new_configuration - 'schedule') IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'Sep17 recovery282 changed frozen configuration';
  END IF;
  SELECT pg_catalog.jsonb_agg(
    CASE WHEN item ->> 'submission_id' = v_baseline_id THEN
      item || pg_catalog.jsonb_build_object(
  ...
  FROM pg_catalog.jsonb_populate_record(
    ...
    pg_catalog.to_jsonb(v_round) || pg_catalog.jsonb_build_object(
      ...
      'rewards_enabled', FALSE,
      'cancel_reason', 'authorized_sep17_recovery278_baseline_archive'
    )
  ) AS row_value;
  ...
  IF v_count <> v_authority.terminal_baseline_run_count THEN
    RAISE EXCEPTION 'Sep17 recovery282 run archive count differs';
  END IF;
  ...
  IF v_count <> v_authority.terminal_baseline_ledger_count THEN
    RAISE EXCEPTION 'Sep17 recovery282 ledger archive count differs';
  END IF;
  ...
  WHERE round_id = v_round_id AND submission_id = v_baseline_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Sep17 recovery282 baseline update differs';
  END IF;
  ...
  FOR v_position IN 0 .. 19 LOOP
    ...
    v_assignment := v_round_id || ':' || v_baseline_id || ':' ||
      v_stage::TEXT || ':' || v_position::TEXT || ':rerun282';
  ...
  IF public.lab_arena_sep17_recovery282_archive_valid_v1() IS NOT TRUE
    ...
    RAISE EXCEPTION 'Sep17 recovery282 postcondition differs';
  END IF;
  RETURN pg_catalog.jsonb_build_object(
...
END;
$prepare_sep17_baseline_recovery282$;
ALTER FUNCTION public.lab_arena_prepare_sep17_baseline_recovery282_v1(
  BIGINT, TEXT, TEXT, TEXT, JSONB
)
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_prepare_sep17_baseline_recovery282_v1(
  BIGINT, TEXT, TEXT, TEXT, JSONB
)
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;
GRANT EXECUTE ON FUNCTION public.lab_arena_prepare_sep17_baseline_recovery282_v1(
  BIGINT, TEXT, TEXT, TEXT, JSONB
)
  TO lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep17_recovery282_archive_guard_v1()
RETURNS TRIGGER
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $sep17_recovery282_archive_guard$
BEGIN
  IF NEW.round_id = 'arena-2026-09-17'
     ...
     AND public.lab_arena_sep17_recovery282_archive_valid_v1() IS NOT TRUE THEN
    RAISE EXCEPTION 'Sep17 recovery282 archive seal differs'
      USING ERRCODE = '55000';
  END IF;
  RETURN NEW;
END;
$sep17_recovery282_archive_guard$;
ALTER FUNCTION public.lab_arena_sep17_recovery282_archive_guard_v1()
  OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep17_recovery282_archive_guard_v1()
  FROM PUBLIC, anon, authenticated, service_role, lab_arena_service;
DROP TRIGGER IF EXISTS lab_arena_sep17_recovery282_archive_guard
  ON public.lab_arena_rounds;
CREATE TRIGGER lab_arena_sep17_recovery282_archive_guard
BEFORE UPDATE ON public.lab_arena_rounds
FOR EACH ROW EXECUTE FUNCTION
  public.lab_arena_sep17_recovery282_archive_guard_v1();
