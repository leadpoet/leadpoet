-- Render only after the Sep19 rerun316 round is terminal and every seal below
...
CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun317_archive_valid_v1()
RETURNS BOOLEAN LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,extensions AS $archive317$
...
BEGIN
 ...
 RETURN archived.status='cancelled'
  ...
  AND archived.cancel_reason='authorized_sep19_terminal316_fixed_baseline_archive'
  ...
  AND archived.configuration_doc->>'archived_terminal_round_sha256'='edd40e5004b4b654febc4733f94e62b5e82d9b7f69720035f3ce20a7c6fcad2e'
  AND archived.configuration_doc->>'archived_terminal_baseline_sha256'='adee534ed61bb1ea0761142d66d22d2edef6de2b89cd73d1b03a5f5a1e675f9f'
  ...
  AND archived.configuration_doc->>'archived_terminal_runs_sha256'='a40087077d409c804d044bdbfaa04b93bbc72f6dc93ccf61c76dca85ffc478f1'
  AND archived.configuration_doc->>'archived_terminal_ledger_sha256'='3bc4b1624c68abc3c10cfbea4ddb296969e3c04d6ade07a6b96976f0660c4129'
  ...
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_submissions
       WHERE round_id='arena-2026-09-19-r317archive')=5
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r317archive')=30
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r317archive'
        AND status='accepted')=2
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r317archive'
        AND status='failed')=28
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r317archive'
  ...
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r317archive'
        AND assignment_id NOT LIKE '%:rerun316')
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='score')=0
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='score'
        AND submission_id NOT LIKE '%:r317archive')
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r317archive' AND kind='score'
        AND assignment_id NOT LIKE '%:score:rerun316')
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger
       WHERE round_id='arena-2026-09-19-r317archive'
        AND submission_id='baseline-2026-09-19:r317archive')=
      6579
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger ledger
       WHERE ledger.round_id='arena-2026-09-19-r317archive'
        AND ledger.submission_id='baseline-2026-09-19:r317archive'
        AND EXISTS(SELECT 1 FROM public.lab_arena_runs run
          WHERE run.round_id='arena-2026-09-19-r317archive'
...
END $archive317$;
ALTER FUNCTION public.lab_arena_sep19_rerun317_archive_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun317_archive_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun317_active_valid_v1()
RETURNS BOOLEAN LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public AS $active317$
...
BEGIN
 ...
 RETURN active.round_id IS NOT NULL AND archived.round_id IS NOT NULL
  AND public.lab_arena_sep19_rerun317_archive_valid_v1() IS TRUE
  AND pg_catalog.encode(extensions.digest(pg_catalog.jsonb_build_object(
  ...
  AND EXISTS(SELECT 1 FROM public.lab_arena_submissions baseline
    WHERE baseline.round_id='arena-2026-09-19'
     ...
     AND baseline.source_ref='arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun317-6d6cd059.tar.gz'
     AND baseline.source_size_bytes=793558
     AND baseline.submission_doc->>'source_sha256'='5da106d1f3b5078460aa43297ead185f1a1257b75c6c30a1effd3b6464cdf5ff'
     AND baseline.submission_doc->>'source_commit'='6d6cd05979153e09d7d00d3b2bf04990fb023d3b')
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
...
END $active317$;
ALTER FUNCTION public.lab_arena_sep19_rerun317_active_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun317_active_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

-- Replace the exact rerun316 guard and namespace in the current scorer. A
-- second stacked guard would reject rerun317 after the archive is created.
DO $patch_sep19_rerun317_scorer$
DECLARE definition TEXT; current_hash TEXT;
 old_assignment TEXT:=$assignment316$    IF p_round_id='arena-2026-09-19'
       AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
         WHERE round_id='arena-2026-09-19-r316archive') THEN
      v_assignment:=p_round_id||':'||v_scored.submission_id||':'||p_stage::TEXT||':'||
        v_scored.icp_position::TEXT||':score:rerun316';
    END IF;
    v_status := CASE WHEN v_cache.cache_key IS NULL THEN 'pending' ELSE 'accepted' END;$assignment316$;
 old_guard TEXT:=$guard316$  IF p_round_id='arena-2026-09-19'
     AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
       WHERE round_id='arena-2026-09-19-r316archive')
     AND public.lab_arena_sep19_rerun316_active_valid_v1() IS NOT TRUE THEN
    RAISE EXCEPTION 'lab_arena_sep19_rerun316_frozen_state_invalid'
      USING ERRCODE='22023';
  END IF;
  FOR v_item IN$guard316$;
BEGIN
 ...
 IF current_hash='1bf82d9e27e385424ad96d0552573d63ecc84e55ea04ec81bcd8d74e07c07991' THEN
  IF (pg_catalog.length(definition)-pg_catalog.length(pg_catalog.replace(
   ...
   RAISE EXCEPTION 'Sep19 rerun317 scorer seam differs';
  END IF;
  ...
  EXECUTE definition;
 ELSIF current_hash<>'937c57a010c01a3336165f052a65295fe4fef0fab887ce3babba1c82480d95d7' THEN
  RAISE EXCEPTION 'Sep19 rerun317 scorer definition differs';
 END IF;
 IF pg_catalog.encode(extensions.digest(pg_catalog.pg_get_functiondef(
   'public.lab_arena_open_scoring_v2(text,smallint,jsonb)'::pg_catalog.regprocedure),
   'sha256'),'hex')<>'937c57a010c01a3336165f052a65295fe4fef0fab887ce3babba1c82480d95d7' THEN
  RAISE EXCEPTION 'Sep19 rerun317 scorer patch differs';
 END IF;
END $patch_sep19_rerun317_scorer$;

CREATE OR REPLACE FUNCTION public.lab_arena_prepare_sep19_rerun317_v1(
...
SET search_path=pg_catalog,public,extensions AS $prepare317$
DECLARE
...
BEGIN
 ...
 IF p_source_size_bytes<>793558
  OR p_source_sha256<>'5da106d1f3b5078460aa43297ead185f1a1257b75c6c30a1effd3b6464cdf5ff'
  OR p_source_commit<>'6d6cd05979153e09d7d00d3b2bf04990fb023d3b'
  OR p_schedule IS DISTINCT FROM '{"benchmark_deadline":"2026-09-19T14:00:00Z","final_scoring_close":"2026-09-20T02:00:00Z","publication_deadline":"2026-09-20T02:00:01Z","stage_1_close":"2026-09-19T17:00:00Z","stage_1_scoring_close":"2026-09-19T20:00:00Z","stage_1_start":"2026-09-19T14:00:01Z","stage_2_close":"2026-09-19T23:00:00Z","stage_2_start":"2026-09-19T20:00:01Z","submission_cutoff":"2026-09-19T00:00:00Z","submission_open":"2026-09-18T00:00:00Z"}'::JSONB
  ...
  RAISE EXCEPTION 'Sep19 rerun317 source, judge, bank, or schedule differs'
   USING ERRCODE='22023';
 END IF;
 PERFORM pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('arena-2026-09-19-rerun317',0));
 ...
 IF EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r317archive') THEN
  IF public.lab_arena_sep19_rerun317_archive_valid_v1() IS NOT TRUE
   OR public.lab_arena_sep19_rerun317_active_valid_v1() IS NOT TRUE THEN
   RAISE EXCEPTION 'existing Sep19 rerun317 differs' USING ERRCODE='55000';
  END IF;
  RETURN pg_catalog.jsonb_build_object('status','existing',
   'round_id','arena-2026-09-19','fresh_baseline_execute_count',20,
   'retained_miner_execute_count',80,'score_namespace','score:rerun317');
 END IF;
 IF (p_schedule->>'benchmark_deadline')::TIMESTAMPTZ<=pg_catalog.clock_timestamp() THEN
  RAISE EXCEPTION 'Sep19 rerun317 admission window closed' USING ERRCODE='22023';
 END IF;
 ...
 IF active_round.status NOT IN('published','cancelled')
  ...
  OR public.lab_arena_sep19_rerun316_active_valid_v1() IS NOT TRUE
  ...
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_round)::TEXT,
       'sha256'),'hex')<>'edd40e5004b4b654febc4733f94e62b5e82d9b7f69720035f3ce20a7c6fcad2e'
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_baseline)::TEXT,
       'sha256'),'hex')<>'adee534ed61bb1ea0761142d66d22d2edef6de2b89cd73d1b03a5f5a1e675f9f'
  ...
  OR (SELECT pg_catalog.count(DISTINCT assignment_id) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19'
       AND assignment_id LIKE '%:rerun316')<>20
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19'
       AND assignment_id NOT LIKE '%:rerun316')
  ...
  RAISE EXCEPTION 'Sep19 rerun317 terminal preimage differs' USING ERRCODE='55000';
 END IF;
 IF pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'a40087077d409c804d044bdbfaa04b93bbc72f6dc93ccf61c76dca85ffc478f1'
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'3bc4b1624c68abc3c10cfbea4ddb296969e3c04d6ade07a6b96976f0660c4129' THEN
  RAISE EXCEPTION 'Sep19 rerun317 terminal history differs' USING ERRCODE='55000';
 END IF;
 ...
 IF pg_catalog.jsonb_array_length(derived)<>110 THEN
  RAISE EXCEPTION 'Sep19 rerun317 derived judgment count differs'; END IF;
 expected_submission:=active_baseline.submission_doc||pg_catalog.jsonb_build_object(
  'source_ref','arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun317-6d6cd059.tar.gz','source_size_bytes',793558,
  'source_sha256','5da106d1f3b5078460aa43297ead185f1a1257b75c6c30a1effd3b6464cdf5ff','source_commit','6d6cd05979153e09d7d00d3b2bf04990fb023d3b');
 SELECT pg_catalog.jsonb_agg(CASE WHEN item->>'submission_id'='baseline-2026-09-19'
   THEN item||pg_catalog.jsonb_build_object('source_ref','arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun317-6d6cd059.tar.gz',
    'source_size_bytes',793558) ELSE item END ORDER BY ordinal)
 ...
 archive_participants:=(SELECT pg_catalog.jsonb_agg(
   item||pg_catalog.jsonb_build_object('submission_id',(item->>'submission_id')||':r317archive')
 ...
 archive_config:=active_round.configuration_doc||pg_catalog.jsonb_build_object(
  'round_id','arena-2026-09-19-r317archive','mode','shadow','rewards_enabled',FALSE,
  ...
  'archived_terminal_round_sha256','edd40e5004b4b654febc4733f94e62b5e82d9b7f69720035f3ce20a7c6fcad2e',
  'archived_terminal_baseline_sha256','adee534ed61bb1ea0761142d66d22d2edef6de2b89cd73d1b03a5f5a1e675f9f',
  'archived_terminal_miner_submissions_sha256','781027021d3cadc5f2c0540d9f24a67f65cb25126912ef3544867d5a70153297',
  'archived_terminal_runs_sha256','a40087077d409c804d044bdbfaa04b93bbc72f6dc93ccf61c76dca85ffc478f1',
  'archived_terminal_ledger_sha256','3bc4b1624c68abc3c10cfbea4ddb296969e3c04d6ade07a6b96976f0660c4129',
 ...
 SELECT pg_catalog.jsonb_build_object(
  'rounds',(SELECT pg_catalog.jsonb_agg(pg_catalog.to_jsonb(r) ORDER BY round_id)
    FROM public.lab_arena_rounds r WHERE round_id NOT IN(
     'arena-2026-09-19','arena-2026-09-19-r317archive')),
 ...
 INSERT INTO public.lab_arena_rounds(
  ...
  NULL::public.lab_arena_rounds,pg_catalog.to_jsonb(active_round)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r317archive','status','cancelled',
   ...
   'cancel_reason','authorized_sep19_terminal316_fixed_baseline_archive'));
 INSERT INTO public.lab_arena_submissions SELECT (pg_catalog.jsonb_populate_record(
  NULL::public.lab_arena_submissions,pg_catalog.to_jsonb(s)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r317archive',
   'submission_id',s.submission_id||':r317archive'))).*
  FROM public.lab_arena_submissions s WHERE s.round_id='arena-2026-09-19';
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r317archive',
  submission_id=submission_id||':r317archive'
  WHERE round_id='arena-2026-09-19' AND kind='score';
 GET DIAGNOSTICS moved_scores=ROW_COUNT;
 UPDATE public.lab_arena_ledger l SET round_id='arena-2026-09-19-r317archive',
  submission_id=submission_id||':r317archive'
  WHERE round_id='arena-2026-09-19' AND EXISTS(SELECT 1 FROM public.lab_arena_runs s
   WHERE s.round_id='arena-2026-09-19-r317archive' AND s.kind='score' AND s.run_id=l.run_id);
 GET DIAGNOSTICS moved_score_ledger=ROW_COUNT;
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r317archive',
  submission_id='baseline-2026-09-19:r317archive'
  WHERE round_id='arena-2026-09-19' AND kind='execute'
   AND submission_id='baseline-2026-09-19';
 GET DIAGNOSTICS moved_baseline_runs=ROW_COUNT;
 UPDATE public.lab_arena_ledger SET round_id='arena-2026-09-19-r317archive',
  submission_id='baseline-2026-09-19:r317archive'
  WHERE round_id='arena-2026-09-19' AND submission_id='baseline-2026-09-19';
 ...
 UPDATE public.lab_arena_runs SET per_icp_score=NULL,qualification_doc=NULL
  WHERE round_id='arena-2026-09-19' AND kind='execute'
   AND submission_id<>'baseline-2026-09-19';
 UPDATE public.lab_arena_submissions SET source_ref='arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun317-6d6cd059.tar.gz',
  source_size_bytes=793558,submission_doc=expected_submission,
  updated_at=pg_catalog.clock_timestamp()
  WHERE round_id='arena-2026-09-19' AND submission_id='baseline-2026-09-19';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep19 rerun317 baseline update missing'; END IF;
 UPDATE public.lab_arena_rounds SET status='stage1',
  ...
  updated_at=pg_catalog.clock_timestamp()
  WHERE round_id='arena-2026-09-19';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep19 rerun317 round update missing'; END IF;
 inserted_runs:=0;
 ...
 IF moved_scores<>0
  ...
  OR public.lab_arena_sep19_rerun317_archive_valid_v1() IS NOT TRUE
  OR public.lab_arena_sep19_rerun317_active_valid_v1() IS NOT TRUE
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
    WHERE round_id='arena-2026-09-19' AND kind='score') THEN
  RAISE EXCEPTION 'Sep19 rerun317 atomic preservation differs' USING ERRCODE='55000';
 END IF;
 RETURN pg_catalog.jsonb_build_object('status','prepared',
  ...
  'score_namespace','score:rerun317');
END $prepare317$;
ALTER FUNCTION public.lab_arena_prepare_sep19_rerun317_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_prepare_sep19_rerun317_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT)
 FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.lab_arena_prepare_sep19_rerun317_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) TO lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun317_score_namespace_guard_v1()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public AS $score_guard317$
BEGIN
 IF NEW.round_id='arena-2026-09-19' AND NEW.kind='score'
  AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r317archive')
  AND NEW.assignment_id IS DISTINCT FROM NEW.round_id||':'||NEW.submission_id||':'||
   NEW.stage::TEXT||':'||NEW.icp_position::TEXT||':score:rerun317' THEN
  RAISE EXCEPTION 'Sep19 rerun317 scoring requires exact namespace'
   USING ERRCODE='55000';
 END IF;
 RETURN NEW;
END $score_guard317$;
ALTER FUNCTION public.lab_arena_sep19_rerun317_score_namespace_guard_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun317_score_namespace_guard_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;
DROP TRIGGER IF EXISTS lab_arena_sep19_rerun316_score_namespace_guard
 ON public.lab_arena_runs;
DROP TRIGGER IF EXISTS lab_arena_sep19_rerun317_score_namespace_guard
 ON public.lab_arena_runs;
CREATE TRIGGER lab_arena_sep19_rerun317_score_namespace_guard
 BEFORE INSERT ON public.lab_arena_runs FOR EACH ROW
 EXECUTE FUNCTION public.lab_arena_sep19_rerun317_score_namespace_guard_v1();

SELECT public.lab_arena_prepare_sep19_rerun317_v1(
 793558,'5da106d1f3b5078460aa43297ead185f1a1257b75c6c30a1effd3b6464cdf5ff','6d6cd05979153e09d7d00d3b2bf04990fb023d3b',
 '{"benchmark_deadline":"2026-09-19T14:00:00Z","final_scoring_close":"2026-09-20T02:00:00Z","publication_deadline":"2026-09-20T02:00:01Z","stage_1_close":"2026-09-19T17:00:00Z","stage_1_scoring_close":"2026-09-19T20:00:00Z","stage_1_start":"2026-09-19T14:00:01Z","stage_2_close":"2026-09-19T23:00:00Z","stage_2_start":"2026-09-19T20:00:01Z","submission_cutoff":"2026-09-19T00:00:00Z","submission_open":"2026-09-18T00:00:00Z"}'::JSONB,
