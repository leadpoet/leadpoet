-- Render only after the Sep19 rerun318 round is terminal and every seal below
...
CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun320_archive_valid_v1()
RETURNS BOOLEAN LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,extensions AS $archive320$
...
BEGIN
 ...
 RETURN archived.status='cancelled'
  ...
  AND archived.cancel_reason='authorized_sep19_terminal318_fixed_baseline_archive'
  ...
  AND archived.configuration_doc->>'archived_terminal_round_sha256'='7fc79ca58856200455e7109630a50ec5207e54374711688d55909c426573abf0'
  AND archived.configuration_doc->>'archived_terminal_baseline_sha256'='320e15b2eb26cf571e80abad40fe65c7433184fd41211f11cf09be2ab79064f4'
  ...
  AND archived.configuration_doc->>'archived_terminal_runs_sha256'='5f97b2a744e74540b6e79beb82a9b7bd8ee82e84ad27a2a310ee885771fb74c7'
  AND archived.configuration_doc->>'archived_terminal_ledger_sha256'='66d90b41a988043762c491598401f627ef9467bf2051b9308698b2314f7f7c96'
  ...
  AND pg_catalog.jsonb_array_length(derived)=116
  AND (SELECT pg_catalog.count(DISTINCT value->>'run_id')
       FROM pg_catalog.jsonb_array_elements(derived))=116
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_submissions
       WHERE round_id='arena-2026-09-19-r320archive')=5
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r320archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r320archive')=36
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r320archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r320archive'
        AND status='accepted')=6
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r320archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r320archive'
        AND status='failed')=30
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r320archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r320archive'
  ...
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r320archive' AND kind='score')=0
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r320archive' AND kind='score'
        AND submission_id NOT LIKE '%:r320archive')
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r320archive' AND kind='score'
  ...
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger ledger
       WHERE ledger.round_id='arena-2026-09-19-r320archive'
        AND ledger.submission_id='baseline-2026-09-19:r320archive'
        AND EXISTS(SELECT 1 FROM public.lab_arena_runs run
          WHERE run.round_id='arena-2026-09-19-r320archive'
...
END $archive320$;
ALTER FUNCTION public.lab_arena_sep19_rerun320_archive_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun320_archive_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun320_active_valid_v1()
RETURNS BOOLEAN LANGUAGE plpgsql STABLE SECURITY DEFINER
...
BEGIN
 ...
 RETURN active.round_id IS NOT NULL AND archived.round_id IS NOT NULL
  AND public.lab_arena_sep19_rerun320_archive_valid_v1() IS TRUE
  AND pg_catalog.encode(extensions.digest(pg_catalog.jsonb_build_object(
...
END $active318$;
ALTER FUNCTION public.lab_arena_sep19_rerun320_active_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun320_active_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

-- Replace the exact rerun318 guard and namespace in the current scorer. A
-- second stacked guard would reject rerun320 after the archive is created.
DO $patch_sep19_rerun320_scorer$
DECLARE definition TEXT; current_hash TEXT;
 old_assignment TEXT:=$assignment318$    IF p_round_id='arena-2026-09-19'
    ...
    END IF;
    v_status := CASE WHEN v_cache.cache_key IS NULL THEN 'pending' ELSE 'accepted' END;$assignment318$;
 old_guard TEXT:=$guard318$  IF p_round_id='arena-2026-09-19'
     AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
...
BEGIN
 ...
 IF current_hash='6d184e5cc8a401113c38f4ddfabb948eae698057623d7fa4a5641c84603b546c' THEN
  IF (pg_catalog.length(definition)-pg_catalog.length(pg_catalog.replace(
   ...
   RAISE EXCEPTION 'Sep19 rerun320 scorer seam differs';
  END IF;
  ...
  EXECUTE definition;
 ELSIF current_hash<>'38756e97556806ec2b708dcf7ed380003f26261b9d4c84a8a2dd31f9596cdc81' THEN
  RAISE EXCEPTION 'Sep19 rerun320 scorer definition differs';
 END IF;
 IF pg_catalog.encode(extensions.digest(pg_catalog.pg_get_functiondef(
   'public.lab_arena_open_scoring_v2(text,smallint,jsonb)'::pg_catalog.regprocedure),
   'sha256'),'hex')<>'38756e97556806ec2b708dcf7ed380003f26261b9d4c84a8a2dd31f9596cdc81' THEN
  RAISE EXCEPTION 'Sep19 rerun320 scorer patch differs';
 END IF;
END $patch_sep19_rerun320_scorer$;

CREATE OR REPLACE FUNCTION public.lab_arena_prepare_sep19_rerun320_v1(
...
SET search_path=pg_catalog,public,extensions AS $prepare320$
DECLARE
...
BEGIN
 ...
 PERFORM pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('arena-2026-09-19-rerun320',0));
 ...
 IF EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r320archive') THEN
  IF public.lab_arena_sep19_rerun320_archive_valid_v1() IS NOT TRUE
   OR public.lab_arena_sep19_rerun320_active_valid_v1() IS NOT TRUE THEN
   RAISE EXCEPTION 'existing Sep19 rerun320 differs' USING ERRCODE='55000';
  END IF;
  RETURN pg_catalog.jsonb_build_object('status','existing',
   'round_id','arena-2026-09-19','fresh_baseline_execute_count',20,
   'retained_miner_execute_count',80,'score_namespace','score:rerun320');
 END IF;
 IF (p_schedule->>'benchmark_deadline')::TIMESTAMPTZ<=pg_catalog.clock_timestamp() THEN
  RAISE EXCEPTION 'Sep19 rerun320 admission window closed' USING ERRCODE='22023';
 END IF;
 ...
 IF active_round.status NOT IN('published','cancelled')
  ...
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_round)::TEXT,
       'sha256'),'hex')<>'7fc79ca58856200455e7109630a50ec5207e54374711688d55909c426573abf0'
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_baseline)::TEXT,
       'sha256'),'hex')<>'320e15b2eb26cf571e80abad40fe65c7433184fd41211f11cf09be2ab79064f4'
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      ...
      <>86
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute')<>116
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19')<>36
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19' AND status='accepted')
      <>6
  ...
  RAISE EXCEPTION 'Sep19 rerun320 terminal preimage differs' USING ERRCODE='55000';
 END IF;
 IF pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'5f97b2a744e74540b6e79beb82a9b7bd8ee82e84ad27a2a310ee885771fb74c7'
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'66d90b41a988043762c491598401f627ef9467bf2051b9308698b2314f7f7c96' THEN
  RAISE EXCEPTION 'Sep19 rerun320 terminal history differs' USING ERRCODE='55000';
 END IF;
 ...
 archive_participants:=(SELECT pg_catalog.jsonb_agg(
   item||pg_catalog.jsonb_build_object('submission_id',(item->>'submission_id')||':r320archive')
 ...
 archive_config:=active_round.configuration_doc||pg_catalog.jsonb_build_object(
  'round_id','arena-2026-09-19-r320archive','mode','shadow','rewards_enabled',FALSE,
  ...
  'archived_terminal_round_sha256','7fc79ca58856200455e7109630a50ec5207e54374711688d55909c426573abf0',
  'archived_terminal_baseline_sha256','320e15b2eb26cf571e80abad40fe65c7433184fd41211f11cf09be2ab79064f4',
  'archived_terminal_miner_submissions_sha256','781027021d3cadc5f2c0540d9f24a67f65cb25126912ef3544867d5a70153297',
  'archived_terminal_runs_sha256','5f97b2a744e74540b6e79beb82a9b7bd8ee82e84ad27a2a310ee885771fb74c7',
  'archived_terminal_ledger_sha256','66d90b41a988043762c491598401f627ef9467bf2051b9308698b2314f7f7c96',
 ...
 SELECT pg_catalog.jsonb_build_object(
  'rounds',(SELECT pg_catalog.jsonb_agg(pg_catalog.to_jsonb(r) ORDER BY round_id)
    FROM public.lab_arena_rounds r WHERE round_id NOT IN(
     'arena-2026-09-19','arena-2026-09-19-r320archive')),
 ...
 INSERT INTO public.lab_arena_rounds(
  ...
  NULL::public.lab_arena_rounds,pg_catalog.to_jsonb(active_round)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r320archive','status','cancelled',
   ...
   'cancel_reason','authorized_sep19_terminal318_fixed_baseline_archive'));
 INSERT INTO public.lab_arena_submissions SELECT (pg_catalog.jsonb_populate_record(
  NULL::public.lab_arena_submissions,pg_catalog.to_jsonb(s)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r320archive',
   'submission_id',s.submission_id||':r320archive'))).*
  FROM public.lab_arena_submissions s WHERE s.round_id='arena-2026-09-19';
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r320archive',
  submission_id=submission_id||':r320archive'
  WHERE round_id='arena-2026-09-19' AND kind='score';
 GET DIAGNOSTICS moved_scores=ROW_COUNT;
 UPDATE public.lab_arena_ledger l SET round_id='arena-2026-09-19-r320archive',
  submission_id=submission_id||':r320archive'
  WHERE round_id='arena-2026-09-19' AND EXISTS(SELECT 1 FROM public.lab_arena_runs s
   WHERE s.round_id='arena-2026-09-19-r320archive' AND s.kind='score' AND s.run_id=l.run_id);
 GET DIAGNOSTICS moved_score_ledger=ROW_COUNT;
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r320archive',
  submission_id='baseline-2026-09-19:r320archive'
  WHERE round_id='arena-2026-09-19' AND kind='execute'
   AND submission_id='baseline-2026-09-19';
 GET DIAGNOSTICS moved_baseline_runs=ROW_COUNT;
 UPDATE public.lab_arena_ledger SET round_id='arena-2026-09-19-r320archive',
  submission_id='baseline-2026-09-19:r320archive'
  WHERE round_id='arena-2026-09-19' AND submission_id='baseline-2026-09-19';
 ...
 UPDATE public.lab_arena_rounds SET status='stage1',
  ...
  updated_at=pg_catalog.clock_timestamp()
  WHERE round_id='arena-2026-09-19';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep19 rerun320 round update missing'; END IF;
 inserted_runs:=0;
 ...
 IF moved_scores<>0
  ...
  OR moved_baseline_runs<>36
  OR moved_baseline_ledger<>16698
  ...
  OR public.lab_arena_sep19_rerun320_archive_valid_v1() IS NOT TRUE
  OR public.lab_arena_sep19_rerun320_active_valid_v1() IS NOT TRUE
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
    WHERE round_id='arena-2026-09-19' AND kind='score') THEN
  RAISE EXCEPTION 'Sep19 rerun320 atomic preservation differs' USING ERRCODE='55000';
 END IF;
 RETURN pg_catalog.jsonb_build_object('status','prepared',
  ...
  'score_namespace','score:rerun320');
END $prepare320$;
ALTER FUNCTION public.lab_arena_prepare_sep19_rerun320_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_prepare_sep19_rerun320_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT)
 FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.lab_arena_prepare_sep19_rerun320_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) TO lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun320_score_namespace_guard_v1()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public AS $score_guard320$
BEGIN
 IF NEW.round_id='arena-2026-09-19' AND NEW.kind='score'
  AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r320archive')
  AND NEW.assignment_id IS DISTINCT FROM NEW.round_id||':'||NEW.submission_id||':'||
   NEW.stage::TEXT||':'||NEW.icp_position::TEXT||':score:rerun320' THEN
  RAISE EXCEPTION 'Sep19 rerun320 scoring requires exact namespace'
   USING ERRCODE='55000';
 END IF;
 RETURN NEW;
END $score_guard320$;
ALTER FUNCTION public.lab_arena_sep19_rerun320_score_namespace_guard_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun320_score_namespace_guard_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;
...
DROP TRIGGER IF EXISTS lab_arena_sep19_rerun320_score_namespace_guard
 ON public.lab_arena_runs;
CREATE TRIGGER lab_arena_sep19_rerun320_score_namespace_guard
