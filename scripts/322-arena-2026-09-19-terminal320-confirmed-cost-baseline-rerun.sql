-- Render only after the Sep19 rerun320 round is terminal and every seal below
...
CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun322_archive_valid_v1()
RETURNS BOOLEAN LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,extensions AS $archive322$
DECLARE archived public.lab_arena_rounds; derived JSONB;
BEGIN
 SELECT * INTO archived FROM public.lab_arena_rounds
  WHERE round_id='arena-2026-09-19-r322archive';
 ...
 RETURN archived.status='cancelled'
  ...
  AND archived.cancel_reason='authorized_sep19_terminal320_confirmed_cost_baseline_archive'
  ...
  AND archived.configuration_doc->>'archived_terminal_round_sha256'='526598f87146dd524cf4c368d74b059dbc26071c1b9549c2dbc0dac6c765db24'
  AND archived.configuration_doc->>'archived_terminal_baseline_sha256'='68cf40d0506ffa77d55fd39af0ddbaa5bef52ea354f4356bdac004ee3de9027c'
  ...
  AND archived.configuration_doc->>'archived_terminal_runs_sha256'='a41083e4a0698dc643073f35819841484ee4fab2bac3c996b18e6a07aa4196a8'
  AND archived.configuration_doc->>'archived_terminal_ledger_sha256'='1ed41423f4dda7201a9807c869ef2193c49fcb75eb739d7454aa27dce9cda886'
  ...
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_submissions
       WHERE round_id='arena-2026-09-19-r322archive')=5
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r322archive')=30
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r322archive'
        AND status='accepted')=1
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r322archive'
        AND status='failed')=29
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r322archive'
  ...
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r322archive'
        AND assignment_id NOT LIKE '%:rerun320')
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='score')=0
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='score'
        AND submission_id NOT LIKE '%:r322archive')
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r322archive' AND kind='score'
        AND assignment_id NOT LIKE '%:score:rerun320')
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger
       WHERE round_id='arena-2026-09-19-r322archive'
        AND submission_id='baseline-2026-09-19:r322archive')=
      2376
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger ledger
       WHERE ledger.round_id='arena-2026-09-19-r322archive'
        AND ledger.submission_id='baseline-2026-09-19:r322archive'
        AND EXISTS(SELECT 1 FROM public.lab_arena_runs run
          WHERE run.round_id='arena-2026-09-19-r322archive'
...
END $archive322$;
ALTER FUNCTION public.lab_arena_sep19_rerun322_archive_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun322_archive_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun322_active_valid_v1()
RETURNS BOOLEAN LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public AS $active322$
...
BEGIN
 ...
 RETURN active.round_id IS NOT NULL AND archived.round_id IS NOT NULL
  AND public.lab_arena_sep19_rerun322_archive_valid_v1() IS TRUE
  AND pg_catalog.encode(extensions.digest(pg_catalog.jsonb_build_object(
  ...
  AND EXISTS(SELECT 1 FROM public.lab_arena_submissions baseline
    WHERE baseline.round_id='arena-2026-09-19'
     ...
     AND baseline.source_ref='arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun322-d1b794d5.tar.gz'
     AND baseline.source_size_bytes=837124
     AND baseline.submission_doc->>'source_sha256'='187a0813a4b026020c195f20cfbac9cc6653b8ff3dab6111082a84410bcc6697'
     AND baseline.submission_doc->>'source_commit'='d1b794d56fa54b860d44ac86d2e29972cbb3ed48')
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
  ...
  AND (SELECT pg_catalog.count(DISTINCT assignment_id) FROM public.lab_arena_runs
    WHERE round_id='arena-2026-09-19' AND kind='execute'
     ...
     AND assignment_id LIKE '%:rerun322')=20
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
    WHERE round_id='arena-2026-09-19' AND kind='execute'
     AND submission_id='baseline-2026-09-19'
     AND assignment_id NOT LIKE '%:rerun322')
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
    WHERE round_id='arena-2026-09-19' AND kind='score'
     AND assignment_id NOT LIKE '%:score:rerun322');
END $active322$;
ALTER FUNCTION public.lab_arena_sep19_rerun322_active_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun322_active_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

-- Replace the exact rerun320 guard and namespace in the current scorer. A
-- second stacked guard would reject rerun322 after the archive is created.
DO $patch_sep19_rerun322_scorer$
DECLARE definition TEXT; current_hash TEXT;
 old_assignment TEXT:=$assignment320$    IF p_round_id='arena-2026-09-19'
       AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
         WHERE round_id='arena-2026-09-19-r320archive') THEN
      v_assignment:=p_round_id||':'||v_scored.submission_id||':'||p_stage::TEXT||':'||
        v_scored.icp_position::TEXT||':score:rerun320';
    END IF;
    v_status := CASE WHEN v_cache.cache_key IS NULL THEN 'pending' ELSE 'accepted' END;$assignment320$;
 old_guard TEXT:=$guard320$  IF p_round_id='arena-2026-09-19'
     AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
       WHERE round_id='arena-2026-09-19-r320archive')
     AND public.lab_arena_sep19_rerun320_active_valid_v1() IS NOT TRUE THEN
    RAISE EXCEPTION 'lab_arena_sep19_rerun320_frozen_state_invalid'
      USING ERRCODE='22023';
  END IF;
  FOR v_item IN$guard320$;
BEGIN
 ...
 IF current_hash='38756e97556806ec2b708dcf7ed380003f26261b9d4c84a8a2dd31f9596cdc81' THEN
  IF (pg_catalog.length(definition)-pg_catalog.length(pg_catalog.replace(
   ...
   RAISE EXCEPTION 'Sep19 rerun322 scorer seam differs';
  END IF;
  definition:=pg_catalog.replace(definition,old_guard,$new_guard$  IF p_round_id='arena-2026-09-19'
     AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
       WHERE round_id='arena-2026-09-19-r322archive')
     AND public.lab_arena_sep19_rerun322_active_valid_v1() IS NOT TRUE THEN
    RAISE EXCEPTION 'lab_arena_sep19_rerun322_frozen_state_invalid'
      USING ERRCODE='22023';
  END IF;
  ...
  definition:=pg_catalog.replace(definition,old_assignment,$new_assignment$    IF p_round_id='arena-2026-09-19'
       AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
         WHERE round_id='arena-2026-09-19-r322archive') THEN
      v_assignment:=p_round_id||':'||v_scored.submission_id||':'||p_stage::TEXT||':'||
        v_scored.icp_position::TEXT||':score:rerun322';
    END IF;
    v_status := CASE WHEN v_cache.cache_key IS NULL THEN 'pending' ELSE 'accepted' END;$new_assignment$);
  EXECUTE definition;
 ELSIF current_hash<>'21dcff455fb074c7ce00cf79e3899298af92e6fee196631a1e33b4afbff917c0' THEN
  RAISE EXCEPTION 'Sep19 rerun322 scorer definition differs';
 END IF;
 IF pg_catalog.encode(extensions.digest(pg_catalog.pg_get_functiondef(
   'public.lab_arena_open_scoring_v2(text,smallint,jsonb)'::pg_catalog.regprocedure),
   'sha256'),'hex')<>'21dcff455fb074c7ce00cf79e3899298af92e6fee196631a1e33b4afbff917c0' THEN
  RAISE EXCEPTION 'Sep19 rerun322 scorer patch differs';
 END IF;
END $patch_sep19_rerun322_scorer$;

CREATE OR REPLACE FUNCTION public.lab_arena_prepare_sep19_rerun322_v1(
...
SET search_path=pg_catalog,public,extensions AS $prepare322$
DECLARE
...
BEGIN
 ...
 IF p_source_size_bytes<>837124
  OR p_source_sha256<>'187a0813a4b026020c195f20cfbac9cc6653b8ff3dab6111082a84410bcc6697'
  OR p_source_commit<>'d1b794d56fa54b860d44ac86d2e29972cbb3ed48'
  OR p_schedule IS DISTINCT FROM '{"benchmark_deadline":"2026-09-19T19:00:00Z","final_scoring_close":"2026-09-20T07:00:00Z","publication_deadline":"2026-09-20T07:00:01Z","stage_1_close":"2026-09-19T22:00:01Z","stage_1_scoring_close":"2026-09-20T01:00:00Z","stage_1_start":"2026-09-19T19:00:01Z","stage_2_close":"2026-09-20T04:00:01Z","stage_2_start":"2026-09-20T01:00:01Z","submission_cutoff":"2026-09-19T00:00:00Z","submission_open":"2026-09-18T00:00:00Z"}'::JSONB
  ...
  RAISE EXCEPTION 'Sep19 rerun322 source, judge, bank, or schedule differs'
   USING ERRCODE='22023';
 END IF;
 PERFORM pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('arena-2026-09-19-rerun322',0));
 ...
 IF EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r322archive') THEN
  IF public.lab_arena_sep19_rerun322_archive_valid_v1() IS NOT TRUE
   OR public.lab_arena_sep19_rerun322_active_valid_v1() IS NOT TRUE THEN
   RAISE EXCEPTION 'existing Sep19 rerun322 differs' USING ERRCODE='55000';
  END IF;
  RETURN pg_catalog.jsonb_build_object('status','existing',
   'round_id','arena-2026-09-19','fresh_baseline_execute_count',20,
   'retained_miner_execute_count',80,'score_namespace','score:rerun322');
 END IF;
 IF (p_schedule->>'benchmark_deadline')::TIMESTAMPTZ<=pg_catalog.clock_timestamp() THEN
  RAISE EXCEPTION 'Sep19 rerun322 admission window closed' USING ERRCODE='22023';
 END IF;
 ...
 IF active_round.status NOT IN('published','cancelled')
  ...
  OR public.lab_arena_sep19_rerun320_active_valid_v1() IS NOT TRUE
  ...
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_round)::TEXT,
       'sha256'),'hex')<>'526598f87146dd524cf4c368d74b059dbc26071c1b9549c2dbc0dac6c765db24'
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_baseline)::TEXT,
       'sha256'),'hex')<>'68cf40d0506ffa77d55fd39af0ddbaa5bef52ea354f4356bdac004ee3de9027c'
  ...
  OR (SELECT pg_catalog.count(DISTINCT assignment_id) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19'
       AND assignment_id LIKE '%:rerun320')<>20
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19'
       AND assignment_id NOT LIKE '%:rerun320')
  ...
  RAISE EXCEPTION 'Sep19 rerun322 terminal preimage differs' USING ERRCODE='55000';
 END IF;
 IF pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'a41083e4a0698dc643073f35819841484ee4fab2bac3c996b18e6a07aa4196a8'
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'1ed41423f4dda7201a9807c869ef2193c49fcb75eb739d7454aa27dce9cda886' THEN
  RAISE EXCEPTION 'Sep19 rerun322 terminal history differs' USING ERRCODE='55000';
 END IF;
 ...
 IF pg_catalog.jsonb_array_length(derived)<>110 THEN
  RAISE EXCEPTION 'Sep19 rerun322 derived judgment count differs'; END IF;
 expected_submission:=active_baseline.submission_doc||pg_catalog.jsonb_build_object(
  'source_ref','arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun322-d1b794d5.tar.gz','source_size_bytes',837124,
  'source_sha256','187a0813a4b026020c195f20cfbac9cc6653b8ff3dab6111082a84410bcc6697','source_commit','d1b794d56fa54b860d44ac86d2e29972cbb3ed48');
 SELECT pg_catalog.jsonb_agg(CASE WHEN item->>'submission_id'='baseline-2026-09-19'
   THEN item||pg_catalog.jsonb_build_object('source_ref','arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun322-d1b794d5.tar.gz',
    'source_size_bytes',837124) ELSE item END ORDER BY ordinal)
 ...
 archive_participants:=(SELECT pg_catalog.jsonb_agg(
   item||pg_catalog.jsonb_build_object('submission_id',(item->>'submission_id')||':r322archive')
 ...
 archive_config:=active_round.configuration_doc||pg_catalog.jsonb_build_object(
  'round_id','arena-2026-09-19-r322archive','mode','shadow','rewards_enabled',FALSE,
  ...
  'archived_terminal_round_sha256','526598f87146dd524cf4c368d74b059dbc26071c1b9549c2dbc0dac6c765db24',
  'archived_terminal_baseline_sha256','68cf40d0506ffa77d55fd39af0ddbaa5bef52ea354f4356bdac004ee3de9027c',
  'archived_terminal_miner_submissions_sha256','781027021d3cadc5f2c0540d9f24a67f65cb25126912ef3544867d5a70153297',
  'archived_terminal_runs_sha256','a41083e4a0698dc643073f35819841484ee4fab2bac3c996b18e6a07aa4196a8',
  'archived_terminal_ledger_sha256','1ed41423f4dda7201a9807c869ef2193c49fcb75eb739d7454aa27dce9cda886',
 ...
 SELECT pg_catalog.jsonb_build_object(
  'rounds',(SELECT pg_catalog.jsonb_agg(pg_catalog.to_jsonb(r) ORDER BY round_id)
    FROM public.lab_arena_rounds r WHERE round_id NOT IN(
     'arena-2026-09-19','arena-2026-09-19-r322archive')),
 ...
 INSERT INTO public.lab_arena_rounds(
  ...
  NULL::public.lab_arena_rounds,pg_catalog.to_jsonb(active_round)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r322archive','status','cancelled',
   ...
   'cancel_reason','authorized_sep19_terminal320_confirmed_cost_baseline_archive'));
 INSERT INTO public.lab_arena_submissions SELECT (pg_catalog.jsonb_populate_record(
  NULL::public.lab_arena_submissions,pg_catalog.to_jsonb(s)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r322archive',
   'submission_id',s.submission_id||':r322archive'))).*
  FROM public.lab_arena_submissions s WHERE s.round_id='arena-2026-09-19';
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r322archive',
  submission_id=submission_id||':r322archive'
  WHERE round_id='arena-2026-09-19' AND kind='score';
 GET DIAGNOSTICS moved_scores=ROW_COUNT;
 UPDATE public.lab_arena_ledger l SET round_id='arena-2026-09-19-r322archive',
  submission_id=submission_id||':r322archive'
  WHERE round_id='arena-2026-09-19' AND EXISTS(SELECT 1 FROM public.lab_arena_runs s
   WHERE s.round_id='arena-2026-09-19-r322archive' AND s.kind='score' AND s.run_id=l.run_id);
 GET DIAGNOSTICS moved_score_ledger=ROW_COUNT;
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r322archive',
  submission_id='baseline-2026-09-19:r322archive'
  WHERE round_id='arena-2026-09-19' AND kind='execute'
   AND submission_id='baseline-2026-09-19';
 GET DIAGNOSTICS moved_baseline_runs=ROW_COUNT;
 UPDATE public.lab_arena_ledger SET round_id='arena-2026-09-19-r322archive',
  submission_id='baseline-2026-09-19:r322archive'
  WHERE round_id='arena-2026-09-19' AND submission_id='baseline-2026-09-19';
 ...
 UPDATE public.lab_arena_runs SET per_icp_score=NULL,qualification_doc=NULL
  WHERE round_id='arena-2026-09-19' AND kind='execute'
   AND submission_id<>'baseline-2026-09-19';
 UPDATE public.lab_arena_submissions SET source_ref='arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun322-d1b794d5.tar.gz',
  source_size_bytes=837124,submission_doc=expected_submission,
  updated_at=pg_catalog.clock_timestamp()
  WHERE round_id='arena-2026-09-19' AND submission_id='baseline-2026-09-19';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep19 rerun322 baseline update missing'; END IF;
 UPDATE public.lab_arena_rounds SET status='stage1',
  ...
  updated_at=pg_catalog.clock_timestamp()
  WHERE round_id='arena-2026-09-19';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep19 rerun322 round update missing'; END IF;
 inserted_runs:=0;
 ...
 IF moved_scores<>0
  ...
  OR public.lab_arena_sep19_rerun322_archive_valid_v1() IS NOT TRUE
  OR public.lab_arena_sep19_rerun322_active_valid_v1() IS NOT TRUE
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
    WHERE round_id='arena-2026-09-19' AND kind='score') THEN
  RAISE EXCEPTION 'Sep19 rerun322 atomic preservation differs' USING ERRCODE='55000';
 END IF;
 RETURN pg_catalog.jsonb_build_object('status','prepared',
  ...
  'score_namespace','score:rerun322');
END $prepare322$;
ALTER FUNCTION public.lab_arena_prepare_sep19_rerun322_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_prepare_sep19_rerun322_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT)
 FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.lab_arena_prepare_sep19_rerun322_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) TO lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun322_score_namespace_guard_v1()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public AS $score_guard322$
BEGIN
 IF NEW.round_id='arena-2026-09-19' AND NEW.kind='score'
  AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r322archive')
  AND NEW.assignment_id IS DISTINCT FROM NEW.round_id||':'||NEW.submission_id||':'||
   NEW.stage::TEXT||':'||NEW.icp_position::TEXT||':score:rerun322' THEN
  RAISE EXCEPTION 'Sep19 rerun322 scoring requires exact namespace'
   USING ERRCODE='55000';
 END IF;
 RETURN NEW;
END $score_guard322$;
ALTER FUNCTION public.lab_arena_sep19_rerun322_score_namespace_guard_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun322_score_namespace_guard_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;
DROP TRIGGER IF EXISTS lab_arena_sep19_rerun320_score_namespace_guard
 ON public.lab_arena_runs;
DROP TRIGGER IF EXISTS lab_arena_sep19_rerun322_score_namespace_guard
 ON public.lab_arena_runs;
CREATE TRIGGER lab_arena_sep19_rerun322_score_namespace_guard
 BEFORE INSERT ON public.lab_arena_runs FOR EACH ROW
 EXECUTE FUNCTION public.lab_arena_sep19_rerun322_score_namespace_guard_v1();

SELECT public.lab_arena_prepare_sep19_rerun322_v1(
 837124,'187a0813a4b026020c195f20cfbac9cc6653b8ff3dab6111082a84410bcc6697','d1b794d56fa54b860d44ac86d2e29972cbb3ed48',
 '{"benchmark_deadline":"2026-09-19T19:00:00Z","final_scoring_close":"2026-09-20T07:00:00Z","publication_deadline":"2026-09-20T07:00:01Z","stage_1_close":"2026-09-19T22:00:01Z","stage_1_scoring_close":"2026-09-20T01:00:00Z","stage_1_start":"2026-09-19T19:00:01Z","stage_2_close":"2026-09-20T04:00:01Z","stage_2_start":"2026-09-20T01:00:01Z","submission_cutoff":"2026-09-19T00:00:00Z","submission_open":"2026-09-18T00:00:00Z"}'::JSONB,
