-- Render only after the Sep19 rerun322 round is terminal and every seal below
...
CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun323_archive_valid_v1()
...
BEGIN
 ...
 RETURN archived.status='cancelled'
  ...
  AND archived.cancel_reason='authorized_sep19_terminal322_completed_429_retry_baseline_archive'
  ...
  AND archived.configuration_doc->>'archived_terminal_cancel_reason'
      IS NOT DISTINCT FROM 'execution_incomplete:stage1:9'
  AND archived.configuration_doc->>'archived_terminal_round_sha256'='1b22b67d72eb4341e46c78e9ec469be2f60ee53b0ee3ca671d9f8354ff5ca068'
  AND archived.configuration_doc->>'archived_terminal_baseline_sha256'='9661c4da940e1fa55469a8084b328af1188ca889d036ad3d70d9f53f0a92d5d5'
  ...
  AND archived.configuration_doc->>'archived_terminal_runs_sha256'='5832c60c8b190761f8ee984734b5f0a396ad0c5eaff16c0a48e9130e37be7094'
  AND archived.configuration_doc->>'archived_terminal_ledger_sha256'='01edafd792ac11ac7b61ef6e81836c41f609bcca6b379b8766170803a2cc6b4e'
  ...
  AND pg_catalog.jsonb_array_length(derived)=119
  AND (SELECT pg_catalog.count(DISTINCT value->>'run_id')
       FROM pg_catalog.jsonb_array_elements(derived))=119
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_submissions
       WHERE round_id='arena-2026-09-19-r323archive')=5
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r323archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r323archive')=39
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r323archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r323archive'
        AND status='accepted')=2
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r323archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r323archive'
        AND status='failed')=37
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r323archive' AND kind='execute'
        AND submission_id='baseline-2026-09-19:r323archive'
  ...
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r323archive' AND kind='score')=0
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r323archive' AND kind='score'
        AND submission_id NOT LIKE '%:r323archive')
  AND NOT EXISTS(SELECT 1 FROM public.lab_arena_runs
       WHERE round_id='arena-2026-09-19-r323archive' AND kind='score'
  ...
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger ledger
       WHERE ledger.round_id='arena-2026-09-19-r323archive'
        AND ledger.submission_id='baseline-2026-09-19:r323archive'
        AND EXISTS(SELECT 1 FROM public.lab_arena_runs run
          WHERE run.round_id='arena-2026-09-19-r323archive'
...
END $archive322$;
ALTER FUNCTION public.lab_arena_sep19_rerun323_archive_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun323_archive_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun323_active_valid_v1()
RETURNS BOOLEAN LANGUAGE plpgsql STABLE SECURITY DEFINER
...
BEGIN
 ...
 RETURN active.round_id IS NOT NULL AND archived.round_id IS NOT NULL
  AND public.lab_arena_sep19_rerun323_archive_valid_v1() IS TRUE
  AND pg_catalog.encode(extensions.digest(pg_catalog.jsonb_build_object(
  ...
  AND EXISTS(SELECT 1 FROM public.lab_arena_submissions baseline
    WHERE baseline.round_id='arena-2026-09-19'
     ...
     AND baseline.source_ref='arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun323-45769176.tar.gz'
     AND baseline.source_size_bytes=852102
     AND baseline.submission_doc->>'source_sha256'='8950f2fc66002885a473b5b55200d93803661a00de8c61927a3768b115dd5b66'
     AND baseline.submission_doc->>'source_commit'='45769176ac7b33ccde1b175749cfd9b922c9dcc1')
  AND (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
...
END $active322$;
ALTER FUNCTION public.lab_arena_sep19_rerun323_active_valid_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun323_active_valid_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;

-- Replace the exact rerun322 guard and namespace in the current scorer. A
-- second stacked guard would reject rerun323 after the archive is created.
DO $patch_sep19_rerun323_scorer$
DECLARE definition TEXT; current_hash TEXT;
 old_assignment TEXT:=$assignment322$    IF p_round_id='arena-2026-09-19'
    ...
    END IF;
    v_status := CASE WHEN v_cache.cache_key IS NULL THEN 'pending' ELSE 'accepted' END;$assignment322$;
 old_guard TEXT:=$guard322$  IF p_round_id='arena-2026-09-19'
     AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
...
BEGIN
 ...
 IF current_hash='21dcff455fb074c7ce00cf79e3899298af92e6fee196631a1e33b4afbff917c0' THEN
  IF (pg_catalog.length(definition)-pg_catalog.length(pg_catalog.replace(
   ...
   RAISE EXCEPTION 'Sep19 rerun323 scorer seam differs';
  END IF;
  ...
  EXECUTE definition;
 ELSIF current_hash<>'bb3ba46e0d4412250eb43182995920e500616a5950c96fbf4b2809f6c077589e' THEN
  RAISE EXCEPTION 'Sep19 rerun323 scorer definition differs';
 END IF;
 IF pg_catalog.encode(extensions.digest(pg_catalog.pg_get_functiondef(
   'public.lab_arena_open_scoring_v2(text,smallint,jsonb)'::pg_catalog.regprocedure),
   'sha256'),'hex')<>'bb3ba46e0d4412250eb43182995920e500616a5950c96fbf4b2809f6c077589e' THEN
  RAISE EXCEPTION 'Sep19 rerun323 scorer patch differs';
 END IF;
END $patch_sep19_rerun323_scorer$;

CREATE OR REPLACE FUNCTION public.lab_arena_prepare_sep19_rerun323_v1(
...
BEGIN
 ...
 IF p_source_size_bytes<>852102
  OR p_source_sha256<>'8950f2fc66002885a473b5b55200d93803661a00de8c61927a3768b115dd5b66'
  OR p_source_commit<>'45769176ac7b33ccde1b175749cfd9b922c9dcc1'
  OR p_schedule IS DISTINCT FROM '{"benchmark_deadline":"2026-09-19T21:00:00Z","final_scoring_close":"2026-09-20T09:00:00Z","publication_deadline":"2026-09-20T09:00:01Z","stage_1_close":"2026-09-20T00:00:01Z","stage_1_scoring_close":"2026-09-20T03:00:00Z","stage_1_start":"2026-09-19T21:00:01Z","stage_2_close":"2026-09-20T06:00:01Z","stage_2_start":"2026-09-20T03:00:01Z","submission_cutoff":"2026-09-19T00:00:00Z","submission_open":"2026-09-18T00:00:00Z"}'::JSONB
  ...
  RAISE EXCEPTION 'Sep19 rerun323 source, judge, bank, or schedule differs'
   USING ERRCODE='22023';
 END IF;
 PERFORM pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('arena-2026-09-19-rerun323',0));
 ...
 IF EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r323archive') THEN
  IF public.lab_arena_sep19_rerun323_archive_valid_v1() IS NOT TRUE
   OR public.lab_arena_sep19_rerun323_active_valid_v1() IS NOT TRUE THEN
   RAISE EXCEPTION 'existing Sep19 rerun323 differs' USING ERRCODE='55000';
  END IF;
  RETURN pg_catalog.jsonb_build_object('status','existing',
   'round_id','arena-2026-09-19','fresh_baseline_execute_count',20,
   'retained_miner_execute_count',80,'score_namespace','score:rerun323');
 END IF;
 IF (p_schedule->>'benchmark_deadline')::TIMESTAMPTZ<=pg_catalog.clock_timestamp() THEN
  RAISE EXCEPTION 'Sep19 rerun323 admission window closed' USING ERRCODE='22023';
 END IF;
 ...
 IF active_round.status NOT IN('published','cancelled')
  ...
  OR active_round.cancel_reason IS DISTINCT FROM 'execution_incomplete:stage1:9'
  ...
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_round)::TEXT,
       'sha256'),'hex')<>'1b22b67d72eb4341e46c78e9ec469be2f60ee53b0ee3ca671d9f8354ff5ca068'
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active_baseline)::TEXT,
       'sha256'),'hex')<>'9661c4da940e1fa55469a8084b328af1188ca889d036ad3d70d9f53f0a92d5d5'
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      ...
      <>82
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute')<>119
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19')<>39
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-19' AND kind='execute'
       AND submission_id='baseline-2026-09-19' AND status='accepted')
      <>2
  ...
  RAISE EXCEPTION 'Sep19 rerun323 terminal preimage differs' USING ERRCODE='55000';
 END IF;
 IF pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'5832c60c8b190761f8ee984734b5f0a396ad0c5eaff16c0a48e9130e37be7094'
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
      ...
      WHERE round_id='arena-2026-09-19'),''),'sha256'),'hex')<>'01edafd792ac11ac7b61ef6e81836c41f609bcca6b379b8766170803a2cc6b4e' THEN
  RAISE EXCEPTION 'Sep19 rerun323 terminal history differs' USING ERRCODE='55000';
 END IF;
 SELECT pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
  ...
  WHERE round_id='arena-2026-09-19' AND kind='execute';
 IF pg_catalog.jsonb_array_length(derived)<>119 THEN
  RAISE EXCEPTION 'Sep19 rerun323 derived judgment count differs'; END IF;
 expected_submission:=active_baseline.submission_doc||pg_catalog.jsonb_build_object(
  'source_ref','arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun323-45769176.tar.gz','source_size_bytes',852102,
  'source_sha256','8950f2fc66002885a473b5b55200d93803661a00de8c61927a3768b115dd5b66','source_commit','45769176ac7b33ccde1b175749cfd9b922c9dcc1');
 SELECT pg_catalog.jsonb_agg(CASE WHEN item->>'submission_id'='baseline-2026-09-19'
   THEN item||pg_catalog.jsonb_build_object('source_ref','arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun323-45769176.tar.gz',
    'source_size_bytes',852102) ELSE item END ORDER BY ordinal)
 ...
 archive_participants:=(SELECT pg_catalog.jsonb_agg(
   item||pg_catalog.jsonb_build_object('submission_id',(item->>'submission_id')||':r323archive')
 ...
 archive_config:=active_round.configuration_doc||pg_catalog.jsonb_build_object(
  'round_id','arena-2026-09-19-r323archive','mode','shadow','rewards_enabled',FALSE,
  ...
  'archived_terminal_round_sha256','1b22b67d72eb4341e46c78e9ec469be2f60ee53b0ee3ca671d9f8354ff5ca068',
  'archived_terminal_baseline_sha256','9661c4da940e1fa55469a8084b328af1188ca889d036ad3d70d9f53f0a92d5d5',
  'archived_terminal_miner_submissions_sha256','781027021d3cadc5f2c0540d9f24a67f65cb25126912ef3544867d5a70153297',
  'archived_terminal_runs_sha256','5832c60c8b190761f8ee984734b5f0a396ad0c5eaff16c0a48e9130e37be7094',
  'archived_terminal_ledger_sha256','01edafd792ac11ac7b61ef6e81836c41f609bcca6b379b8766170803a2cc6b4e',
 ...
 SELECT pg_catalog.jsonb_build_object(
  'rounds',(SELECT pg_catalog.jsonb_agg(pg_catalog.to_jsonb(r) ORDER BY round_id)
    FROM public.lab_arena_rounds r WHERE round_id NOT IN(
     'arena-2026-09-19','arena-2026-09-19-r323archive')),
 ...
 INSERT INTO public.lab_arena_rounds(
  ...
  NULL::public.lab_arena_rounds,pg_catalog.to_jsonb(active_round)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r323archive','status','cancelled',
   ...
   'cancel_reason','authorized_sep19_terminal322_completed_429_retry_baseline_archive'));
 INSERT INTO public.lab_arena_submissions SELECT (pg_catalog.jsonb_populate_record(
  NULL::public.lab_arena_submissions,pg_catalog.to_jsonb(s)||pg_catalog.jsonb_build_object(
   'round_id','arena-2026-09-19-r323archive',
   'submission_id',s.submission_id||':r323archive'))).*
  FROM public.lab_arena_submissions s WHERE s.round_id='arena-2026-09-19';
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r323archive',
  submission_id=submission_id||':r323archive'
  WHERE round_id='arena-2026-09-19' AND kind='score';
 GET DIAGNOSTICS moved_scores=ROW_COUNT;
 UPDATE public.lab_arena_ledger l SET round_id='arena-2026-09-19-r323archive',
  submission_id=submission_id||':r323archive'
  WHERE round_id='arena-2026-09-19' AND EXISTS(SELECT 1 FROM public.lab_arena_runs s
   WHERE s.round_id='arena-2026-09-19-r323archive' AND s.kind='score' AND s.run_id=l.run_id);
 GET DIAGNOSTICS moved_score_ledger=ROW_COUNT;
 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-19-r323archive',
  submission_id='baseline-2026-09-19:r323archive'
  WHERE round_id='arena-2026-09-19' AND kind='execute'
   AND submission_id='baseline-2026-09-19';
 GET DIAGNOSTICS moved_baseline_runs=ROW_COUNT;
 UPDATE public.lab_arena_ledger SET round_id='arena-2026-09-19-r323archive',
  submission_id='baseline-2026-09-19:r323archive'
  WHERE round_id='arena-2026-09-19' AND submission_id='baseline-2026-09-19';
 ...
 UPDATE public.lab_arena_runs SET per_icp_score=NULL,qualification_doc=NULL
  WHERE round_id='arena-2026-09-19' AND kind='execute'
   AND submission_id<>'baseline-2026-09-19';
 UPDATE public.lab_arena_submissions SET source_ref='arena/arena-2026-09-19/sources/baseline-2026-09-19-rerun323-45769176.tar.gz',
  source_size_bytes=852102,submission_doc=expected_submission,
  updated_at=pg_catalog.clock_timestamp()
  WHERE round_id='arena-2026-09-19' AND submission_id='baseline-2026-09-19';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep19 rerun323 baseline update missing'; END IF;
 UPDATE public.lab_arena_rounds SET status='stage1',
  ...
  updated_at=pg_catalog.clock_timestamp()
  WHERE round_id='arena-2026-09-19';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep19 rerun323 round update missing'; END IF;
 inserted_runs:=0;
 ...
 IF moved_scores<>0
  ...
  OR moved_baseline_runs<>39
  OR moved_baseline_ledger<>7467
  ...
  OR public.lab_arena_sep19_rerun323_archive_valid_v1() IS NOT TRUE
  OR public.lab_arena_sep19_rerun323_active_valid_v1() IS NOT TRUE
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
    WHERE round_id='arena-2026-09-19' AND kind='score') THEN
  RAISE EXCEPTION 'Sep19 rerun323 atomic preservation differs' USING ERRCODE='55000';
 END IF;
 RETURN pg_catalog.jsonb_build_object('status','prepared',
  ...
  'score_namespace','score:rerun323');
END $prepare322$;
ALTER FUNCTION public.lab_arena_prepare_sep19_rerun323_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_prepare_sep19_rerun323_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT)
 FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.lab_arena_prepare_sep19_rerun323_v1(
 BIGINT,TEXT,TEXT,JSONB,TEXT,TEXT) TO lab_arena_service;

CREATE OR REPLACE FUNCTION public.lab_arena_sep19_rerun323_score_namespace_guard_v1()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public AS $score_guard323$
BEGIN
 IF NEW.round_id='arena-2026-09-19' AND NEW.kind='score'
  AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
   WHERE round_id='arena-2026-09-19-r323archive')
  AND NEW.assignment_id IS DISTINCT FROM NEW.round_id||':'||NEW.submission_id||':'||
   NEW.stage::TEXT||':'||NEW.icp_position::TEXT||':score:rerun323' THEN
  RAISE EXCEPTION 'Sep19 rerun323 scoring requires exact namespace'
   USING ERRCODE='55000';
 END IF;
 RETURN NEW;
END $score_guard323$;
ALTER FUNCTION public.lab_arena_sep19_rerun323_score_namespace_guard_v1()
 OWNER TO lab_arena_owner;
REVOKE ALL ON FUNCTION public.lab_arena_sep19_rerun323_score_namespace_guard_v1()
 FROM PUBLIC,anon,authenticated,service_role,lab_arena_service;
...
DROP TRIGGER IF EXISTS lab_arena_sep19_rerun323_score_namespace_guard
 ON public.lab_arena_runs;
CREATE TRIGGER lab_arena_sep19_rerun323_score_namespace_guard
 BEFORE INSERT ON public.lab_arena_runs FOR EACH ROW
 EXECUTE FUNCTION public.lab_arena_sep19_rerun323_score_namespace_guard_v1();

SELECT public.lab_arena_prepare_sep19_rerun323_v1(
 852102,'8950f2fc66002885a473b5b55200d93803661a00de8c61927a3768b115dd5b66','45769176ac7b33ccde1b175749cfd9b922c9dcc1',
 '{"benchmark_deadline":"2026-09-19T21:00:00Z","final_scoring_close":"2026-09-20T09:00:00Z","publication_deadline":"2026-09-20T09:00:01Z","stage_1_close":"2026-09-20T00:00:01Z","stage_1_scoring_close":"2026-09-20T03:00:00Z","stage_1_start":"2026-09-19T21:00:01Z","stage_2_close":"2026-09-20T06:00:01Z","stage_2_start":"2026-09-20T03:00:01Z","submission_cutoff":"2026-09-19T00:00:00Z","submission_open":"2026-09-18T00:00:00Z"}'::JSONB,
