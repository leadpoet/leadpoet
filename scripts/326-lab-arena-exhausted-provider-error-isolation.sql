-- A provider-owned execution failure that remains after both normal attempts
-- scores zero for that ICP.  Other infrastructure and trust failures still
-- stop the round.  This migration changes only future stage closure; it does
-- not rewrite historical rounds or run results.

BEGIN;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '30s';

DO $exhausted_provider_error_prerequisites$
BEGIN
  IF pg_catalog.to_regprocedure(
    ...
    RAISE EXCEPTION 'apply migrations 193 and 255 before migration 326';
  END IF;
END;
$exhausted_provider_error_prerequisites$;
