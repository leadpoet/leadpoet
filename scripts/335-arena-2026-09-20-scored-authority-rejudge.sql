-- Render only after rerun332 is scored but unpublished, drained, and every
...
DO $patch_sep20_rerun335_scoring$
DECLARE
 definition TEXT;
 ...
 replacement TEXT:=$replacement$    IF p_round_id='arena-2026-09-20'
       AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
         WHERE round_id='arena-2026-09-20-r335archive') THEN
      v_assignment:=p_round_id||':'||v_scored.submission_id||':'||p_stage::TEXT||':'||
        v_scored.icp_position::TEXT||':score:rerun335';
    END IF;
...
BEGIN
 ...
 IF definition_hash='3b551cc00f123897e6672df49f7aa7eb6092dc51d62a871725c5f8a4627b4474' THEN
  IF (pg_catalog.length(definition)-pg_catalog.length(pg_catalog.replace(
      definition,anchor,'')))<>pg_catalog.length(anchor) THEN
   RAISE EXCEPTION 'Sep20 rerun335 scoring seam differs';
  END IF;
  EXECUTE pg_catalog.replace(definition,anchor,replacement);
 ELSIF definition_hash<>'9f52f5a9ee3b24c70dbaee9d67b5db17159d7e45cbd64bcc83b4acfc02a48396' THEN
  RAISE EXCEPTION 'Sep20 rerun335 scoring definition differs';
 END IF;
 IF pg_catalog.encode(extensions.digest(pg_catalog.pg_get_functiondef(
   'public.lab_arena_open_scoring_v2(text,smallint,jsonb)'::pg_catalog.regprocedure),
   'sha256'),'hex')<>'9f52f5a9ee3b24c70dbaee9d67b5db17159d7e45cbd64bcc83b4acfc02a48396' THEN
  RAISE EXCEPTION 'Sep20 rerun335 scoring patch differs';
 END IF;
END $patch_sep20_rerun335_scoring$;
...
BEGIN
 IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_trigger trigger
  ...
  RAISE EXCEPTION 'Sep20 rerun335 publication stop differs' USING ERRCODE='55000';
 END IF;
...
BEGIN
 PERFORM pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('arena-2026-09-20-rerun335',0));
  ...
  IF NOT EXISTS(SELECT 1 FROM public.lab_arena_rounds archived
      WHERE archived.round_id='arena-2026-09-20-r335archive'
       ...
       AND archived.configuration_doc->>'archived_terminal_round_sha256'=
           '04003438459d829574d8f6e352cab8cf23e66b2bd3a5eb6ca05a4f0367a97e98'
       ...
       AND archived.configuration_doc->>'archived_terminal_runs_sha256'=
           'c32ebf55e13a60cd434174f07a2139fb4a1161515fd9ef716f3e0dbc9c69efe1'
       AND archived.configuration_doc->>'archived_terminal_ledger_sha256'=
           '68c5211abb6a53aeb289ccb01e690d09ad52b9762b7edf08d56b927d7b9d57f9'
       ...
       AND archived.configuration_doc->>'archived_terminal_judgment_cache_sha256'=
           '574a5c54395f1ec459d65dad3ffcf42c083a6114e87013f9eae53886e8dcf82e'
    ...
    OR active.configuration_doc->>'scorer_image_reference' IS DISTINCT FROM
       '493765492819.dkr.ecr.us-east-1.amazonaws.com/leadpoet/sourcing-model@sha256:c0342bcced7f7de2552427cbb6fcf62ab6f10f42a63c0004d1466727863343ec'
    ...
    OR active.configuration_doc->'schedule' IS DISTINCT FROM
       '{"benchmark_deadline":"2026-09-20T11:35:00Z","final_scoring_close":"2026-09-21T01:35:00Z","publication_deadline":"2026-09-21T01:35:01Z","stage_1_close":"2026-09-20T18:35:00Z","stage_1_scoring_close":"2026-09-20T21:35:00Z","stage_1_start":"2026-09-20T11:35:01Z","stage_2_close":"2026-09-20T22:35:00Z","stage_2_start":"2026-09-20T21:35:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB
    ...
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_submissions
        WHERE round_id='arena-2026-09-20-r335archive')<>
       8
    OR EXISTS(SELECT 1 FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r335archive' AND kind='execute')
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r335archive' AND kind='score')<>
       98
   ...
   RAISE EXCEPTION 'existing Sep20 rerun335 differs' USING ERRCODE='55000';
  END IF;
  RETURN;
 END IF;

 IF ('{"benchmark_deadline":"2026-09-20T11:35:00Z","final_scoring_close":"2026-09-21T01:35:00Z","publication_deadline":"2026-09-21T01:35:01Z","stage_1_close":"2026-09-20T18:35:00Z","stage_1_scoring_close":"2026-09-20T21:35:00Z","stage_1_start":"2026-09-20T11:35:01Z","stage_2_close":"2026-09-20T22:35:00Z","stage_2_start":"2026-09-20T21:35:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB->>'submission_open') IS DISTINCT FROM
      '2026-09-19T00:00:00Z'
 OR ('{"benchmark_deadline":"2026-09-20T11:35:00Z","final_scoring_close":"2026-09-21T01:35:00Z","publication_deadline":"2026-09-21T01:35:01Z","stage_1_close":"2026-09-20T18:35:00Z","stage_1_scoring_close":"2026-09-20T21:35:00Z","stage_1_start":"2026-09-20T11:35:01Z","stage_2_close":"2026-09-20T22:35:00Z","stage_2_start":"2026-09-20T21:35:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB->>'submission_cutoff') IS DISTINCT FROM
  ...
  RAISE EXCEPTION 'Sep20 rerun335 schedule differs' USING ERRCODE='22023';
 END IF;

 IF 'sha256:c0342bcced7f7de2552427cbb6fcf62ab6f10f42a63c0004d1466727863343ec' !~ '^sha256:[0-9a-f]{64}$'
  OR '493765492819.dkr.ecr.us-east-1.amazonaws.com/leadpoet/sourcing-model@sha256:c0342bcced7f7de2552427cbb6fcf62ab6f10f42a63c0004d1466727863343ec' IS DISTINCT FROM
    '493765492819.dkr.ecr.us-east-1.amazonaws.com/leadpoet/sourcing-model@'||
    'sha256:c0342bcced7f7de2552427cbb6fcf62ab6f10f42a63c0004d1466727863343ec' THEN
  RAISE EXCEPTION 'Sep20 rerun335 scorer identity differs' USING ERRCODE='22023';
 END IF;
 ...
 IF active.status IS DISTINCT FROM 'scored'
  ...
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active)::TEXT,
       'sha256'),'hex')<>'04003438459d829574d8f6e352cab8cf23e66b2bd3a5eb6ca05a4f0367a97e98'
  ...
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20' AND kind='score'
       AND (status<>'accepted' OR terminal_cause<>'accepted'
        OR assignment_id NOT LIKE '%:score:rerun332'))
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20' AND kind='execute'
       AND submission_id='baseline-2026-09-20')<>
     24
  ...
  RAISE EXCEPTION 'Sep20 rerun335 terminal preimage differs' USING ERRCODE='55000';
 END IF;
 ...
 IF pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger
      WHERE round_id='arena-2026-09-20')<>40828
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
    ...
    'sha256'),'hex')<>'574a5c54395f1ec459d65dad3ffcf42c083a6114e87013f9eae53886e8dcf82e'
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_judgment_cache)<>
       921 THEN
  RAISE EXCEPTION 'Sep20 rerun335 terminal history differs' USING ERRCODE='55000';
 END IF;
 ...
 SELECT pg_catalog.jsonb_agg(item||pg_catalog.jsonb_build_object(
   'submission_id',(item->>'submission_id')||':r335archive') ORDER BY ordinal)
 ...
 archive_config:=active.configuration_doc||pg_catalog.jsonb_build_object(
  'round_id','arena-2026-09-20-r335archive','mode','shadow','rewards_enabled',FALSE,
  'archived_terminal_status',active.status,
  'archived_terminal_cancel_reason',active.cancel_reason,
  'archived_terminal_round_sha256','04003438459d829574d8f6e352cab8cf23e66b2bd3a5eb6ca05a4f0367a97e98',
  ...
  'archived_terminal_runs_sha256','c32ebf55e13a60cd434174f07a2139fb4a1161515fd9ef716f3e0dbc9c69efe1',
  'archived_terminal_ledger_sha256','68c5211abb6a53aeb289ccb01e690d09ad52b9762b7edf08d56b927d7b9d57f9',
  'archived_terminal_company_judgments_sha256',
   '0ef317eb17e2318bae2ef9d4bc8de770cc8cbbae941ab9456941b2c44b9c5c41',
  'archived_terminal_judgment_cache_sha256','574a5c54395f1ec459d65dad3ffcf42c083a6114e87013f9eae53886e8dcf82e',
 ...
 SELECT round_id,status,status_generation,stage_generation,configuration_doc,
  ...
  champion_fallback_providers FROM pg_catalog.jsonb_populate_record(
   ...
   pg_catalog.jsonb_build_object(
    'round_id','arena-2026-09-20-r335archive','status','cancelled',
 ...
 INSERT INTO public.lab_arena_submissions SELECT (pg_catalog.jsonb_populate_record(
  ...
  pg_catalog.jsonb_build_object('round_id','arena-2026-09-20-r335archive',
   'submission_id',s.submission_id||':r335archive'))).*
 FROM public.lab_arena_submissions s WHERE s.round_id='arena-2026-09-20';

 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-20-r335archive',
  submission_id=submission_id||':r335archive'
 WHERE round_id='arena-2026-09-20' AND kind='score';
 GET DIAGNOSTICS moved_score_runs=ROW_COUNT;
 UPDATE public.lab_arena_ledger l SET round_id='arena-2026-09-20-r335archive',
  submission_id=submission_id||':r335archive'
 WHERE round_id='arena-2026-09-20' AND EXISTS(SELECT 1
  FROM public.lab_arena_runs s WHERE s.round_id='arena-2026-09-20-r335archive'
 ...
 UPDATE public.lab_arena_rounds SET status='stage1',
  ...
  configuration_doc=configuration_doc||pg_catalog.jsonb_build_object(
   'schedule','{"benchmark_deadline":"2026-09-20T11:35:00Z","final_scoring_close":"2026-09-21T01:35:00Z","publication_deadline":"2026-09-21T01:35:01Z","stage_1_close":"2026-09-20T18:35:00Z","stage_1_scoring_close":"2026-09-20T21:35:00Z","stage_1_start":"2026-09-20T11:35:01Z","stage_2_close":"2026-09-20T22:35:00Z","stage_2_start":"2026-09-20T21:35:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB,
   'scorer_image_digest',
    'sha256:c0342bcced7f7de2552427cbb6fcf62ab6f10f42a63c0004d1466727863343ec',
 ...
 WHERE round_id='arena-2026-09-20';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep20 rerun335 round update missing'; END IF;
 ...
 IF moved_score_runs<>98
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20' AND kind='execute'
       AND status='accepted' AND terminal_cause='accepted')<>
     98 THEN
  RAISE EXCEPTION 'Sep20 rerun335 preservation differs' USING ERRCODE='55000';
 END IF;
END $sep20_rerun335$;
COMMIT;
