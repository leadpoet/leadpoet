-- Render only after rerun335 is scored but unpublished, drained, and every
...
DO $patch_sep20_rerun337_scoring$
DECLARE
 definition TEXT;
 ...
 replacement TEXT:=$replacement$    IF p_round_id='arena-2026-09-20'
       AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
         WHERE round_id='arena-2026-09-20-r337archive') THEN
      v_assignment:=p_round_id||':'||v_scored.submission_id||':'||p_stage::TEXT||':'||
        v_scored.icp_position::TEXT||':score:rerun337';
    END IF;
...
BEGIN
 ...
 IF definition_hash='9f52f5a9ee3b24c70dbaee9d67b5db17159d7e45cbd64bcc83b4acfc02a48396' THEN
  IF (pg_catalog.length(definition)-pg_catalog.length(pg_catalog.replace(
      definition,anchor,'')))<>pg_catalog.length(anchor) THEN
   RAISE EXCEPTION 'Sep20 rerun337 scoring seam differs';
  END IF;
  EXECUTE pg_catalog.replace(definition,anchor,replacement);
 ELSIF definition_hash<>'1f7964d1880378c83b2247ca03811528d8eee8680f3c7c4c866dcf9e9ebbeefb' THEN
  RAISE EXCEPTION 'Sep20 rerun337 scoring definition differs';
 END IF;
 IF pg_catalog.encode(extensions.digest(pg_catalog.pg_get_functiondef(
   'public.lab_arena_open_scoring_v2(text,smallint,jsonb)'::pg_catalog.regprocedure),
   'sha256'),'hex')<>'1f7964d1880378c83b2247ca03811528d8eee8680f3c7c4c866dcf9e9ebbeefb' THEN
  RAISE EXCEPTION 'Sep20 rerun337 scoring patch differs';
 END IF;
END $patch_sep20_rerun337_scoring$;
...
BEGIN
 IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_trigger trigger
  ...
  RAISE EXCEPTION 'Sep20 rerun337 publication stop differs' USING ERRCODE='55000';
 END IF;
...
BEGIN
 IF EXISTS(SELECT 1 FROM pg_catalog.pg_trigger
  ...
  RAISE EXCEPTION 'Sep20 rerun337 USER trigger state differs'
   USING ERRCODE='55000';
 END IF;
...
BEGIN
 PERFORM pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('arena-2026-09-20-rerun337',0));
  ...
  IF NOT EXISTS(SELECT 1 FROM public.lab_arena_rounds archived
      WHERE archived.round_id='arena-2026-09-20-r337archive'
       ...
       AND archived.configuration_doc->>'archived_terminal_round_sha256'=
           '02192a07c0c628c18be568e088d5cb56a0c052a4f103a2d2bd51d13abf5cc917'
       ...
       AND archived.configuration_doc->>'archived_terminal_runs_sha256'=
           'f76d4e8d2448b4b6d6e591d9a91e861ea6b9577cad15f348117609f6d0cb7b42'
       AND archived.configuration_doc->>'archived_terminal_ledger_sha256'=
           'b114587b1b99fafd925ef0acd814a2b9aa64ec9f7686c93fb8e8e534c5cd1349'
       ...
       AND archived.configuration_doc->>'archived_terminal_judgment_cache_sha256'=
           '16000c8699d6e8af2fc6a55e3203ea7589ab3e3046b7865525dd71341717ae17'
    ...
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_submissions
        WHERE round_id='arena-2026-09-20-r337archive')<>
       8
    OR EXISTS(SELECT 1 FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r337archive' AND kind='execute')
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r337archive' AND kind='score')<>
       98
   ...
   RAISE EXCEPTION 'existing Sep20 rerun337 differs' USING ERRCODE='55000';
  END IF;
  RETURN;
 END IF;

 IF ('{"benchmark_deadline":"2026-09-20T13:30:00Z","final_scoring_close":"2026-09-21T03:30:00Z","publication_deadline":"2026-09-21T03:30:01Z","stage_1_close":"2026-09-20T20:30:00Z","stage_1_scoring_close":"2026-09-20T23:30:00Z","stage_1_start":"2026-09-20T13:30:01Z","stage_2_close":"2026-09-21T00:30:00Z","stage_2_start":"2026-09-20T23:30:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB->>'submission_open') IS DISTINCT FROM
      '2026-09-19T00:00:00Z'
 OR ('{"benchmark_deadline":"2026-09-20T13:30:00Z","final_scoring_close":"2026-09-21T03:30:00Z","publication_deadline":"2026-09-21T03:30:01Z","stage_1_close":"2026-09-20T20:30:00Z","stage_1_scoring_close":"2026-09-20T23:30:00Z","stage_1_start":"2026-09-20T13:30:01Z","stage_2_close":"2026-09-21T00:30:00Z","stage_2_start":"2026-09-20T23:30:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB->>'submission_cutoff') IS DISTINCT FROM
 ...
 IF active.status IS DISTINCT FROM 'scored'
  ...
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active)::TEXT,
       'sha256'),'hex')<>'02192a07c0c628c18be568e088d5cb56a0c052a4f103a2d2bd51d13abf5cc917'
  ...
  OR EXISTS(SELECT 1 FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20' AND kind='score'
       AND (status<>'accepted' OR terminal_cause<>'accepted'
        OR assignment_id NOT LIKE '%:score:rerun335'))
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20' AND kind='execute'
       AND submission_id='baseline-2026-09-20')<>
     24
  ...
  RAISE EXCEPTION 'Sep20 rerun337 terminal preimage differs' USING ERRCODE='55000';
 END IF;
 ...
 IF pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger
      WHERE round_id='arena-2026-09-20')<>40831
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
    ...
    'sha256'),'hex')<>'16000c8699d6e8af2fc6a55e3203ea7589ab3e3046b7865525dd71341717ae17'
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_judgment_cache)<>
       976 THEN
  RAISE EXCEPTION 'Sep20 rerun337 terminal history differs' USING ERRCODE='55000';
 END IF;
 ...
 SELECT pg_catalog.jsonb_agg(item||pg_catalog.jsonb_build_object(
   'submission_id',(item->>'submission_id')||':r337archive') ORDER BY ordinal)
 ...
 archive_config:=active.configuration_doc||pg_catalog.jsonb_build_object(
  'round_id','arena-2026-09-20-r337archive','mode','shadow','rewards_enabled',FALSE,
  'archived_terminal_status',active.status,
  'archived_terminal_cancel_reason',active.cancel_reason,
  'archived_terminal_round_sha256','02192a07c0c628c18be568e088d5cb56a0c052a4f103a2d2bd51d13abf5cc917',
  ...
  'archived_terminal_runs_sha256','f76d4e8d2448b4b6d6e591d9a91e861ea6b9577cad15f348117609f6d0cb7b42',
  'archived_terminal_ledger_sha256','b114587b1b99fafd925ef0acd814a2b9aa64ec9f7686c93fb8e8e534c5cd1349',
  'archived_terminal_company_judgments_sha256',
   '0ef317eb17e2318bae2ef9d4bc8de770cc8cbbae941ab9456941b2c44b9c5c41',
  'archived_terminal_judgment_cache_sha256','16000c8699d6e8af2fc6a55e3203ea7589ab3e3046b7865525dd71341717ae17',
 ...
 SELECT round_id,status,status_generation,stage_generation,configuration_doc,
  ...
  champion_fallback_providers FROM pg_catalog.jsonb_populate_record(
   ...
   pg_catalog.jsonb_build_object(
    'round_id','arena-2026-09-20-r337archive','status','cancelled',
 ...
 INSERT INTO public.lab_arena_submissions SELECT (pg_catalog.jsonb_populate_record(
  ...
  pg_catalog.jsonb_build_object('round_id','arena-2026-09-20-r337archive',
   'submission_id',s.submission_id||':r337archive'))).*
 FROM public.lab_arena_submissions s WHERE s.round_id='arena-2026-09-20';

 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-20-r337archive',
  submission_id=submission_id||':r337archive'
 WHERE round_id='arena-2026-09-20' AND kind='score';
 GET DIAGNOSTICS moved_score_runs=ROW_COUNT;
 UPDATE public.lab_arena_ledger l SET round_id='arena-2026-09-20-r337archive',
  submission_id=submission_id||':r337archive'
 WHERE round_id='arena-2026-09-20' AND EXISTS(SELECT 1
  FROM public.lab_arena_runs s WHERE s.round_id='arena-2026-09-20-r337archive'
 ...
 WHERE round_id='arena-2026-09-20';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep20 rerun337 round update missing'; END IF;
 ...
 IF moved_score_runs<>98
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20' AND kind='execute'
       AND status='accepted' AND terminal_cause='accepted')<>
     98 THEN
  RAISE EXCEPTION 'Sep20 rerun337 preservation differs' USING ERRCODE='55000';
 END IF;
END $sep20_rerun337$;
COMMIT;
