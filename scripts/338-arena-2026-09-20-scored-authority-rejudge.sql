-- Render only after rerun337 is scored but unpublished, drained, and every
...
DO $patch_sep20_rerun338_scoring$
DECLARE
 definition TEXT;
 ...
 replacement TEXT:=$replacement$    IF p_round_id='arena-2026-09-20'
       AND EXISTS(SELECT 1 FROM public.lab_arena_rounds
         WHERE round_id='arena-2026-09-20-r338archive') THEN
      v_assignment:=p_round_id||':'||v_scored.submission_id||':'||p_stage::TEXT||':'||
        v_scored.icp_position::TEXT||':score:rerun338';
    END IF;
...
BEGIN
 ...
 IF definition_hash='1f7964d1880378c83b2247ca03811528d8eee8680f3c7c4c866dcf9e9ebbeefb' THEN
  IF (pg_catalog.length(definition)-pg_catalog.length(pg_catalog.replace(
      definition,anchor,'')))<>pg_catalog.length(anchor) THEN
   RAISE EXCEPTION 'Sep20 rerun338 scoring seam differs';
  END IF;
  EXECUTE pg_catalog.replace(definition,anchor,replacement);
 ELSIF definition_hash<>'7bd21582d2d810ece08a4d3a6202cb9351848ef916662c583562d206a9931de7' THEN
  RAISE EXCEPTION 'Sep20 rerun338 scoring definition differs';
 END IF;
 IF pg_catalog.encode(extensions.digest(pg_catalog.pg_get_functiondef(
   'public.lab_arena_open_scoring_v2(text,smallint,jsonb)'::pg_catalog.regprocedure),
   'sha256'),'hex')<>'7bd21582d2d810ece08a4d3a6202cb9351848ef916662c583562d206a9931de7' THEN
  RAISE EXCEPTION 'Sep20 rerun338 scoring patch differs';
 END IF;
END $patch_sep20_rerun338_scoring$;
...
BEGIN
 IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_trigger trigger
  ...
  RAISE EXCEPTION 'Sep20 rerun338 publication stop differs' USING ERRCODE='55000';
 END IF;
...
BEGIN
 IF EXISTS(SELECT 1 FROM pg_catalog.pg_trigger
  ...
  RAISE EXCEPTION 'Sep20 rerun338 USER trigger state differs'
   USING ERRCODE='55000';
 END IF;
...
BEGIN
 PERFORM pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('arena-2026-09-20-rerun338',0));
  ...
  IF NOT EXISTS(SELECT 1 FROM public.lab_arena_rounds archived
      WHERE archived.round_id='arena-2026-09-20-r338archive'
       ...
       AND archived.configuration_doc->>'archived_terminal_round_sha256'=
           '0cadec139ddb5263b937a7a732ddfd741ffc9769f0fe4b37a4b04ddfc8abf68b'
       ...
       AND archived.configuration_doc->>'archived_terminal_runs_sha256'=
           'ffbacbf8438437f215a81094c8383e53adb5a3016bbf0aee6b860802843fa145'
       AND archived.configuration_doc->>'archived_terminal_ledger_sha256'=
           'bdf0a0bba94007a40b67f2c2b4be2e7d247970fa97bede97b66d9b82feb7dba0'
       ...
       AND archived.configuration_doc->>'archived_terminal_judgment_cache_sha256'=
           '0b4e199bb22143a1c0109037be162a0138bb905a7c4c949a898144ac3bb173bb'
    ...
    OR active.configuration_doc->'schedule' IS DISTINCT FROM
       '{"benchmark_deadline":"2026-09-20T14:30:00Z","final_scoring_close":"2026-09-21T04:30:00Z","publication_deadline":"2026-09-21T04:30:01Z","stage_1_close":"2026-09-20T21:30:00Z","stage_1_scoring_close":"2026-09-21T00:30:00Z","stage_1_start":"2026-09-20T14:30:01Z","stage_2_close":"2026-09-21T01:30:00Z","stage_2_start":"2026-09-21T00:30:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB
    ...
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_submissions
        WHERE round_id='arena-2026-09-20-r338archive')<>
       8
    OR EXISTS(SELECT 1 FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r338archive' AND kind='execute')
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r338archive' AND kind='score')<>
       99
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r338archive' AND kind='score'
         AND status='accepted' AND terminal_cause='accepted'
         AND assignment_id LIKE '%:score:rerun337')<>
       98
    OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
        WHERE round_id='arena-2026-09-20-r338archive' AND kind='score'
         AND status='failed' AND terminal_cause='judge_error'
         AND submission_id='baseline-2026-09-20:r338archive'
         AND icp_position=8 AND attempt=1
         AND assignment_id LIKE '%:score:rerun337')<>
       1
   ...
   RAISE EXCEPTION 'existing Sep20 rerun338 differs' USING ERRCODE='55000';
  END IF;
  RETURN;
 END IF;

 IF ('{"benchmark_deadline":"2026-09-20T14:30:00Z","final_scoring_close":"2026-09-21T04:30:00Z","publication_deadline":"2026-09-21T04:30:01Z","stage_1_close":"2026-09-20T21:30:00Z","stage_1_scoring_close":"2026-09-21T00:30:00Z","stage_1_start":"2026-09-20T14:30:01Z","stage_2_close":"2026-09-21T01:30:00Z","stage_2_start":"2026-09-21T00:30:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB->>'submission_open') IS DISTINCT FROM
      '2026-09-19T00:00:00Z'
 OR ('{"benchmark_deadline":"2026-09-20T14:30:00Z","final_scoring_close":"2026-09-21T04:30:00Z","publication_deadline":"2026-09-21T04:30:01Z","stage_1_close":"2026-09-20T21:30:00Z","stage_1_scoring_close":"2026-09-21T00:30:00Z","stage_1_start":"2026-09-20T14:30:01Z","stage_2_close":"2026-09-21T01:30:00Z","stage_2_start":"2026-09-21T00:30:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB->>'submission_cutoff') IS DISTINCT FROM
  ...
  RAISE EXCEPTION 'Sep20 rerun338 schedule differs' USING ERRCODE='22023';
 END IF;

 IF 'sha256:acd64107ee3ed6d52d81708bf7cc951042eb0f02b6151d1197cd29fd6365dcfb' !~ '^sha256:[0-9a-f]{64}$'
  OR '493765492819.dkr.ecr.us-east-1.amazonaws.com/leadpoet/sourcing-model@sha256:acd64107ee3ed6d52d81708bf7cc951042eb0f02b6151d1197cd29fd6365dcfb' IS DISTINCT FROM
    '493765492819.dkr.ecr.us-east-1.amazonaws.com/leadpoet/sourcing-model@'||
    'sha256:acd64107ee3ed6d52d81708bf7cc951042eb0f02b6151d1197cd29fd6365dcfb' THEN
  RAISE EXCEPTION 'Sep20 rerun338 scorer identity differs' USING ERRCODE='22023';
 END IF;
 ...
 IF active.status IS DISTINCT FROM 'scored'
  OR active.status IS DISTINCT FROM 'scored'
  ...
  OR pg_catalog.encode(extensions.digest(pg_catalog.to_jsonb(active)::TEXT,
       'sha256'),'hex')<>'0cadec139ddb5263b937a7a732ddfd741ffc9769f0fe4b37a4b04ddfc8abf68b'
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20')<>203
  ...
  RAISE EXCEPTION 'Sep20 rerun338 terminal preimage differs' USING ERRCODE='55000';
 END IF;
 ...
 IF pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_ledger
      WHERE round_id='arena-2026-09-20')<>41041
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
  ...
  OR pg_catalog.encode(extensions.digest(COALESCE((SELECT pg_catalog.string_agg(
    ...
    'sha256'),'hex')<>'0b4e199bb22143a1c0109037be162a0138bb905a7c4c949a898144ac3bb173bb'
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_judgment_cache)<>
       1031 THEN
  RAISE EXCEPTION 'Sep20 rerun338 terminal history differs' USING ERRCODE='55000';
 END IF;
 ...
 SELECT pg_catalog.jsonb_agg(item||pg_catalog.jsonb_build_object(
   'submission_id',(item->>'submission_id')||':r338archive') ORDER BY ordinal)
 ...
 archive_config:=active.configuration_doc||pg_catalog.jsonb_build_object(
  'round_id','arena-2026-09-20-r338archive','mode','shadow','rewards_enabled',FALSE,
  'archived_terminal_status',active.status,
  'archived_terminal_cancel_reason',active.cancel_reason,
  'archived_terminal_round_sha256','0cadec139ddb5263b937a7a732ddfd741ffc9769f0fe4b37a4b04ddfc8abf68b',
  ...
  'archived_terminal_runs_sha256','ffbacbf8438437f215a81094c8383e53adb5a3016bbf0aee6b860802843fa145',
  'archived_terminal_ledger_sha256','bdf0a0bba94007a40b67f2c2b4be2e7d247970fa97bede97b66d9b82feb7dba0',
  'archived_terminal_company_judgments_sha256',
   '0ef317eb17e2318bae2ef9d4bc8de770cc8cbbae941ab9456941b2c44b9c5c41',
  'archived_terminal_judgment_cache_sha256','0b4e199bb22143a1c0109037be162a0138bb905a7c4c949a898144ac3bb173bb',
 ...
 SELECT round_id,status,status_generation,stage_generation,configuration_doc,
  ...
  champion_fallback_providers FROM pg_catalog.jsonb_populate_record(
   ...
   pg_catalog.jsonb_build_object(
    'round_id','arena-2026-09-20-r338archive','status','cancelled',
 ...
 INSERT INTO public.lab_arena_submissions SELECT (pg_catalog.jsonb_populate_record(
  ...
  pg_catalog.jsonb_build_object('round_id','arena-2026-09-20-r338archive',
   'submission_id',s.submission_id||':r338archive'))).*
 FROM public.lab_arena_submissions s WHERE s.round_id='arena-2026-09-20';

 UPDATE public.lab_arena_runs SET round_id='arena-2026-09-20-r338archive',
  submission_id=submission_id||':r338archive'
 WHERE round_id='arena-2026-09-20' AND kind='score';
 GET DIAGNOSTICS moved_score_runs=ROW_COUNT;
 UPDATE public.lab_arena_ledger l SET round_id='arena-2026-09-20-r338archive',
  submission_id=submission_id||':r338archive'
 WHERE round_id='arena-2026-09-20' AND EXISTS(SELECT 1
  FROM public.lab_arena_runs s WHERE s.round_id='arena-2026-09-20-r338archive'
 ...
 UPDATE public.lab_arena_rounds SET status='stage1',
  ...
  configuration_doc=configuration_doc||pg_catalog.jsonb_build_object(
   'schedule','{"benchmark_deadline":"2026-09-20T14:30:00Z","final_scoring_close":"2026-09-21T04:30:00Z","publication_deadline":"2026-09-21T04:30:01Z","stage_1_close":"2026-09-20T21:30:00Z","stage_1_scoring_close":"2026-09-21T00:30:00Z","stage_1_start":"2026-09-20T14:30:01Z","stage_2_close":"2026-09-21T01:30:00Z","stage_2_start":"2026-09-21T00:30:01Z","submission_cutoff":"2026-09-20T00:00:00Z","submission_open":"2026-09-19T00:00:00Z"}'::JSONB,
   'scorer_image_digest',
    'sha256:acd64107ee3ed6d52d81708bf7cc951042eb0f02b6151d1197cd29fd6365dcfb',
 ...
 WHERE round_id='arena-2026-09-20';
 IF NOT FOUND THEN RAISE EXCEPTION 'Sep20 rerun338 round update missing'; END IF;
 ...
 WHERE round_id='arena-2026-09-20';
 IF moved_score_runs<>99
  OR moved_score_ledger<>2136
  ...
  OR (SELECT pg_catalog.count(*) FROM public.lab_arena_runs
      WHERE round_id='arena-2026-09-20' AND kind='execute'
       AND status='accepted' AND terminal_cause='accepted')<>
     98 THEN
  RAISE EXCEPTION 'Sep20 rerun338 preservation differs' USING ERRCODE='55000';
 END IF;
END $sep20_rerun338$;
COMMIT;
