-- Retire contacts only for the authorized, unstarted September 24 round.
-- Company criteria, generated ICP bank, costs, schedules and historical results
-- stay unchanged. Future rounds use the company's existing integrity adapter.
BEGIN;
...
BEGIN
  ...
  WHERE round_id = 'arena-2026-09-24' FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'September 24 round is missing' USING ERRCODE = 'P0002';
  END IF;
  IF v_before.configuration_doc ->> 'schema_version'
     ...
     OR v_before.configuration_doc ->> 'round_id'
       IS DISTINCT FROM 'arena-2026-09-24'
     ...
     OR v_before.configuration_doc #>> '{schedule,submission_cutoff}'
       IS DISTINCT FROM '2026-09-24T00:00:00Z' THEN
    RAISE EXCEPTION 'September 24 configuration identity differs';
  END IF;

  -- Once converted, replay never changes a started or published round.
  IF NOT (v_before.configuration_doc ? 'contact_policy')
     AND v_before.configuration_doc #>> '{scorer_policy,scoring_adapter_version}'
       = 'qualification_integrity_v2' THEN
    RETURN;
  END IF;
  IF v_before.configuration_doc ->> 'contact_policy' IS DISTINCT FROM 'contacts_v1'
     OR v_before.configuration_doc #>> '{scorer_policy,scoring_adapter_version}'
       IS DISTINCT FROM 'qualification_contacts_v3'
     OR v_before.status IS DISTINCT FROM 'open'
     ...
     OR EXISTS (SELECT 1 FROM public.lab_arena_submissions
                WHERE round_id = 'arena-2026-09-24'
                  AND (status = 'frozen' OR frozen_at IS NOT NULL)) THEN
    RAISE EXCEPTION 'September 24 requires an open, unfrozen, unstarted round';
  END IF;

  v_expected := pg_catalog.jsonb_set(
    v_before.configuration_doc - 'contact_policy',
  ...
  WHERE round_id = 'arena-2026-09-24';
  IF v_count <> 1 OR v_after.configuration_doc IS DISTINCT FROM v_expected
    ...
    RAISE EXCEPTION 'Company-only conversion changed protected round state';
  END IF;
END;
$sep24_company_only$;
COMMIT;
