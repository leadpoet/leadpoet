-- Enable authenticated provider-observation handoff only for the exact,
-- uncommitted September 27 company-only intent-details round. The active
-- September 26 round and all other frozen round policies stay unchanged.
BEGIN;
...
DO $sep27_provider_observation_handoff$
DECLARE
...
BEGIN
  ...
  WHERE round_id = 'arena-2026-09-27' FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'September 27 round is missing' USING ERRCODE = 'P0002';
  END IF;
  ...
  IF v_before.configuration_doc ->> 'schema_version'
     ...
     OR v_before.configuration_doc ->> 'round_id'
       IS DISTINCT FROM 'arena-2026-09-27'
     ...
     OR v_before.configuration_doc #>> '{schedule,submission_cutoff}'
       IS DISTINCT FROM '2026-09-27T00:00:00Z' THEN
    RAISE EXCEPTION 'September 27 configuration identity differs';
  END IF;

  -- A replay after successful conversion is a strict no-op, including after
  -- normal round progress. A different binding value is never overwritten.
  IF v_before.configuration_doc
       #>> '{scorer_policy,env_bindings,ARENA_PROVIDER_OBSERVATION_HANDOFF}'
       = 'authenticated_first_observed_v1' THEN
    RETURN;
  END IF;
  IF (v_before.configuration_doc #> '{scorer_policy,env_bindings}')
       ? 'ARENA_PROVIDER_OBSERVATION_HANDOFF' THEN
    RAISE EXCEPTION 'September 27 provider observation binding differs';
  END IF;
  ...
  IF v_before.status IS DISTINCT FROM 'open'
     ...
     OR EXISTS (
       ...
       WHERE round_id = 'arena-2026-09-27'
     )
     OR EXISTS (
    ...
    RAISE EXCEPTION 'September 27 requires an open, unfrozen, uncommitted round';
  END IF;

  v_expected := pg_catalog.jsonb_set(
    v_before.configuration_doc,
    '{scorer_policy,env_bindings,ARENA_PROVIDER_OBSERVATION_HANDOFF}',
    '"authenticated_first_observed_v1"'::JSONB,
    TRUE
  );
  ...
  IF v_count <> 1
    ...
    RAISE EXCEPTION 'Provider observation handoff changed protected round state';
  END IF;
END;
$sep27_provider_observation_handoff$;
COMMIT;
