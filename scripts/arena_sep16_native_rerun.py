...
RECOVERY275_ARCHIVE_ROUND = "arena-2026-09-16-rerun273archive"
RECOVERY277_ARCHIVE_ROUND = "arena-2026-09-16-rerun275archive"
...
RECOVERY275_SUFFIX = ":rerun275"
RECOVERY277_SUFFIX = ":rerun277"
...
def _recover275(service: Any, args: argparse.Namespace) -> dict[str, Any]:
    row = service.store.get_round(ROUND)
    if row is None or row.get("round_id") != ROUND or row.get("reward_basis_hash") != BASIS_HASH:
        raise ExactRerunRefused("sealed Sep16 round state differs")
    if row.get("status") != "cancelled":
        raise ExactRerunRefused("Sep16 recovery275 round status differs")
    bank_hash = _bank_proof(service, row)
    terminal_size, terminal_hash, terminal_commit = _terminal_recovery_source_proof(
        service, RECOVERY273_SOURCE_REF
    )
    source, source_hash, commit = _source_proof(service, args.expected_lab_commit)
    schedule = _schedule_proof(row, args.forward_schedule_file)
    if args.dry_run:
        return {
            "status": "recovery275_preflight_ok", "round_id": ROUND,
            "bank_sha256": bank_hash, "source_sha256": source_hash,
            "source_commit": commit, "source_size_bytes": len(source),
            "source_ref": RECOVERY275_SOURCE_REF,
            "terminal_source_sha256": terminal_hash,
            "terminal_source_commit": terminal_commit,
            "terminal_source_size_bytes": terminal_size,
            "execute_namespace": RECOVERY275_SUFFIX.removeprefix(":"),
            "verified_parallel_runner_slots": VERIFIED_RUNNER_SLOTS,
        }
    try:
        existing = _read_bounded(
            service.config.object_store, RECOVERY275_SOURCE_REF, len(source) + 1
        )
    except Exception:
        service.config.object_store.put(RECOVERY275_SOURCE_REF, source)
    else:
        if existing != source:
            raise ExactRerunRefused("Sep16 recovery275 source object holds different bytes")
    if _read_bounded(
        service.config.object_store, RECOVERY275_SOURCE_REF, len(source) + 1
    ) != source:
        raise ExactRerunRefused("Sep16 recovery275 source readback differs")
    result = service.store._transport.rpc(
    ...
    if not isinstance(result, dict) or result.get("status") not in ("prepared", "existing"):
        raise ExactRerunRefused("Sep16 recovery275 RPC did not acknowledge the sealed rerun")
    return dict(result)


def _recover277(service: Any, args: argparse.Namespace) -> dict[str, Any]:
    row = service.store.get_round(ROUND)
    if row is None or row.get("round_id") != ROUND or row.get("reward_basis_hash") != BASIS_HASH:
        raise ExactRerunRefused("sealed Sep16 round state differs")
    if row.get("status") != "cancelled":
        raise ExactRerunRefused("Sep16 recovery277 round status differs")
    bank_hash = _bank_proof(service, row)
    terminal_size, terminal_hash, terminal_commit = _terminal_recovery_source_proof(
        service, RECOVERY275_SOURCE_REF
    )
    source, source_hash, commit = _source_proof(service, args.expected_lab_commit)
    schedule = _schedule_proof(row, args.forward_schedule_file)
    if args.dry_run:
        return {
            "status": "recovery277_preflight_ok", "round_id": ROUND,
            "bank_sha256": bank_hash, "source_sha256": source_hash,
            "source_commit": commit, "source_size_bytes": len(source),
            "source_ref": RECOVERY277_SOURCE_REF,
            "terminal_source_sha256": terminal_hash,
            "terminal_source_commit": terminal_commit,
            "terminal_source_size_bytes": terminal_size,
            "execute_namespace": RECOVERY277_SUFFIX.removeprefix(":"),
            "verified_parallel_runner_slots": VERIFIED_RUNNER_SLOTS,
            "openrouter_calls_per_icp": 200,
        }
    try:
        existing = _read_bounded(
            service.config.object_store, RECOVERY277_SOURCE_REF, len(source) + 1
        )
    except Exception:
        service.config.object_store.put(RECOVERY277_SOURCE_REF, source)
    else:
        if existing != source:
            raise ExactRerunRefused("Sep16 recovery277 source object holds different bytes")
    if _read_bounded(
        service.config.object_store, RECOVERY277_SOURCE_REF, len(source) + 1
    ) != source:
        raise ExactRerunRefused("Sep16 recovery277 source readback differs")
    result = service.store._transport.rpc(
    ...
    if not isinstance(result, dict) or result.get("status") not in ("prepared", "existing"):
        raise ExactRerunRefused("Sep16 recovery277 RPC did not acknowledge the sealed rerun")
    return dict(result)
...
def _audit(service: Any) -> dict[str, Any]:
    ...
    recovery275_archived = service.store.list_runs(RECOVERY275_ARCHIVE_ROUND)
    recovery277_archived = service.store.list_runs(RECOVERY277_ARCHIVE_ROUND)
    rerun265_execute = {
    ...
    rerun275_execute = {
        run.get("assignment_id") for run in runs
        if run.get("submission_id") == BASELINE and run.get("kind") == "execute"
        and str(run.get("assignment_id") or "").endswith(RECOVERY275_SUFFIX)
    }
    rerun277_execute = {
        run.get("assignment_id") for run in runs
        if run.get("submission_id") == BASELINE and run.get("kind") == "execute"
        and str(run.get("assignment_id") or "").endswith(RECOVERY277_SUFFIX)
    }
    active_namespace = (
        "rerun277" if rerun277_execute
        else "rerun275" if rerun275_execute
        else "rerun273" if rerun273_execute
    ...
    return {
        ...
        "new_baseline_source_selected": bool(
            ...
            == (
                RECOVERY277_SOURCE_REF if active_namespace == "rerun277"
                else RECOVERY275_SOURCE_REF if active_namespace == "rerun275"
                else RECOVERY273_SOURCE_REF if active_namespace == "rerun273"
        ...
        "archived_failed_recovery273_runs": len(recovery275_archived),
        "archived_failed_recovery275_runs": len(recovery277_archived),
        ...
        "rerun275_execute_assignments": len(rerun275_execute),
        "rerun277_execute_assignments": len(rerun277_execute),
        "rerun_execute_assignments": len(
            rerun277_execute if active_namespace == "rerun277"
            else rerun275_execute if active_namespace == "rerun275"
            else rerun273_execute if active_namespace == "rerun273"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    recovery275 = commands.add_parser("recover275")
    recovery275.add_argument("--expected-lab-commit", required=True)
    recovery275.add_argument("--forward-schedule-file", type=Path, required=True)
    recovery275.add_argument("--dry-run", action="store_true")
    recovery277 = commands.add_parser("recover277")
    recovery277.add_argument("--expected-lab-commit", required=True)
    recovery277.add_argument("--forward-schedule-file", type=Path, required=True)
    recovery277.add_argument("--dry-run", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.command == "recover275":
            result = _recover275(service, args)
        elif args.command == "recover277":
            result = _recover277(service, args)
