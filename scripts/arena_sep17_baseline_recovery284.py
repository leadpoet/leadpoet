#!/usr/bin/env python3
"""Prepare sealed September 17 recovery284; the normal Arena driver runs it."""
...
SOURCE_REF = f"arena/{ROUND}/sources/{BASELINE}-recovery284.tar.gz"
SOURCE_COMMIT = "91e8b95637c4cbc919f3150d6643985624dbb34a"
SOURCE_SHA256 = "b5ca95d7c9c25650c5ecb04b05bde859cffa3fe9c6c7a6602e6a006d5e3dcd99"
SOURCE_SIZE = int("582191")
TERMINAL_SOURCE_REF = f"arena/{ROUND}/sources/{BASELINE}-recovery283.tar.gz"
TERMINAL_SOURCE_COMMIT = "e40484afde214a417128d1eadacaff716755784f"
TERMINAL_SOURCE_SHA256 = "7df255f5db5267b13a65c9c3476cb47fd84837e00ce503bd1f588585ee8a37ae"
TERMINAL_SOURCE_SIZE = 581173
FORWARD_SCHEDULE = {
...
def prepare(service: Any, *, dry_run: bool, now: datetime | None = None) -> dict[str, Any]:
    ...
    # Recovery284 reads recovery283 as immutable history and never rewrites it.
    ...
    result = {
        "status": "recovery284_preflight_ok", "round_id": ROUND,
        ...
        "source_size_bytes": SOURCE_SIZE, "execute_namespace": "rerun284",
    ...
    acknowledged = service.store._transport.rpc(
        "lab_arena_prepare_sep17_baseline_recovery284_v1",
        {
    ...
    if not isinstance(acknowledged, dict) or acknowledged.get("status") not in (
        ...
        or acknowledged.get("execute_namespace") != "rerun284"
    ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Sep17 recovery284 refused: {exc}", file=sys.stderr)
        return 2
    except Exception:
        print("Sep17 recovery284 failed; inspect redacted service logs", file=sys.stderr)
        return 3
