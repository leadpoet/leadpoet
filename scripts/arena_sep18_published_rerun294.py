#!/usr/bin/env python3
"""Seal and prepare the one-time Sep18 published-baseline rerun294."""
...
SOURCE_REF = f"arena/{ROUND}/sources/{BASELINE}-rerun294.tar.gz"
...
PREFLIGHT_SCHEMA = "leadpoet.sep18_published_rerun294.preflight.v1"
READ_PAGE_SIZE = 500
...
def prepare(
    ...
    *,
    ...
    if not replay:
        ...
        if now >= deadline:
            raise RerunRefused("Sep18 rerun294 admission window has closed")
    ...
    result = {
        "status": "rerun294_preflight_ok", "round_id": ROUND,
        "bank_sha256": BANK_SHA256, **facts,
        "execute_namespace": "rerun294", "score_namespace": "score:rerun294",
    ...
    acknowledged = service.store._transport.rpc(
        "lab_arena_prepare_sep18_published_rerun294_v1",
        {
    ...
    if not isinstance(acknowledged, dict) or acknowledged.get("status") not in ("prepared", "existing") or (
        ...
        or acknowledged.get("execute_namespace") != "rerun294"
        or acknowledged.get("score_namespace") != "score:rerun294"
        or any(acknowledged.get(key) != facts[key] for key in (
            "source_size_bytes", "source_sha256", "source_commit"
        ))
    ):
        raise RerunRefused("rerun294 RPC did not acknowledge the exact prepared cycle")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Sep18 rerun294 refused: {exc}", file=sys.stderr)
        return 2
    except Exception:
        print("Sep18 rerun294 failed; inspect redacted service logs", file=sys.stderr)
        return 3
