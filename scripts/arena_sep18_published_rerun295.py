#!/usr/bin/env python3
"""Seal and prepare the one-time Sep18 published-baseline rerun295."""
...
SOURCE_REF = f"arena/{ROUND}/sources/{BASELINE}-rerun295.tar.gz"
...
PREFLIGHT_SCHEMA = "leadpoet.sep18_published_rerun295.preflight.v1"
READ_PAGE_SIZE = 500
...
def prepare(
    ...
    *,
    ...
    if not replay:
        ...
        if now >= deadline:
            raise RerunRefused("Sep18 rerun295 admission window has closed")
    ...
    result = {
        "status": "rerun295_preflight_ok", "round_id": ROUND,
    ...
    if not isinstance(acknowledged, dict) or acknowledged.get("status") not in ("prepared", "existing") or (
        ...
        or acknowledged.get("execute_namespace") != "rerun295"
        or acknowledged.get("score_namespace") != "score:rerun295"
        or any(acknowledged.get(key) != facts[key] for key in (
            "source_size_bytes", "source_sha256", "source_commit"
        ))
    ):
        raise RerunRefused("rerun295 RPC did not acknowledge the exact prepared cycle")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Sep18 rerun295 refused: {exc}", file=sys.stderr)
        return 2
    except Exception:
        print("Sep18 rerun295 failed; inspect redacted service logs", file=sys.stderr)
        return 3
