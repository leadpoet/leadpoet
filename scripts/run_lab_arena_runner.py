...
A runner claims one ICP assignment per free local slot
(``LAB_ARENA_MAX_PARALLEL_RUNS``, default 1), executes the frozen miner image
in a fresh gVisor sandbox, and submits runner-signed receipts. Startup fails
unless the host is Linux x86_64 with the exact pinned runsc binary.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--round-id", required=False, default=os.environ.get("LAB_ARENA_ROUND_ID", ""))
...
def main(argv=None) -> int:
    ...
    if not args.api_base_url or not args.round_id:
        print("--api-base-url and --round-id (or LAB_ARENA_API_BASE_URL / LAB_ARENA_ROUND_ID) are required", file=sys.stderr)
        return 2
