...
def test_preflight(errors):
    ...
    with patched(preflight_openrouter_key=pf_depleted):
        try:
            asyncio.run(w._preflight_openrouter_credit(ctx, {"OPENROUTER_API_KEY": "test-openrouter-key"}))
    ...
    with patched(preflight_openrouter_key=pf_funded):
        asyncio.run(w._preflight_openrouter_credit(ctx, {"OPENROUTER_API_KEY": "test-openrouter-key"}))  # no raise
