...
def main() -> int:
    ...
    # --- credit-block ---
    check(errors, "paused+blocked_for_credit", "blocked_for_credit",
    ...
    sql_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                            "scripts", "57-research-lab-public-loop-completed-no-candidate-label.sql")
    try:
        sql = open(sql_path).read()
        for label in ("blocked_for_credit", "waiting_for_baseline", "needs_rescore", "completed_no_candidate"):
        ...
        errors.append("[migration] scripts/57 not found")
    ...
    print("PASS — public status derivation (completed-label matrix: waiting_for_baseline / scoring / "
          "needs_rescore / completed_no_candidate; blocked_for_credit; running/queued regressions; "
