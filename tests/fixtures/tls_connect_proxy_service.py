#!/usr/bin/env python3
"""Local authenticated TLS CONNECT boundary for security tests."""
...
STATE_ROOT = Path(os.environ.get("TEST_TLS_CONNECT_STATE_ROOT", "/tmp/tls-connect-test"))
...
HOST = "127.0.0.1"
PORT = int(os.environ.get("TEST_TLS_CONNECT_PROXY_PORT", "18443"))
PROXY_HOSTS = (
...
EXPECTED_CREDENTIALS = {
    "test-auto": "test-auto-password",
    "test-scoring": "test-scoring-password",
}
ALLOWED_TARGETS = {
    "openrouter.ai:443",
    "api.exa.ai:443",
...
def _write_event(**details: object) -> None:
    row = {
        ...
        "fixture_authenticity": "local_security_test",
        "reject_unknown": True,
        **details,
    }
    with _EVENT_LOCK:
...
def _write_certificates() -> None:
    ...
    ca_name = x509.Name(
        [x509.NameAttribute(NameOID.COMMON_NAME, "Leadpoet TLS CONNECT Test CA")]
    )
    ca_cert = (
