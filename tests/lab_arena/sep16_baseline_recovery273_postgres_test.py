"""PostgreSQL proof for the exact Sep16 recovery273 template."""
...
from tests.lab_arena.sep16_baseline_recovery272_postgres_test import (
    ...
    _install_and_recover272 as _install_prior_recovery272,
    _render_recovery272 as _render_prior_recovery272,
)
...
ARCHIVE_ROUND = "arena-2026-09-16-rerun272archive"
ARCHIVE_SUBMISSION = "baseline-2026-09-16-native-rerun272-archive"
RECOVERY_SOURCE_REF = (
    "arena/arena-2026-09-16/sources/"
    "baseline-2026-09-16-recovery273.tar.gz"
)
...
def _prepare_terminal_recovery272(connection, objects=None):
...
def _render_recovery273(connection, new_schedule):
    ...
    values = {
        ...
        "__SEALED_PRIOR_RECOVERY_AUTHORITY_HASH__": _single_hash(
            connection, "lab_arena_sep16_baseline_recovery272_authority", f"round_id='{ROUND}'"
        ),
        "__SEALED_PRIOR_RECOVERY_AUDIT_HASH__": _single_hash(
            connection, "lab_arena_sep16_baseline_recovery272_audit", f"round_id='{ROUND}'"
        ),
...
def _install_and_recover273(connection, sql, schedule):
    with connection.cursor() as cursor:
        cursor.execute(sql)
        cursor.execute(sql)
        arguments = (
        ...
        cursor.execute(
            "SELECT public.lab_arena_prepare_sep16_baseline_recovery273_v1("
            "%s,%s,%s,%s::jsonb)",
            arguments,
        )
...
def _accept_recovery273_scores(connection, objects, stage, icps):
    with connection.cursor() as cursor:
        cursor.execute("ALTER TABLE public.lab_arena_runs DISABLE TRIGGER USER")
        cursor.execute(
            ...
            "AND assignment_id LIKE '%%:score:rerun273' ORDER BY icp_position",
...
def test_recovery273_preserves_history_and_publishes_positive(connect, tmp_path, monkeypatch):
    connection = connect()
    harness = Harness(connect, tmp_path, challengers=[], runners=["recovery273-proof"])
    ...
    try:
        ...
        protected = {
            ...
            "recovery272_authority": _single_hash(
                connection, "lab_arena_sep16_baseline_recovery272_authority", f"round_id='{ROUND}'"
            ),
            "recovery272_audit": _single_hash(
                connection, "lab_arena_sep16_baseline_recovery272_audit", f"round_id='{ROUND}'"
            ),
        ...
        with connection.cursor() as cursor:
            ...
            assert {
                name: definition.count(":rerun273")
                for name, definition in definitions.items()
            } == expected_namespace_counts
            assert all(":rerun272" not in definition for definition in definitions.values())
            publication = next(
                definition for definition in definitions.values()
                if "lab_arena_sep16_recovery273_archive_valid_v1" in definition
            )
            archive_clause = (
        ...
        assert _single_hash(
            connection, "lab_arena_sep16_baseline_recovery272_authority", f"round_id='{ROUND}'"
        ) == protected["recovery272_authority"]
        assert _single_hash(
            connection, "lab_arena_sep16_baseline_recovery272_audit", f"round_id='{ROUND}'"
        ) == protected["recovery272_audit"]
        ...
        for stage in (1, 2):
            ...
            connection.commit()
            _accept_recovery273_scores(connection, objects, stage, icps)
            connection.commit()
...
def test_recovery273_refuses_partial_function_rewrite(connect, drift):
    connection = connect()
    try:
        schedule272, _hotkeys, _ids = _prepare_terminal_recovery272(connection)
        schedule273 = _shift_schedule(schedule272, minutes=90)
        migration273 = _render_recovery273(connection, schedule273)
        with connection.cursor() as cursor:
            cursor.execute(
            ...
            if drift == "namespace":
                changed = definition.replace(":rerun272", ":rerun273", 1)
                ...
                old = "OR NOT public.lab_arena_sep16_recovery272_archive_valid_v1()"
        ...
        with connection.cursor() as cursor, pytest.raises(
            ...
            match=(
                ...
                "recovery273 guard shape differs"
            ),
        ):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT to_regclass('public.lab_arena_sep16_baseline_recovery273_audit')"
            )
...
def test_recovery273_seal_fails_closed_on_terminal_drift(connect, drift):
    connection = connect()
    try:
        schedule272, _hotkeys, _ids = _prepare_terminal_recovery272(connection)
        if drift == "active":
...
def test_recovery273_uses_normal_two_attempt_limit(connect, tmp_path):
    connection = connect()
    harness = Harness(connect, tmp_path, challengers=[], runners=["retry273-proof"])
    store = harness.service.store
    try:
        schedule272, hotkeys, _ids = _prepare_terminal_recovery272(connection)
        schedule273 = _shift_schedule(schedule272, minutes=90)
        _install_and_recover273(
            connection, _render_recovery273(connection, schedule273), schedule273
        )
        ...
        assert first["assignment_id"].endswith(":rerun273")
