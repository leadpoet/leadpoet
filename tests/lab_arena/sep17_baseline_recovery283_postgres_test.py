"""Exact recovery282 terminal fixture; sealed same-round recovery283."""
...
from scripts import arena_sep17_baseline_recovery283 as operator
...
from tests.lab_arena.sep17_baseline_recovery282_postgres_test import (
    restore_current as restore_recovery282_terminal,
)


ROOT = Path(__file__).parents[2]
PRIVATE = Path("/private/tmp/leadpoet-tyche-rerun-evidence-20260915/private")
SNAPSHOT = PRIVATE / "sep17-recovery283-terminal-snapshot-20260917T101259.899310Z.json"
SNAPSHOT_SHA = "a4fa76360a3442736d0277ac6eb5e4bfe578d2130b485f7e3e8a7fb2a1d083b1"
BANK_SNAPSHOT = PRIVATE / "sep17-original-bank.json"
MIGRATION_282 = ROOT / "scripts/282-arena-2026-09-17-baseline-recovery.sql"
MIGRATION = ROOT / "scripts/283-arena-2026-09-17-baseline-recovery.sql"
TEMPLATE = Path(str(MIGRATION) + ".template")
ROUND = operator.ROUND
BASELINE = operator.BASELINE
ARCHIVE = "arena-2026-09-17-rerun282archive"
ARCHIVE_BASELINE = "baseline-2026-09-17-rerun282archive"
pytestmark = pytest.mark.skipif(
    not all(path.is_file() for path in (SNAPSHOT, BANK_SNAPSHOT)),
    reason=(
        "Recovery283 requires the protected exact recovery282 terminal snapshot "
...
def captured():
    ...
    assert receipt["schema_version"] == "leadpoet.sep17_recovery283_terminal_snapshot.v1"
...
def replacements(snapshot):
    ...
    return {
        ...
        "__PRIOR282_AUTHORITY_HASH__": prior["authority_hash"],
        "__PRIOR282_AUDIT_HASH__": prior["audit_hash"],
    }
...
def restore_terminal(connection):
    snapshot = captured()
    restore_recovery282_terminal(connection)
    with connection.cursor() as cursor:
        cursor.execute(MIGRATION_282.read_text())
    connection.commit()
    rows = snapshot["protected_rows"]
    with connection.cursor() as cursor:
        cursor.execute("SET session_replication_role=replica")
        cursor.execute(
            "TRUNCATE public.lab_arena_sep17_baseline_recovery282_audit,"
...
def prepare(cursor, **overrides):
    ...
    cursor.execute(
        "SELECT public.lab_arena_prepare_sep17_baseline_recovery283_v1("
        "%s,%s,%s,%s,%s::jsonb)",
    ...
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                "'public.lab_arena_prepare_sep17_baseline_recovery283_v1(bigint,text,text,text,jsonb)',"
                "'EXECUTE'),has_function_privilege('service_role',"
                "'public.lab_arena_prepare_sep17_baseline_recovery283_v1(bigint,text,text,text,jsonb)',"
                "'EXECUTE'),has_table_privilege('lab_arena_service',"
                "'public.lab_arena_sep17_baseline_recovery283_authority','SELECT')"
            )
            assert cursor.fetchone() == (True, False, False)
            cursor.execute(
                "SELECT count(*) FROM pg_catalog.pg_trigger WHERE tgname="
                "'lab_arena_sep17_recovery283_archive_guard' AND NOT tgisinternal"
            )
        ...
        with pytest.raises(Exception):
            with connection.cursor() as cursor:
                cursor.execute(
                    "UPDATE public.lab_arena_sep17_baseline_recovery283_authority "
        ...
        for statement in (
            ...
            "UPDATE public.lab_arena_sep17_baseline_recovery282_audit "
        ...
        with pytest.raises(Exception):
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    "RENAME TO recovery283_test_original_cost_state"
                )
                cursor.execute(
                    ...
                    "SELECT public.recovery283_test_original_cost_state("
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT public.lab_arena_sep17_recovery283_prior_valid_v1(),"
        ...
        connection.rollback()
        with connection.cursor() as cursor:
            cursor.execute("SELECT public.lab_arena_sep17_recovery283_archive_valid_v1()")
        ...
        harness = Harness(lambda: psycopg2.connect(**dsn), tmp_path,
                          challengers=[], runners=["recovery283-proof"])
