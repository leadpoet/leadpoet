"""Disposable PostgreSQL proof for the sealed Sep19 rerun315 template."""
...
from tests.lab_arena import (
    sep19_terminal310_fixed_baseline_rerun313_postgres_test as prior,
)
from tests.lab_arena.lab_arena_pg_harness import database_with_lab_arena_migration


ROOT = Path(__file__).parents[2]
TEMPLATE = ROOT / "scripts/315-arena-2026-09-19-terminal313-fixed-baseline-rerun.sql.template"
ROUND = prior.rerun310.ROUND
BASELINE = prior.rerun310.BASELINE
# The reservation fix is SQL-only. Recovery315 reuses the exact immutable
# rerun313 source and scorer while creating a fresh assignment namespace.
...
def _scorer_patch(definition: str) -> str:
    old_guard = _template_block("guard313")
    new_guard = _template_block("new_guard")
    old_assignment = _template_block("assignment313")
...
def _seed_terminal_rerun313_failure(conn) -> None:
    prior._seed_terminal_rerun310(conn)
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            ...
            SET status='failed',
                terminal_cause=CASE WHEN icp_position<15 THEN 'model_error'
                                    ELSE 'provider_error' END,
                runner_hotkey=miner_hotkey,
                result_doc=jsonb_build_object(
                  'terminal_status',CASE WHEN icp_position<15 THEN 'model_error'
                                         ELSE 'provider_error' END)
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun313' AND attempt=1
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            SELECT assignment_id||':2',assignment_id,round_id,submission_id,
                   ...
                   CASE WHEN icp_position<10 THEN 'model_error'
                        ELSE 'provider_error' END,
                   jsonb_build_object(
                     'terminal_status',CASE WHEN icp_position<10 THEN 'model_error'
                                            ELSE 'provider_error' END)
        ...
        cursor.execute(
            """
            SELECT public.lab_arena_sep19_rerun313_active_valid_v1(),
            ...
            GROUP BY public.lab_arena_sep19_rerun313_active_valid_v1()
            """,
            (ROUND, BASELINE),
        )
        assert cursor.fetchone() == (True, 40, 25, 15, 20)
        cursor.execute(
...
def test_terminal313_rerun_executes_scores_and_publishes_with_preservation(
    database, tmp_path, monkeypatch
):
    psycopg2, dsn = database
    conn = psycopg2.connect(**dsn)
    try:
        _seed_terminal_rerun313_failure(conn)
        schedule = _schedule()
        with conn.cursor() as cursor:
            cursor.execute(
            ...
            failed_baseline_before = cursor.fetchone()[0]
            rerun313_history_before = _round_history(cursor, ROUND + "-r313archive")
            cursor.execute(
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun315 scorer definition differs"):
            with conn.cursor() as cursor:
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun315 terminal preimage differs"):
            with conn.cursor() as cursor:
                cursor.execute(attacked)
        conn.rollback()
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "bool_and(assignment_id LIKE '%%:rerun315') "
            ...
            assert "lab_arena_sep19_rerun313_active_valid_v1()" not in definition
            assert ":score:rerun313" not in definition
            assert "lab_arena_sep19_rerun315_active_valid_v1()" in definition
            assert ":score:rerun315" in definition
            cursor.execute(
            ...
            assert cursor.fetchall() == [
                ("lab_arena_sep19_rerun315_score_namespace_guard",)
            ]
            assert _function_seals(cursor) == behavior_seals_before
            assert _round_history(cursor, ROUND + "-r313archive") == rerun313_history_before
            cursor.execute(
                ...
                "WHERE round_id=%s AND kind='execute' AND submission_id=%s",
                (ROUND + "-r315archive", BASELINE + ":r315archive"),
            )
            assert cursor.fetchone() == (40, 40, 25, 15, 20)
            cursor.execute(
                "SELECT jsonb_agg(to_jsonb(r)-'round_id'-'submission_id' ORDER BY run_id) "
                "FROM public.lab_arena_runs r WHERE round_id=%s AND kind='execute' "
                "AND submission_id=%s",
            ...
            assert cursor.fetchone()[0] == failed_baseline_before
            cursor.execute(
                "SELECT count(*) FROM public.lab_arena_ledger WHERE round_id=%s "
                "AND submission_id=%s",
                (ROUND + "-r315archive", BASELINE + ":r315archive"),
        ...
        monkeypatch.setattr(lifecycle, "_proof_execution", prior.rerun310._proof_execution_v2)
        ...
        harness.objects.put(
            f"arena/{ROUND}/benchmark.json", prior.rerun310.BENCHMARK_BYTES
        )
        with conn.cursor() as cursor:
            cursor.execute(
            ...
            for index, (run_id, output_ref, position) in enumerate(cursor.fetchall(), 1):
                prior.rerun310._proof_execution_v2(
                    harness.objects,
                    prior.rerun310.BENCHMARK["icps"][position],
                    index,
                    position,
                    run_id,
                )
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT count(*),count(*) FILTER(WHERE assignment_id LIKE '%%:score:rerun315'),"
        ...
        with conn.cursor() as cursor:
            ...
            archive315_before = _round_history(cursor, ROUND + "-r315archive")
        conn.commit()

        with conn.cursor() as cursor:
            cursor.execute(rendered)
            assert _round_history(cursor, ROUND) == published_before
            assert _round_history(cursor, ROUND + "-r315archive") == archive315_before
            assert _round_history(cursor, ROUND + "-r313archive") == rerun313_history_before
    finally:
        conn.close()


def test_terminal313_preimage_requires_exact_failure_shape_and_active_validity(
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SET session_replication_role=replica")
            cursor.execute(
                "UPDATE public.lab_arena_runs SET terminal_cause='provider_error' "
                "WHERE run_id=(SELECT run_id FROM public.lab_arena_runs "
                "WHERE round_id=%s AND submission_id=%s AND kind='execute' "
                "AND terminal_cause='model_error' ORDER BY run_id LIMIT 1)",
                (ROUND, BASELINE),
            )
            cursor.execute("SET session_replication_role=origin")
        with pytest.raises(psycopg2.Error, match="Sep19 rerun315 terminal preimage differs"):
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SET session_replication_role=origin")
            cursor.execute("SELECT public.lab_arena_sep19_rerun313_active_valid_v1()")
...
def test_template_is_sealed_terminal313_only():
    body = TEMPLATE.read_text()
    assert _test_migrations()[-1] == (
        "314-lab-arena-openrouter-web-search-reservation.sql"
    )
    assert "after the Sep19 rerun313 round is terminal" in body
    assert "assignment_id LIKE '%:rerun313'" in body
    assert "assignment_id NOT LIKE '%:score:rerun313'" in body
    ...
    assert "old_guard TEXT:=$guard313$" in body
    assert "old_assignment TEXT:=$assignment313$" in body
    assert "lab_arena_sep19_rerun313_active_valid_v1()" in body
    assert "lab_arena_sep19_rerun315_active_valid_v1()" in body
    assert "DROP TRIGGER IF EXISTS lab_arena_sep19_rerun313_score_namespace_guard" in body
    assert "active_round.status<>'cancelled'" in body
    assert "execution_incomplete:stage1:5" in body
    assert "terminal_cause='model_error')<>25" in body
    assert "terminal_cause='provider_error')<>15" in body
