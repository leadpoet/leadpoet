"""Disposable PostgreSQL proof for the sealed Sep19 rerun317 template."""
...
from tests.lab_arena import (
    sep19_terminal315_fixed_baseline_rerun316_postgres_test as prior,
)
...
# Synthetic values prove that recovery317 binds a fresh source and scorer.
# Production values remain unfilled placeholders in the SQL template.
NEW_SOURCE_REF = "arena/arena-2026-09-19/sources/baseline-rerun317-test.tar.gz"
NEW_SOURCE_SIZE = 700_317
NEW_SOURCE_SHA = "a" * 64
...
def _scorer_patch(definition: str) -> str:
    old_guard = _template_block("guard316")
    new_guard = _template_block("new_guard")
    old_assignment = _template_block("assignment316")
...
def _seed_terminal_rerun316(conn, *, terminal_status: str = "cancelled") -> None:
    assert terminal_status in {"published", "cancelled"}
    prior._seed_terminal_rerun315(conn)
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            ...
            SET status=CASE WHEN icp_position=0 THEN 'accepted' ELSE 'failed' END,
                ...
                output_ref=CASE WHEN icp_position=0
                  THEN 'arena/output/rerun316-accepted.json' ELSE NULL END
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun316' AND attempt=1
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun316' AND attempt=1 AND icp_position>0
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            SELECT execute.round_id||':'||execute.submission_id||':'||
                   ...
                   'arena/score/rerun316-baseline-accepted.json',execute.run_id,
            ...
            WITH selected AS (
              ...
              SELECT selected.*,
                     'sha256:'||encode(extensions.digest(
                       ('rerun316-paid-'||n::text)::bytea,'sha256'),'hex') AS identity,
        ...
        cursor.execute(
            ...
            SELECT entry_kind,miner_hotkey,round_id,submission_id,run_id,stage,
                   'sha256:'||repeat('9',64),'openrouter','openrouter.responses','host',
        ...
        cursor.execute(
            """
            SELECT public.lab_arena_sep19_rerun316_active_valid_v1(),
            ...
            GROUP BY public.lab_arena_sep19_rerun316_active_valid_v1()
            """,
            (ROUND, BASELINE),
        )
...
def test_terminal316_rerun_executes_scores_and_publishes_with_preservation(
    database, tmp_path, monkeypatch
):
    psycopg2, dsn = database
    conn = psycopg2.connect(**dsn)
    try:
        _seed_terminal_rerun316(conn, terminal_status="published")
        schedule = _schedule()
        with conn.cursor() as cursor:
            cursor.execute(
            ...
            rerun316_history_before = _round_history(cursor, ROUND + "-r316archive")
            cursor.execute(
            ...
            assert terminal_publication_before[2]["fixture"] == "terminal316-published"
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun317 scorer definition differs"):
            with conn.cursor() as cursor:
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun317 terminal preimage differs"):
            with conn.cursor() as cursor:
                cursor.execute(attacked)
        conn.rollback()
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "bool_and(assignment_id LIKE '%%:rerun317') "
            ...
            assert "lab_arena_sep19_rerun316_active_valid_v1()" not in definition
            assert ":score:rerun316" not in definition
            assert "lab_arena_sep19_rerun317_active_valid_v1()" in definition
            assert ":score:rerun317" in definition
            cursor.execute(
            ...
            assert cursor.fetchall() == [
                ("lab_arena_sep19_rerun317_score_namespace_guard",)
            ]
            ...
            cursor.execute(
                ...
                (ROUND + "-r317archive", BASELINE + ":r317archive"),
            )
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT count(*),count(*) FILTER(WHERE assignment_id LIKE '%%:score:rerun317'),"
        ...
        with conn.cursor() as cursor:
            ...
            archive317_before = _round_history(cursor, ROUND + "-r317archive")
        conn.commit()
...
def test_terminal316_preimage_requires_exact_shape_active_validity_and_settlement(
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SET session_replication_role=origin")
            cursor.execute("SELECT public.lab_arena_sep19_rerun316_active_valid_v1()")
...
def test_template_is_sealed_terminal316_only():
    body = TEMPLATE.read_text()
    ...
    assert "assignment_id NOT LIKE '%:score:rerun316'" in body
    ...
    assert "old_guard TEXT:=$guard316$" in body
    assert "old_assignment TEXT:=$assignment316$" in body
    assert "lab_arena_sep19_rerun316_active_valid_v1()" in body
    assert "lab_arena_sep19_rerun317_active_valid_v1()" in body
    assert "DROP TRIGGER IF EXISTS lab_arena_sep19_rerun316_score_namespace_guard" in body
    assert "AS $score_guard317$" in body and "END $score_guard317$" in body
