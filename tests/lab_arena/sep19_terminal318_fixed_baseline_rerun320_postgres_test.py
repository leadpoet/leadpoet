"""Disposable PostgreSQL proof for the sealed Sep19 rerun320 template."""
...
from tests.lab_arena import (
    sep19_terminal317_fixed_baseline_rerun318_postgres_test as prior,
)
...
# Synthetic values prove that recovery320 binds a fresh source and scorer.
# Production values remain unfilled placeholders in the SQL template.
NEW_SOURCE_REF = "arena/arena-2026-09-19/sources/baseline-rerun320-test.tar.gz"
NEW_SOURCE_SIZE = 700_320
NEW_SOURCE_SHA = "a" * 64
NEW_SOURCE_COMMIT = "8996c719c8504ab4a3bac0502ae4d23e63abf6f7"
NEW_SOURCE_BRANCH = "codex/arena-finish-cost-release-20260919"
...
FAILED_RESPONSE_HOLDS = (
    {
        ...
        "generation_id": "gen-rerun318-web-429-a",
    ...
    },
    {
        ...
        "generation_id": "gen-rerun318-web-429-b",
...
class Rerun320Harness(lifecycle.Harness):
    """Keep this recovery's synthetic object bytes isolated from predecessor tests."""

    def objects_key(self) -> str:
        return "sep19-rerun320"
...
def _scorer_patch(definition: str) -> str:
    old_guard = _template_block("guard318")
    new_guard = _template_block("new_guard")
    old_assignment = _template_block("assignment318")
...
def _seed_terminal_rerun318(conn, *, terminal_status: str = "cancelled") -> None:
    assert terminal_status in {"published", "cancelled"}
    prior._seed_terminal_rerun317(conn)
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            ...
            SET status=CASE WHEN icp_position=0 THEN 'accepted' ELSE 'failed' END,
                ...
                output_ref=CASE WHEN icp_position=0
                  THEN 'arena/output/rerun318-accepted.json' ELSE NULL END
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun318' AND attempt=1
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun318' AND attempt=1 AND icp_position>0
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            SELECT execute.round_id||':'||execute.submission_id||':'||
                   ...
                   'arena/score/rerun318-baseline-accepted.json',execute.run_id,
            ...
            WITH selected AS (
              ...
              SELECT selected.*,
                     'sha256:'||encode(extensions.digest(
                       ('rerun318-paid-'||n::text)::bytea,'sha256'),'hex') AS identity,
        ...
        cursor.execute(
            ...
            SELECT entry_kind,miner_hotkey,round_id,submission_id,run_id,stage,
                   'sha256:'||'56b1b58f42af81eea300730987f94ba040735b0c08cdbbdc2c4c357fcbcbdb08','openrouter','openrouter.responses','host',
        ...
        cursor.execute(
            """
            SELECT public.lab_arena_sep19_rerun318_active_valid_v1(),
            ...
            GROUP BY public.lab_arena_sep19_rerun318_active_valid_v1()
            """,
            (ROUND, BASELINE),
        )
...
def test_rerun320_from_terminal318_executes_scores_and_publishes_with_preservation(
    database, tmp_path, monkeypatch
):
    psycopg2, dsn = database
    conn = psycopg2.connect(**dsn)
    try:
        _seed_terminal_rerun318(conn, terminal_status="published")
        schedule = _schedule()
        with conn.cursor() as cursor:
            cursor.execute(
            ...
            rerun318_history_before = _round_history(cursor, ROUND + "-r318archive")
            cursor.execute(
            ...
            assert terminal_publication_before[2]["fixture"] == "terminal318-published"
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun320 scorer definition differs"):
            with conn.cursor() as cursor:
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun320 terminal preimage differs"):
            with conn.cursor() as cursor:
                cursor.execute(attacked)
        conn.rollback()
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "bool_and(assignment_id LIKE '%%:rerun320') "
            ...
            assert "lab_arena_sep19_rerun318_active_valid_v1()" not in definition
            assert ":score:rerun318" not in definition
            assert "lab_arena_sep19_rerun320_active_valid_v1()" in definition
            assert ":score:rerun320" in definition
            cursor.execute(
            ...
            assert _round_history(cursor, ROUND + "-r318archive") == rerun318_history_before
            cursor.execute(
            ...
            cursor.execute(
                ...
                (ROUND + "-r320archive", BASELINE + ":r320archive"),
            )
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT count(*),count(*) FILTER(WHERE assignment_id LIKE '%%:score:rerun320'),"
        ...
        with conn.cursor() as cursor:
            ...
            archive320_before = _round_history(cursor, ROUND + "-r320archive")
        conn.commit()

        with conn.cursor() as cursor:
            cursor.execute(rendered)
            assert _round_history(cursor, ROUND) == published_before
            assert _round_history(cursor, ROUND + "-r320archive") == archive320_before
            assert _round_history(cursor, ROUND + "-r318archive") == rerun318_history_before
    finally:
        conn.close()


def test_rerun320_archives_failed_response_holds_without_zero_fee_assumption(
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            archive_round = ROUND + "-r320archive"
            archive_submission = BASELINE + ":r320archive"
            cursor.execute(
                "SELECT public.lab_arena__successful_call_cost_state(%s,'execute',"
                "'openrouter')",
                (archive_submission,),
            ...
            cursor.execute(
                "SELECT public.lab_arena_sep19_rerun320_archive_valid_v1(),"
                "public.lab_arena_sep19_rerun320_active_valid_v1()"
            )
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                (ROUND + "-r320archive", identities[0]),
            )
            assert cursor.rowcount == 1
            cursor.execute("SET session_replication_role=origin")
            cursor.execute(
        ...
        with pytest.raises(psycopg2.Error, match="existing Sep19 rerun320 differs"):
            with conn.cursor() as cursor:
                cursor.execute(rendered)
        conn.rollback()
        with conn.cursor() as cursor:
...
def test_rerun320_requires_exact_terminal318_shape_validity_and_settlement(
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SET session_replication_role=origin")
            cursor.execute("SELECT public.lab_arena_sep19_rerun318_active_valid_v1()")
...
def test_rerun320_template_is_unrendered_and_sealed_to_terminal318():
    ...
    assert "assignment_id NOT LIKE '%:score:rerun318'" in body
    ...
    assert "old_guard TEXT:=$guard318$" in body
    assert "old_assignment TEXT:=$assignment318$" in body
    assert "lab_arena_sep19_rerun318_active_valid_v1()" in body
    assert "lab_arena_sep19_rerun320_active_valid_v1()" in body
    assert "DROP TRIGGER IF EXISTS lab_arena_sep19_rerun318_score_namespace_guard" in body
    assert "AS $score_guard320$" in body and "END $score_guard320$" in body
