"""Prepared PostgreSQL proof for a future sealed Sep19 rerun322 render."""
...
from tests.lab_arena import (
    sep19_terminal318_fixed_baseline_rerun320_postgres_test as prior,
)
...
TEMPLATE = Path(__file__).resolve().parents[2] / "scripts" / (
    "322-arena-2026-09-19-terminal320-confirmed-cost-baseline-rerun.sql.template"
)
...
TERMINAL_MARKERS = {
    "__SCORING_DEFINITION_SHA256__": (
        "38756e97556806ec2b708dcf7ed380003f26261b9d4c84a8a2dd31f9596cdc81"
    ),
    "__PATCHED_SCORING_DEFINITION_SHA256__": (
        "21dcff455fb074c7ce00cf79e3899298af92e6fee196631a1e33b4afbff917c0"
    ),
    "__TERMINAL_ACCEPTED_EXECUTE_RUN_COUNT__": "81",
    ...
    "__TERMINAL_CANCEL_REASON_SQL__": "'operator'",
    "__TERMINAL_BASELINE_ACCEPTED_RUN_COUNT__": "1",
    "__TERMINAL_BASELINE_FAILED_RUN_COUNT__": "29",
    "__TERMINAL_BASELINE_LEDGER_COUNT__": "2376",
    "__TERMINAL_BASELINE_EXECUTE_LEDGER_COUNT__": "2376",
    "__TERMINAL_SCORE_LEDGER_COUNT__": "0",
    "__TERMINAL_ROUND_SHA256__": (
        "526598f87146dd524cf4c368d74b059dbc26071c1b9549c2dbc0dac6c765db24"
    ),
    "__TERMINAL_BASELINE_SHA256__": (
        "68cf40d0506ffa77d55fd39af0ddbaa5bef52ea354f4356bdac004ee3de9027c"
    ),
    ...
    "__TERMINAL_RUNS_SHA256__": (
        "a41083e4a0698dc643073f35819841484ee4fab2bac3c996b18e6a07aa4196a8"
    ),
    "__TERMINAL_LEDGER_SHA256__": (
        "1ed41423f4dda7201a9807c869ef2193c49fcb75eb739d7454aa27dce9cda886"
    ),
...
EXACT_SCHEDULE = {
    ...
    "benchmark_deadline": "2026-09-19T19:00:00Z",
    "stage_1_start": "2026-09-19T19:00:01Z",
    "stage_1_close": "2026-09-19T22:00:01Z",
    "stage_1_scoring_close": "2026-09-20T01:00:00Z",
    "stage_2_start": "2026-09-20T01:00:01Z",
    "stage_2_close": "2026-09-20T04:00:01Z",
    "final_scoring_close": "2026-09-20T07:00:00Z",
    "publication_deadline": "2026-09-20T07:00:01Z",
}
...
NEW_SOURCE_REF = (
    "arena/arena-2026-09-19/sources/"
    "baseline-rerun322-synthetic.tar.gz"
)
...
FAILED_RESPONSE_HOLDS = (
    {
        ...
        "generation_id": "gen-rerun320-web-429-a",
    ...
    },
    {
        ...
        "generation_id": "gen-rerun320-web-429-b",
...
def _assert_exact_terminal320_render(body: str) -> None:
    ...
    assert "moved_baseline_ledger<>2376" in body
...
def _scorer_patch(definition: str) -> str:
    old_guard = _template_block("guard320")
    new_guard = _template_block("new_guard")
    old_assignment = _template_block("assignment320")
...
def _seed_terminal_rerun320(conn, *, terminal_status: str = "cancelled") -> None:
    assert terminal_status in {"published", "cancelled"}
    prior._seed_terminal_rerun318(conn)
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            ...
            SET status=CASE WHEN icp_position=0 THEN 'accepted' ELSE 'failed' END,
                ...
                output_ref=CASE WHEN icp_position=0
                  THEN 'arena/output/rerun320-accepted.json' ELSE NULL END
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun320' AND attempt=1
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun320' AND attempt=1 AND icp_position>0
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            SELECT execute.round_id||':'||execute.submission_id||':'||
                   ...
                   'arena/score/rerun320-baseline-accepted.json',execute.run_id,
            ...
            WITH selected AS (
              ...
              SELECT selected.*,
                     'sha256:'||encode(extensions.digest(
                       ('rerun320-paid-'||n::text)::bytea,'sha256'),'hex') AS identity,
        ...
        cursor.execute(
            ...
            SELECT entry_kind,miner_hotkey,round_id,submission_id,run_id,stage,
                   'sha256:'||encode(extensions.digest(
                     'terminal320-score-paid-call'::bytea,'sha256'),'hex'),
        ...
        cursor.execute(
            """
            SELECT public.lab_arena_sep19_rerun320_active_valid_v1(),
            ...
            GROUP BY public.lab_arena_sep19_rerun320_active_valid_v1()
            """,
            (ROUND, BASELINE),
        )
...
def test_terminal320_rerun_executes_scores_and_publishes_with_preservation(
    database, tmp_path, monkeypatch
):
    psycopg2, dsn = database
    conn = psycopg2.connect(**dsn)
    try:
        _seed_terminal_rerun320(conn, terminal_status="published")
        schedule = _schedule()
        with conn.cursor() as cursor:
            cursor.execute(
            ...
            rerun320_history_before = _round_history(cursor, ROUND + "-r320archive")
            cursor.execute(
            ...
            assert terminal_publication_before[2]["fixture"] == "terminal320-published"
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun322 scorer definition differs"):
            with conn.cursor() as cursor:
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun322 terminal preimage differs"):
            with conn.cursor() as cursor:
                cursor.execute(attacked)
        conn.rollback()
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "bool_and(assignment_id LIKE '%%:rerun322') "
            ...
            assert "lab_arena_sep19_rerun320_active_valid_v1()" not in definition
            assert ":score:rerun320" not in definition
            assert "lab_arena_sep19_rerun322_active_valid_v1()" in definition
            assert ":score:rerun322" in definition
            cursor.execute(
            ...
            assert cursor.fetchall() == [
                ("lab_arena_sep19_rerun322_score_namespace_guard",)
            ]
            assert _function_seals(cursor) == behavior_seals_before
            assert _round_history(cursor, ROUND + "-r320archive") == rerun320_history_before
            cursor.execute(
            ...
            cursor.execute(
                ...
                (ROUND + "-r322archive", BASELINE + ":r322archive"),
            )
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT count(*),count(*) FILTER(WHERE assignment_id LIKE '%%:score:rerun322'),"
        ...
        with conn.cursor() as cursor:
            ...
            archive320_before = _round_history(cursor, ROUND + "-r322archive")
        conn.commit()

        with conn.cursor() as cursor:
            cursor.execute(rendered)
            assert _round_history(cursor, ROUND) == published_before
            assert _round_history(cursor, ROUND + "-r322archive") == archive320_before
            assert _round_history(cursor, ROUND + "-r320archive") == rerun320_history_before
    finally:
        conn.close()
...
def test_failed_responses_unknown_costs_archive_with_full_holds_and_no_fee_assumption(
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            archive_round = ROUND + "-r322archive"
            archive_submission = BASELINE + ":r322archive"
            cursor.execute(
                "SELECT public.lab_arena__successful_call_cost_state(%s,'execute',"
                "'openrouter')",
                (archive_submission,),
            ...
            cursor.execute(
                "SELECT public.lab_arena_sep19_rerun322_archive_valid_v1(),"
                "public.lab_arena_sep19_rerun322_active_valid_v1()"
            )
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                (ROUND + "-r322archive", identities[0]),
            )
            assert cursor.rowcount == 1
            cursor.execute("SET session_replication_role=origin")
            cursor.execute(
        ...
        with pytest.raises(psycopg2.Error, match="existing Sep19 rerun322 differs"):
            with conn.cursor() as cursor:
                cursor.execute(rendered)
        conn.rollback()
        with conn.cursor() as cursor:
...
def test_terminal320_preimage_requires_exact_shape_active_validity_and_settlement(
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SET session_replication_role=origin")
            cursor.execute("SELECT public.lab_arena_sep19_rerun320_active_valid_v1()")
...
def test_template_is_sealed_terminal320_only():
    body = TEMPLATE.read_text()
    ...
    assert "after the Sep19 rerun320 round is terminal" in body
    assert "assignment_id LIKE '%:rerun320'" in body
    assert "assignment_id NOT LIKE '%:score:rerun320'" in body
    ...
    assert "old_guard TEXT:=$guard320$" in body
    assert "old_assignment TEXT:=$assignment320$" in body
    assert "lab_arena_sep19_rerun320_active_valid_v1()" in body
    assert "lab_arena_sep19_rerun322_active_valid_v1()" in body
    assert "DROP TRIGGER IF EXISTS lab_arena_sep19_rerun320_score_namespace_guard" in body
    assert "AS $score_guard322$" in body and "END $score_guard322$" in body
...
def test_exact_render_has_terminal_seals_and_rejects_wrong_preimage_atomically(
    ...
    body = raw.decode()
    _assert_exact_terminal320_render(body)
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            previous_archive_before = _round_history(cursor, ROUND + "-r320archive")
        ...
        with pytest.raises(
            psycopg2.Error,
            match=(
                "Sep19 rerun322 (scorer definition differs|terminal preimage differs|"
        ...
        with conn.cursor() as cursor:
            ...
            assert _round_history(cursor, ROUND + "-r320archive") == (
            ...
            cursor.execute(
                ...
                "'public.lab_arena_prepare_sep19_rerun322_v1(bigint,text,text,jsonb,text,text)')"
            )
