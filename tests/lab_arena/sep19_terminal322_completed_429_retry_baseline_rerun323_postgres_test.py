"""PostgreSQL proof for the sealed Sep19 rerun323 migration."""
...
from tests.lab_arena import (
    sep19_terminal320_confirmed_cost_baseline_rerun322_postgres_test as prior,
)
...
TEMPLATE = Path(__file__).resolve().parents[2] / "scripts" / (
    "323-arena-2026-09-19-terminal322-completed-429-retry-baseline-rerun.sql.template"
)
RENDERED = TEMPLATE.with_suffix("")
# These constants bind the rendered migration to the release owner's captured
# terminal preimage and approved source identity.
RENDERED_SHA256 = "4479552d26a7334466ba72ea6dc86cc7fea3898a6dba904e6458ae533140eab8"
TERMINAL_CAPTURE_SHA256 = "1d3b4d219a77ab5af41967d264d730c8969c5572218c5d2be6fa48dc0177d758"
TERMINAL_MARKERS = {
    "__PATCHED_SCORING_DEFINITION_SHA256__": "bb3ba46e0d4412250eb43182995920e500616a5950c96fbf4b2809f6c077589e",
    "__SCORING_DEFINITION_SHA256__": "21dcff455fb074c7ce00cf79e3899298af92e6fee196631a1e33b4afbff917c0",
    "__TERMINAL_ACCEPTED_EXECUTE_RUN_COUNT__": "82",
    "__TERMINAL_BANK_STATE_SHA256__": "1af19ff80c88e0fa121d57fa11ea6af14b35220017b0d11a099c70f6b60b314f",
    "__TERMINAL_BASELINE_ACCEPTED_RUN_COUNT__": "2",
    "__TERMINAL_BASELINE_EXECUTE_LEDGER_COUNT__": "7467",
    "__TERMINAL_BASELINE_FAILED_RUN_COUNT__": "37",
    "__TERMINAL_BASELINE_LEDGER_COUNT__": "7467",
    "__TERMINAL_BASELINE_RUN_COUNT__": "39",
    "__TERMINAL_BASELINE_SHA256__": "9661c4da940e1fa55469a8084b328af1188ca889d036ad3d70d9f53f0a92d5d5",
    "__TERMINAL_CANCEL_REASON_SQL__": "'execution_incomplete:stage1:9'",
    "__TERMINAL_EXECUTE_RUN_COUNT__": "119",
    "__TERMINAL_LEDGER_SHA256__": "01edafd792ac11ac7b61ef6e81836c41f609bcca6b379b8766170803a2cc6b4e",
    ...
    "__TERMINAL_ROUND_SHA256__": "1b22b67d72eb4341e46c78e9ec469be2f60ee53b0ee3ca671d9f8354ff5ca068",
    "__TERMINAL_RUNS_SHA256__": "5832c60c8b190761f8ee984734b5f0a396ad0c5eaff16c0a48e9130e37be7094",
...
EXACT_SOURCE_REF = (
    ...
    "baseline-2026-09-19-rerun323-45769176.tar.gz"
)
EXACT_SOURCE_SIZE = 852_102
EXACT_SOURCE_SHA = (
    "8950f2fc66002885a473b5b55200d93803661a00de8c61927a3768b115dd5b66"
)
EXACT_SOURCE_COMMIT = "45769176ac7b33ccde1b175749cfd9b922c9dcc1"
EXACT_SCHEDULE = {
    "benchmark_deadline": "2026-09-19T21:00:00Z",
    "final_scoring_close": "2026-09-20T09:00:00Z",
    "publication_deadline": "2026-09-20T09:00:01Z",
    "stage_1_close": "2026-09-20T00:00:01Z",
    "stage_1_scoring_close": "2026-09-20T03:00:00Z",
    "stage_1_start": "2026-09-19T21:00:01Z",
    "stage_2_close": "2026-09-20T06:00:01Z",
    "stage_2_start": "2026-09-20T03:00:01Z",
...
NEW_SOURCE_REF = (
    "arena/arena-2026-09-19/sources/"
    "baseline-rerun323-synthetic.tar.gz"
)
...
FAILED_RESPONSE_HOLDS = (
    {
        ...
        "generation_id": "gen-rerun322-web-429-a",
    ...
    },
    {
        ...
        "generation_id": "gen-rerun322-web-429-b",
    ...
    },
    {
        ...
        "generation_id": "gen-rerun322-web-429-confirmed-cost",
...
def _assert_exact_terminal322_render(body: str) -> None:
...
def _scorer_patch(definition: str) -> str:
    old_guard = _template_block("guard322")
    new_guard = _template_block("new_guard")
    old_assignment = _template_block("assignment322")
...
def _seed_terminal_rerun322(conn, *, terminal_status: str = "cancelled") -> None:
    assert terminal_status in {"published", "cancelled"}
    prior._seed_terminal_rerun320(conn)
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            ...
            SET status=CASE WHEN icp_position=0 THEN 'accepted' ELSE 'failed' END,
                ...
                output_ref=CASE WHEN icp_position=0
                  THEN 'arena/output/rerun322-accepted.json' ELSE NULL END
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun322' AND attempt=1
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            WHERE round_id=%s AND kind='execute' AND submission_id=%s
              AND assignment_id LIKE '%%:rerun322' AND attempt=1 AND icp_position>0
            """,
            (ROUND, BASELINE),
        )
        ...
        cursor.execute(
            ...
            SELECT execute.round_id||':'||execute.submission_id||':'||
                   ...
                   'arena/score/rerun322-baseline-accepted.json',execute.run_id,
            ...
            WITH selected AS (
              ...
              SELECT selected.*,
                     'sha256:'||encode(extensions.digest(
                       ('rerun322-paid-'||n::text)::bytea,'sha256'),'hex') AS identity,
        ...
        cursor.execute(
            ...
            SELECT entry_kind,miner_hotkey,round_id,submission_id,run_id,stage,
                   'sha256:'||encode(extensions.digest(
                     'terminal322-score-paid-call'::bytea,'sha256'),'hex'),
        ...
        cursor.execute(
            """
            SELECT public.lab_arena_sep19_rerun322_active_valid_v1(),
            ...
            GROUP BY public.lab_arena_sep19_rerun322_active_valid_v1()
            """,
            (ROUND, BASELINE),
        )
...
def test_terminal322_rerun_executes_scores_and_publishes_with_preservation(
    database, tmp_path, monkeypatch
):
    psycopg2, dsn = database
    conn = psycopg2.connect(**dsn)
    try:
        _seed_terminal_rerun322(conn, terminal_status="published")
        ...
        with conn.cursor() as cursor:
            ...
            rerun322_history_before = _round_history(cursor, ROUND + "-r322archive")
            cursor.execute(
            ...
            assert terminal_publication_before[2]["fixture"] == "terminal322-published"
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun323 scorer definition differs"):
            with conn.cursor() as cursor:
        ...
        with pytest.raises(psycopg2.Error, match="Sep19 rerun323 terminal preimage differs"):
            with conn.cursor() as cursor:
                cursor.execute(attacked)
        conn.rollback()
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "bool_and(assignment_id LIKE '%%:rerun323') "
            ...
            assert "lab_arena_sep19_rerun322_active_valid_v1()" not in definition
            assert ":score:rerun322" not in definition
            assert "lab_arena_sep19_rerun323_active_valid_v1()" in definition
            assert ":score:rerun323" in definition
            cursor.execute(
            ...
            assert cursor.fetchall() == [
                ("lab_arena_sep19_rerun323_score_namespace_guard",)
            ]
            assert _function_seals(cursor) == behavior_seals_before
            assert _round_history(cursor, ROUND + "-r322archive") == rerun322_history_before
            cursor.execute(
            ...
            cursor.execute(
                ...
                (ROUND + "-r323archive", BASELINE + ":r323archive"),
            )
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT count(*),count(*) FILTER(WHERE assignment_id LIKE '%%:score:rerun323'),"
        ...
        with conn.cursor() as cursor:
            ...
            archive320_before = _round_history(cursor, ROUND + "-r323archive")
        conn.commit()

        with conn.cursor() as cursor:
            cursor.execute(rendered)
            assert _round_history(cursor, ROUND) == published_before
            assert _round_history(cursor, ROUND + "-r323archive") == archive320_before
            assert _round_history(cursor, ROUND + "-r322archive") == rerun322_history_before
    finally:
        conn.close()
...
def test_terminal322_preimage_requires_exact_shape_active_validity_and_settlement(
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("SET session_replication_role=origin")
            cursor.execute("SELECT public.lab_arena_sep19_rerun322_active_valid_v1()")
...
def test_template_is_sealed_terminal322_only():
    body = TEMPLATE.read_text()
    ...
    assert "after the Sep19 rerun322 round is terminal" in body
    assert "assignment_id LIKE '%:rerun322'" in body
    assert "assignment_id NOT LIKE '%:score:rerun322'" in body
    ...
    assert "old_guard TEXT:=$guard322$" in body
    assert "old_assignment TEXT:=$assignment322$" in body
    assert "lab_arena_sep19_rerun322_active_valid_v1()" in body
    assert "lab_arena_sep19_rerun323_active_valid_v1()" in body
    assert "DROP TRIGGER IF EXISTS lab_arena_sep19_rerun322_score_namespace_guard" in body
    assert "AS $score_guard323$" in body and "END $score_guard323$" in body
...
def test_exact_render_has_terminal_seals_and_rejects_wrong_preimage_atomically(
    ...
    body = raw.decode()
    _assert_exact_terminal322_render(body)
    ...
    try:
        with conn.cursor() as cursor:
            ...
            previous_archive_before = _round_history(cursor, ROUND + "-r322archive")
        ...
        with pytest.raises(
            psycopg2.Error,
            match=(
                "Sep19 rerun323 (scorer definition differs|terminal preimage differs|"
        ...
        with conn.cursor() as cursor:
            ...
            assert _round_history(cursor, ROUND + "-r322archive") == (
            ...
            cursor.execute(
                ...
                "'public.lab_arena_prepare_sep19_rerun323_v1(bigint,text,text,jsonb,text,text)')"
            )
