"""Sealed Sep20 same-source rejudge-only rerun332 in disposable PostgreSQL."""
...
from lab_arena import rewards, source_disclosure
...
TEMPLATE = (
    ...
    / "scripts/332-arena-2026-09-20-authority-preserving-rejudge.sql.template"
)
...
def _authority_snapshot(cursor):
    return _json(
        cursor,
        "SELECT jsonb_build_object('rewards_enabled',rewards_enabled,"
...
def _render(cursor, schedule: dict[str, str], objects) -> tuple[str, str, str]:
    ...
    values = {
        ...
        "__TERMINAL_AUTHORITY_SHA256__": _scalar(
            cursor,
            ...
            "'promotion_required',promotion_required,"
            "'promotion_doc',promotion_doc,"
            "'baseline_promoted_at',baseline_promoted_at)::text,'sha256'),'hex') "
            "FROM public.lab_arena_rounds WHERE round_id=%s",
            (ROUND,),
        ),
...
def _publish_rerun328(
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute(
            ...
            "configuration_doc||jsonb_build_object('reward_constants',%s::jsonb) "
            "WHERE round_id=%s",
            (json.dumps(rewards.reward_constants_document()), ROUND),
        )
        cursor.execute("ALTER TABLE public.lab_arena_rounds ENABLE TRIGGER USER")
        # The historical seed predates consent persistence. Production rerun328
        # submissions have public-rerun consent, which is required by the real
        # promotion path exercised below.
        cursor.execute("ALTER TABLE public.lab_arena_submissions DISABLE TRIGGER USER")
        cursor.execute(
            "UPDATE public.lab_arena_submissions SET "
            "consent='{\"public_rerun\":true}'::jsonb WHERE round_id=%s",
            (ROUND,),
        )
        cursor.execute(
            "SELECT submission_id,source_ref FROM public.lab_arena_submissions "
            "WHERE round_id=%s AND submission_id<>%s AND source_ref IS NOT NULL",
            (ROUND, BASELINE),
        )
        for submission_id, source_ref in cursor.fetchall():
            harness.objects.put(source_ref, harness.baseline_source)
            cursor.execute(
                "UPDATE public.lab_arena_submissions SET source_size_bytes=%s,"
                "submission_doc=submission_doc||jsonb_build_object("
                "'source_size_bytes',%s) WHERE submission_id=%s",
                (len(harness.baseline_source), len(harness.baseline_source), submission_id),
            )
    ...
    if zero_baseline:
        ...
        return
    promotion_root = harness.tmp / "rerun332-terminal-promotion"
    ...
    harness.service.config.baseline_promoter_factory = lambda: GitPromoter(
        str(remote), harness.tmp / "rerun332-terminal-promotion-cache"
    )
    terminal_round = harness.service.store.get_round(ROUND)
    winner = terminal_round["publication_doc"]["king_decision"][
        "winner_submission_id"
    ]
    winner_submission = harness.service.store.get_submission(winner)
    disclosure = source_disclosure.disclosure_status(
        winner_submission, harness.clock(), round_row=terminal_round
    )
    assert disclosure["available"], {
        "disclosure": disclosure,
        "clock": harness.clock().isoformat(),
        "submission_status": winner_submission["status"],
        "consent": winner_submission["consent"],
        "round_status": terminal_round["status"],
        "published_at": terminal_round["published_at"],
        "icp_set_date": terminal_round["icp_set_date"],
    }
    ...
    assert subprocess.check_output(
        ["git", "--git-dir", str(remote), "rev-parse", "lab"]
    ).strip()
    assert harness.service.activate_reward(ROUND)["status"] == "activated"
...
def _inject_exhausted_provider_zero_and_failed_unknown(connection) -> str:
    ...
    identity = "sha256:" + hashlib.sha256(b"rerun332-failed-unknown").hexdigest()
    with connection.cursor() as cursor:
...
def test_sep20_rerun332_preserves_history_and_isolates_new_scorer_cache(
    ...
    with psycopg2.connect(**dsn) as connection:
        ...
        with connection.cursor() as cursor:
            ...
            historical_award_before = _authority_snapshot(cursor)
            assert historical_award_before["king_outcome"] is not None
            assert historical_award_before["reward_activated_at"] is not None
            assert historical_award_before["baseline_promoted_at"] is not None
            ...
            archived_award = _json(
                cursor,
                "SELECT jsonb_build_object('rewards_enabled',"
                "configuration_doc->'archived_rewards_enabled',"
            ...
            assert archived_award == historical_award_before
            assert _authority_snapshot(cursor) == historical_award_before
            assert _json(
                cursor,
                "SELECT jsonb_agg(to_jsonb(j) ORDER BY cache_key,authority_slot) "
                "FROM public.lab_arena_company_judgments j",
        ...
        with connection.cursor() as cursor:
            ...
            assert _execution_snapshot(cursor) == execution_before
            # The activated basis remains available while corrected scores await review.
            assert any(
                row["reward_basis_hash"]
                == historical_award_before["reward_basis_hash"]
                for row in harness.service.store.published_reward_bases(
                    mode="live", network_name="finney", netuid=71, limit=200
                )
            )
            # The exact migration remains idempotent after the replay reaches scored.
            cursor.execute(rendered)
            assert _authority_snapshot(cursor) == historical_award_before


def test_sep20_rerun332_refuses_genuine_positive_publication_without_mutation(
    database, tmp_path, monkeypatch
):
    psycopg2, dsn = database
    schedule = _schedule()
...
def test_sep20_rerun332_refuses_malformed_publication_worker_loss_and_award_replay(
    database, tmp_path, monkeypatch
):
    psycopg2, dsn = database
    schedule = _schedule()
    ...
    with psycopg2.connect(**dsn) as connection:
        ...
        with connection.cursor() as cursor:
            ...
            sealed_rendered, _, _ = _render(cursor, schedule, harness.objects)
            for label, mutation in (
                ...
                with pytest.raises(psycopg2.Error, match="terminal preimage differs"):
                    cursor.execute(_migration_body(sealed_rendered))
                cursor.execute("ROLLBACK TO SAVEPOINT refused_migration")
                assert _protected_database_snapshot(cursor) == before, label
                cursor.execute("ROLLBACK TO SAVEPOINT invalid_terminal")


def test_sep20_rerun332_template_is_inactive_and_narrow():
    body = TEMPLATE.read_text()
    ...
    assert ARCHIVE in body and "score:rerun332" in body
    ...
    assert "331-arena-2026-09-20-promotion-reward-hold" not in body
    assert "active.configuration_doc ? 'sep20_rerun328_promotion_reward_hold'" in body
    ...
    assert "CREATE TRIGGER lab_arena_000_sep20_rejudge332_publication_stop" in body
    assert "publication requires sealed review release" in body
