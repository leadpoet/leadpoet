...
def test_encrypt_key_produces_an_envelope_the_broker_decrypts_and_never_prints_the_key(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.delenv(MINER["KEY_ENV"], raising=False)
    assert MINER["main"](["encrypt-key", "--recipient", str(recipient_path), "--out", str(out)]) == 2
    monkeypatch.setenv(MINER["KEY_ENV"], KEY)
    assert MINER["main"](["encrypt-key", "--recipient", str(recipient_path), "--out", str(out)]) == 0
    ...
    assert handle.bearer_header()["Authorization"] == "Bearer " + KEY
...
def test_sign_produces_a_valid_scoped_request(tmp_path, capsys):
    ...
    with pytest.raises(contracts.ArenaContractError):
        contracts.validate_signed_request(envelope, expected_scope=contracts.SCOPE_FUNDING, now=int(time.time()), verify_signature=verify)
