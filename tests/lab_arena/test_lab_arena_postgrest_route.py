...
def test_anon_and_service_role_tokens_are_denied_on_arena_tables_and_functions(stack):
    for role in ("anon", "service_role", "authenticated"):
        ...
        create = client.post(stack["base_url"] + "/rpc/lab_arena_create_round", headers=headers, content=json.dumps({"p_round_id": "arena-2026-09-03", "p_configuration_hash": contracts.document_hash("x"), "p_configuration_doc": {}}).encode())
