...
def test_round_at_the_challenger_cap_scores_every_participant_and_publishes(connect, tmp_path):
    ...
    assert opened["assignments"] == contracts.STAGE_1_ICP_COUNT * CHALLENGERS
    ...
    assert timed("stage1_scoring_open", lambda: service.advance_round(round_id))["assignments"] == contracts.STAGE_1_ICP_COUNT * CHALLENGERS
    ...
    finalist_count = min(contracts.FINALIST_COUNT, CHALLENGERS - 1)
    assert len(publication["stage1_ranking"]) == CHALLENGERS - 1
    assert len(round_row["finalists"]) == finalist_count
    assert len(publication["final_ranking"]) == finalist_count + 1
    assert len(service.store.list_runs(round_id, stage=1, kind="execute")) == contracts.STAGE_1_ICP_COUNT * CHALLENGERS
    ...
    report = {
        ...
        "stage1_runs": contracts.STAGE_1_ICP_COUNT * CHALLENGERS,
