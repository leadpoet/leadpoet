...
def scoring_input(tmp_path):
    document = scoring.build_scoring_input(work_item_id=contracts.document_hash("item"), icp=ICP, companies=COMPANIES, policy=scoring.build_scorer_policy(), evaluation_date="2026-09-02")
...
def test_entrypoint_scores_with_the_lab_scorer_under_trusted_mode_and_placeholder_credentials(tmp_path, monkeypatch):
    ...
    assert result["schema_version"] == scoring.SCORING_OUTPUT_SCHEMA_VERSION and result["work_item_id"] == document["work_item_id"]
    assert [b["final_score"] for b in result["breakdowns"]] == [60.0, 61.0, 62.0]
    assert seen["trusted"] is True and seen["policy"]["policy_hash"] == document["scorer_policy"]["policy_hash"]
...
@pytest.mark.parametrize("error, expected", [
    (shim.ShimRequestError("budget_refused"), "judge_key_refused"),
    (shim.ShimRequestError("budget_exhausted"), "judge_key_refused"),
