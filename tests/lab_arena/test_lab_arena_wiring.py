...
def test_daily_baseline_environment_allows_only_promoted_lab(monkeypatch):
    ...
    assert wiring.DEFAULT_BASELINE_SOURCE_URL == (
        "https://github.com/leadpoet/leadpoet-sales-agent/archive/refs/heads/lab.tar.gz"
    )
    ...
    monkeypatch.setenv(
        ...
        "https://github.com/leadpoet/leadpoet-sales-agent/archive/refs/heads/main.tar.gz",
    )
...
def test_promoter_keeps_github_token_out_of_repository_and_command(monkeypatch, tmp_path):
    ...
    assert promoter.repo_url == "https://github.com/leadpoet/leadpoet-sales-agent.git"
...
def test_promoter_ssh_credentials_are_repo_scoped_and_require_private_permissions(monkeypatch, tmp_path):
    ...
    path.chmod(0o600)
    promoter = wiring.baseline_promoter_from_environment()
    assert promoter.repo_url == "git@github.com:leadpoet/leadpoet-sales-agent.git"
