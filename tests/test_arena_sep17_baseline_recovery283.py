...
from scripts import arena_sep17_baseline_recovery283 as r
from tests.lab_arena.icp_fixtures import daily_icps
from tests.lab_arena.test_lab_arena_contracts import base_round_configuration


TEMPLATE = Path(__file__).parents[1] / "scripts/283-arena-2026-09-17-baseline-recovery.sql.template"
EXPECTED_TERMINAL_PLACEHOLDERS = {
    "__FORWARD_SCHEDULE_JSON__",
    "__OLD_SCHEDULE_JSON__",
    "__PRIOR282_AUDIT_HASH__",
    "__PRIOR282_AUTHORITY_HASH__",
...
def test_exact_source_pins_and_unsealed_terminal_template():
    ...
    assert "arena-2026-09-17-rerun282archive" in template
    assert "baseline-2026-09-17-rerun282archive" in template
    assert "assignment_id LIKE '%:rerun282'" in template
    assert "assignment_id LIKE '%:rerun283'" in template
    assert "lab_arena_sep17_recovery282_archive_valid_v1() IS TRUE" in template
    assert "lab_arena_sep17_recovery282_nonbaseline_ledger_valid_v1() IS TRUE" in template
...
def test_prepare_stages_conditional_source_then_exact_rpc(setup):
    ...
    assert setup.calls == [("lab_arena_prepare_sep17_baseline_recovery283_v1", {
