...
from scripts import arena_sep17_published_rerun286 as r
...
def setup(monkeypatch):
    ...
    schedule = {
        ...
        "benchmark_deadline": "2026-09-17T23:00:00Z",
        "stage_1_start": "2026-09-17T23:00:01Z",
        "stage_1_close": "2026-09-18T01:00:00Z",
        "stage_1_scoring_close": "2026-09-18T03:00:00Z",
        "stage_2_start": "2026-09-18T03:00:01Z",
        "stage_2_close": "2026-09-18T05:00:00Z",
        "final_scoring_close": "2026-09-18T07:00:00Z",
        "publication_deadline": "2026-09-18T07:30:00Z",
    }
    config = base_round_configuration()
    config.update(
        round_id=r.ROUND,
        schedule={**schedule, "benchmark_deadline": "2026-09-17T18:30:00Z"},
    ...
    baseline = {
        ...
        "submission_doc": dict(terminal_facts),
    }
    row = {
        "round_id": r.ROUND,
        "status": "published",
        ...
        "reward_activated_at": "2026-09-17T18:29:00Z",
        "reward_basis_hash": "sha256:" + "a" * 64,
        "reward_basis_doc": {"sealed": True},
        "signing_key_doc": {"public_key_hash": "sha256:" + "b" * 64},
        "effective_reward_epoch": 1,
    }
    runs = [
    ...
    tables = {
        "lab_arena_submissions": [baseline],
        "lab_arena_runs": runs,
    ...
    store = SimpleNamespace(
        ...
        submission_costs=lambda _: copy.deepcopy(costs),
...
def test_collect_preflight_is_read_only_and_binds_flat_source_document(setup):
    ...
    assert setup.preflight["baseline"]["submission_doc"] == {
        key: setup.preflight["terminal_source"][key]
        for key in ("source_ref", "source_size_bytes", "source_sha256", "source_commit")
    }
...
def test_prepare_stages_exact_archive_and_calls_typed_rpc(setup):
    result = invoke(setup)
    facts = r._source_facts(setup.new, r.SOURCE_REF)
    assert result["execute_namespace"] == "rerun286"
    assert result["score_namespace"] == "score:rerun286"
    assert setup.fetches == [(r.SOURCE_URL, source_bundle.MAX_SOURCE_ARCHIVE_BYTES)]
    assert setup.client.data[r.SOURCE_REF] == setup.new
    prepare_calls = [call for call in setup.calls if call[0].endswith("rerun286_v1")]
    ...
    assert setup.row["configuration_doc"]["baseline_source_url"].endswith(
        "leadpoet/pydantic-harness/archive/refs/heads/lab.tar.gz"
    )


def test_dry_run_has_no_write_or_prepare_rpc(setup):
    assert invoke(setup, dry_run=True)["status"] == "rerun286_preflight_ok"
    assert not setup.client.writes
    assert not [call for call in setup.calls if call[0].endswith("rerun286_v1")]
...
def test_sealed_or_quiescent_drift_fails_before_write(setup, defect):
    if defect == "round":
        setup.row["published_at"] = "changed"
...
def test_rpc_signature_is_role_bound_and_exact():
    assert FUNCTION_SIGNATURES["lab_arena_prepare_sep17_published_rerun286_v1"] == (
...
def test_source_url_is_current_default_while_terminal_metadata_stays_frozen():
    ...
    assert r.TERMINAL_SOURCE_REF.endswith("recovery285.tar.gz")
    assert r.SOURCE_REF.endswith("recovery286.tar.gz")
