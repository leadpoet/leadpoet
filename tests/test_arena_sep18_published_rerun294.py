...
from scripts import arena_sep18_published_rerun294 as r
...
def test_prepare_stages_exact_archive_and_calls_typed_rpc(setup):
    result = invoke(setup)
    facts = r._source_facts(setup.new, r.SOURCE_REF)
    assert result["execute_namespace"] == "rerun294"
    assert result["score_namespace"] == "score:rerun294"
    assert setup.fetches == [(r.SOURCE_URL, source_bundle.MAX_SOURCE_ARCHIVE_BYTES)]
    assert setup.client.data[r.SOURCE_REF] == setup.new
    prepare_calls = [call for call in setup.calls if call[0].endswith("rerun294_v1")]
...
def test_dry_run_has_no_write_or_prepare_rpc(setup):
    assert invoke(setup, dry_run=True)["status"] == "rerun294_preflight_ok"
    assert not setup.client.writes
    assert not [call for call in setup.calls if call[0].endswith("rerun294_v1")]
...
def test_rpc_signature_is_role_bound_and_exact():
    assert FUNCTION_SIGNATURES["lab_arena_prepare_sep18_published_rerun294_v1"] == (
...
def test_source_url_is_current_default_while_terminal_metadata_stays_frozen():
    ...
    assert r.SOURCE_REF.endswith("baseline-2026-09-18-rerun294.tar.gz")
