...
TEMPLATE = ROOT / "scripts/295-arena-2026-09-18-published-baseline-rerun.sql.template"
...
def test_current_scorer_changes_only_admitted_baseline_namespace():
    ...
    assert "THEN ':rerun295' ELSE '' END" in sql
    assert "NEW.submission_id = 'baseline-2026-09-18'" in sql
    assert "':score:rerun295'" in sql
...
def test_requires_null_score_publication_contract_from_migration_292():
    sql = TEMPLATE.read_text()
    assert "apply migration 292 before rerun295" in sql
...
def test_prepare_is_one_private_atomic_archive_and_preserves_activated_authority():
    ...
    assert "assignment_id LIKE '%:rerun295'" in sql
    assert "assignment_id NOT LIKE '%:score:rerun295'" in sql
    for field in (
...
def test_no_sep17_archive_chain_and_all_nonbaseline_evidence_remains_sealed():
    ...
    assert "lab_arena_sep18_published_rerun295_nonbaseline_valid_v1()" in sql
    assert sql.count(
        "OR public.lab_arena_sep18_published_rerun295_nonbaseline_valid_v1()"
...
def test_publication_guard_seals_only_winner_identity_not_new_scores_or_source():
    ...
    guard = sql[sql.index(
        "CREATE OR REPLACE FUNCTION public.lab_arena_sep18_published_rerun295_publication_guard_v1"
    ):]
