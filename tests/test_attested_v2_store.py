...
def _model_compatibility_receipt(
    private_key,
    *,
    issued_at=NOW,
    output_root=HASH_B,
    ...
    return create_signed_execution_receipt(
        body=build_execution_receipt_body(
            role=SCORING_ROLE,
            purpose=purpose,
            job_id="scoring-v2:run-model-sandbox-v2:" + "1" * 32,
            epoch_id=0,
            sequence=0,
            commit_sha="d" * 40,
            pcr0="e" * 96,
            ...
            status=status,
            failure_code=None if status == "succeeded" else "rejected",
            issued_at=issued_at,
        ),
...
@pytest.mark.asyncio
async def test_model_compatibility_receipt_reuses_exact_signed_semantics(
    monkeypatch,
):
    private_key = Ed25519PrivateKey.generate()
    stored_receipt = _model_compatibility_receipt(private_key)
    current_receipt = _model_compatibility_receipt(
        private_key,
        issued_at=LATER,
    )
    assert stored_receipt["receipt_hash"] != current_receipt["receipt_hash"]
    assert stored_receipt["enclave_signature"] != current_receipt[
        "enclave_signature"
    ]
    stored_row = attested_v2_store.receipt_storage_row(stored_receipt)
    observed = []

    async def select_many(_table, *, filters, order_by, limit):
        observed.append((tuple(filters), tuple(order_by), limit))
    ...
    assert observed == [
        (
            tuple(
                (field, current_receipt[field])
                for field in attested_v2_store._RECEIPT_IDEMPOTENCY_FIELDS_V2
            ),
            (("receipt_hash", False),),
            2,
        )
...
@pytest.mark.parametrize(
    ("mutation", "error"),
    (
        ("output", "conflicts at output_root"),
        ("selector", "selector conflicts at job_id"),
        ("signature", "signature"),
...
async def test_model_compatibility_receipt_reuse_fails_closed(
    ...
    current_receipt = _model_compatibility_receipt(
        private_key,
        issued_at=LATER,
        output_root=HASH if mutation == "output" else HASH_B,
    )
    stored_row = attested_v2_store.receipt_storage_row(stored_receipt)
    if mutation == "selector":
        stored_row["job_id"] = "wrong-job"
    elif mutation == "signature":
        stored_row["receipt_doc"] = {
            **stored_receipt,
            "enclave_signature": "0" * 128,
        }

    async def select_many(_table, **_kwargs):
        return [dict(stored_row), dict(stored_row)] if mutation == "duplicate" else [dict(stored_row)]

    monkeypatch.setattr(attested_v2_store, "select_many", select_many)

    with pytest.raises(Exception, match=error):
        await attested_v2_store.load_compatible_model_compatibility_receipt_v2(
            current_receipt
        )


@pytest.mark.asyncio
async def test_model_compatibility_receipt_reuse_never_selects_other_purpose(
    monkeypatch,
):
    receipt = _model_compatibility_receipt(
        Ed25519PrivateKey.generate(),
        purpose="research_lab.private_model_run.v2",
    )

    async def unexpected(*_args, **_kwargs):
        raise AssertionError("non-model-compatibility purpose queried durable reuse")

    monkeypatch.setattr(attested_v2_store, "select_many", unexpected)
