...
def test_company_website_requires_a_lead_domain_in_strict_mode() -> None:
    assert check_source_url_mismatch(
        "company_website",
        "https://self-published.example/esri-award",
        "",
        reject_unknown_third_party=True,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "https://careers.acme.co.uk/open-positions/engineer",
            "https://www.acme.co.uk/",
        ),
        (
            "https://jobs.xn--bcher-kva.de/open-positions/engineer",
            "https://bücher.de/",
        ),
    ],
)
def test_first_party_ownership_accepts_psl_and_idna_sibling_subdomains(
    source_url: str,
    company_website: str,
) -> None:
    assert (
...
@pytest.mark.parametrize(
    ...
    [
        (
            "https://acme.co.uk.attacker.com/open-positions/engineer",
            "https://www.acme.co.uk/",
        ),
        (
            "https://evil.github.io/open-positions/engineer",
            "https://acme.github.io/",
        ),
        (
            "https://jobs.xn--80ak6aa92e.com/open-positions/engineer",
            "https://apple.com/",
        ),
    ],
)
def test_first_party_ownership_rejects_suffix_and_idna_spoofs(
    source_url: str,
    company_website: str,
) -> None:
    assert check_source_url_mismatch(
...
@pytest.mark.asyncio
async def test_declared_source_body_gate_is_fulfillment_only() -> None:
    signal = IntentSignal(
        source="job_board",
        description=(
            "Acme is actively hiring data engineers to expand its analytics "
            "platform and customer integrations."
        ),
        url="https://jobs.lever.co/acme/1234",
        date="2026-07-01",
        snippet="Acme is hiring data engineers for its analytics platform.",
        matched_icp_signal=0,
    )
    icp = ICPPrompt(
        icp_id="icp-source-scope",
        industry="Software",
        sub_industry="Analytics",
        employee_count="51-200",
        company_stage="growth",
        geography="United States",
        product_service="analytics",
        intent_signals=["actively hiring data engineers"],
    )
    accepted = {
        "client_ready": True,
        "decision": "approve",
        ...
        "stage3": {"status": "supported"},
        "scrape": {"result_count": 1},
    }
    ...
    assert verifier.await_args_list[0].kwargs["declared_source"] is None
    assert verifier.await_args_list[1].kwargs["declared_source"] == "job_board"
