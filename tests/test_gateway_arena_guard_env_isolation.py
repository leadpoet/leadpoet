...
def test_guard_helper_uses_canonical_file_before_and_after_clone_removal(
    tmp_path: Path,
) -> None:
    script = SCRIPT_PATH.read_text(encoding="utf-8")
    run_guard = _shell_function_source(script, "run_lab_arena_restart_guard")
    ...
    stale_clone = tmp_path / "merged-env-clone.sh"
    stale_clone.write_text(
        "LAB_ARENA_SUPABASE_URL=https://old.invalid\n"
        "LAB_ARENA_SUPABASE_URL=https://other.invalid\n"
        "LAB_ARENA_SERVICE_KEY=sb_secret_old\n",
        encoding="utf-8",
    )
    harness = tmp_path / "guard-harness.sh"
    harness.write_text(
        ...
        + "\n"
        + f"ENV_CLONE={shlex.quote(str(stale_clone))}\n"
        + f"GATEWAY_ENV_FILE={shlex.quote(str(environment_file))}\n"
        ...
        + f"ENV_CLONE={shlex.quote(str(tmp_path / 'removed-env-clone.sh'))}\n"
        + f"run_lab_arena_restart_guard {shlex.quote(str(source_root))} ready "
        + "--generation 7 --phase gateway_ready\n",
        encoding="utf-8",
    )
    harness.chmod(0o755)
    ambient = {
    ...
    assert selected == [
        {"hostname": "file-authority.invalid", **file_values},
        {"hostname": "file-authority.invalid", **file_values},
    ]


def test_guard_helper_missing_canonical_file_fails_closed(
    tmp_path: Path,
) -> None:
    script = SCRIPT_PATH.read_text(encoding="utf-8")
    run_guard = _shell_function_source(script, "run_lab_arena_restart_guard")
    ...
    harness.write_text(
        ...
        + run_guard
        + "\n"
        + f"ENV_CLONE={shlex.quote(str(tmp_path / 'missing-clone.env'))}\n"
        + f"GATEWAY_ENV_FILE={shlex.quote(str(tmp_path / 'missing.env'))}\n"
    ...
    assert "environment file is unavailable" in completed.stderr
