...
def test_exact_gateway_release_restores_from_verified_archive(tmp_path, monkeypatch):
    ...
    restored_root.mkdir()
    (restored_root / "tee-enclave-gateway_coordinator.eif").write_bytes(b"stale")
    verified_roots = []
...
def test_gateway_restore_copies_read_only_live_rollback_backups(
    ...
    restored_root = tmp_path / "restored"
    restored_root.mkdir()
    sentinel = restored_root / "tee-enclave-gateway_coordinator.eif"
...
def test_gateway_archive_rejects_artifact_tampering(tmp_path):
    ...
    (archive / "tee-enclave-gateway_coordinator.eif").write_bytes(b"tampered")
...
def test_gateway_archive_rejects_eif_rehashed_after_tampering(tmp_path):
    ...
    archive = Path(result["archive_path"])
    relative_path = "tee-enclave-gateway_coordinator.eif"
