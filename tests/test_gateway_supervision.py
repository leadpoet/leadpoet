...
def _unsupervised_launch_env_unsets() -> tuple[str, ...]:
    """The `env -u` list on gw_restart.sh's unsupervised launch.

    Read out of the script rather than copied, so that a variable added to or
    removed from the direct launch is compared against the launcher instead of
    ...
    start = script.rindex("\nenv -u ", 0, launch)
    return tuple(re.findall(r"-u ([A-Z0-9_]+)", script[start:launch]))


# Kept identical to the `env -u` list in gw_restart.sh's unsupervised launch.
RESTART_ONLY_VARIABLES = _unsupervised_launch_env_unsets()
...
def test_launcher_unset_list_matches_the_unsupervised_launch() -> None:
    """The supervised and unsupervised launches must hand the gateway the same
    environment; a variable stripped from one has to be stripped from both."""

    launcher = LAUNCHER.read_text()
    unset = set(re.findall(r"^unset ([A-Z0-9_]+)$", launcher, re.MULTILINE))
    assert set(RESTART_ONLY_VARIABLES) == unset
