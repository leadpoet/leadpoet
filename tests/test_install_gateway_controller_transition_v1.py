...
def test_transitions_installed_old_controller_to_exact_current_candidate(
    tmp_path: Path,
) -> None:
    repo, old_commit = _repo(tmp_path)
    old_payloads = dict(FILES)
    current_payloads = {
        relative: payload + b"# current controller\n"
        for relative, payload in FILES.items()
    }
    for relative, payload in current_payloads.items():
    ...
    subprocess.run(
        ["git", "fetch", "-q", "origin", "main"], cwd=repo, check=True
    )

    root = tmp_path / "controller"
    old_release = root / "releases" / old_commit
    for relative, payload in old_payloads.items():
        installed = old_release / relative
        installed.parent.mkdir(parents=True, exist_ok=True)
        installed.write_bytes(payload)
        installed.chmod(0o700 if relative == "gw_restart.sh" else 0o600)
    root.mkdir(parents=True, exist_ok=True)
    (root / "current").symlink_to(f"releases/{old_commit}")
    host = tmp_path / "gw_restart.sh"
    host.write_bytes(old_payloads["gw_restart.sh"])
    host.chmod(0o700)

    installed = install_transition(
        repo=repo,
    ...
    assert installed == root / "releases" / current_commit
    assert (root / "current").readlink() == Path(
        f"releases/{current_commit}"
    )
    assert host.read_bytes() == current_payloads["gw_restart.sh"]
    assert host.stat().st_mode & 0o777 == 0o700
    for relative, payload in old_payloads.items():
        assert (old_release / relative).read_bytes() == payload
    for relative, payload in current_payloads.items():
        assert (installed / relative).read_bytes() == payload
...
def test_transition_wrapper_binds_origin_main_controller_and_restart_lock() -> None:
    ...
    assert "203-retire-legacy-incentive-weight-bridge.sql" not in source
    assert "GATEWAY_MIGRATION_203_SQL_SHA256" not in source
    assert 'export GATEWAY_RESTART_LOCK_FILE="$restart_lock"' in source
    assert "export GATEWAY_RESTART_LOCK_HELD=1" in source
