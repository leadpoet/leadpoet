...
MIGRATIONS = ("117", "118", "119", "120", "121", "122", "123")
...
BEGIN
    -- 119: batched insert is conflict-safe and idempotent.
    ...
    IF (r->>'inserted')::INT <> 2 THEN RAISE EXCEPTION '119 first insert expected 2, got %', r->>'inserted'; END IF;
    r := public.insert_research_lab_provider_usage_ledger_rows(
        '[{"usage_row_id":"11111111-1111-4111-8111-111111111111","provider_id":"scrapingdog"},
          {"usage_row_id":"33333333-3333-4333-8333-333333333333","provider_id":"deepline"}]'::JSONB);
    IF (r->>'inserted')::INT <> 1 THEN RAISE EXCEPTION '119 idempotent insert expected 1, got %', r->>'inserted'; END IF;

    -- 117: anti-join returns only the ids absent from research_trajectories.
    ...
    IF n <> 1 THEN RAISE EXCEPTION '117 expected 1 missing, got %', n; END IF;

    -- 120: in-SQL deterministic id matches Python (pins uuid5(canonical_hash)).
    IF public.research_lab_trajectory_id('3f2a1c00-0000-4000-8000-000000000001')
       <> '1544660e-89e4-52e6-84be-8bc3d0e01907' THEN
        RAISE EXCEPTION '120 trajectory_id reproduction drifted from Python';
    END IF;

    -- 120 delta: seed one projected + one unprojected terminal run.
    ...
    IF n <> 1 THEN RAISE EXCEPTION '120 expected 1 unprojected run, got %', n; END IF;
    IF (SELECT run_id FROM public.research_lab_next_unprojected_terminal_runs(25, false))
       <> 'd0000000-0000-4000-8000-000000000002' THEN
        RAISE EXCEPTION '120 unprojected delta returned the wrong run';
    END IF;
    ...
    IF n <> 1 THEN RAISE EXCEPTION '120 expected 1 missing-trace run, got %', n; END IF;
    ...
    SELECT count(*) INTO n FROM public.research_lab_terminal_runs_missing_traces(25, true);
    IF n <> 0 THEN RAISE EXCEPTION '120 traced run should no longer be flagged, got %', n; END IF;

    -- 123 corpus-completeness discovery. Project run 2 now (120's unprojected
    ...
    IF n <> 2 THEN RAISE EXCEPTION '123 expected 2 runs needing corpus, got %', n; END IF;
    ...
    SELECT count(*) INTO n FROM public.research_lab_terminal_runs_needing_corpus(25, true);
    IF n <> 1 THEN RAISE EXCEPTION '123 marked run must drop from discovery, got %', n; END IF;
    IF (SELECT run_id FROM public.research_lab_terminal_runs_needing_corpus(25, true))
       <> 'd0000000-0000-4000-8000-000000000002' THEN
        RAISE EXCEPTION '123 wrong run left needing corpus';
    END IF;

    -- 123 REQUIRED late-event rediscovery: append a late LOOP event to the
    ...
    SELECT count(*) INTO n FROM public.research_lab_terminal_runs_needing_corpus(25, true);
    IF n <> 2 THEN RAISE EXCEPTION '123 late loop event must force rediscovery, got %', n; END IF;
    ...
    PERFORM public.research_lab_mark_corpus_complete(
        ...
        public.research_lab_corpus_source_watermark('d0000000-0000-4000-8000-000000000001'));
    SELECT count(*) INTO n FROM public.research_lab_terminal_runs_needing_corpus(25, true);
    IF n <> 1 THEN RAISE EXCEPTION '123 re-mark at new watermark must settle, got %', n; END IF;
    ...
    SELECT count(*) INTO n FROM public.research_lab_terminal_runs_needing_corpus(25, true);
    IF n <> 2 THEN RAISE EXCEPTION '123 late promotion/version events must force rediscovery, got %', n; END IF;
    ...
    SELECT count(*) INTO n FROM public.research_lab_terminal_runs_needing_corpus(25, true);
    IF n <> 2 THEN RAISE EXCEPTION '123 late score bundle must force rediscovery, got %', n; END IF;

    -- 121: atomic candidate claim -- sequential claims yield DISTINCT candidates.
    ...
    IF c1 IS NULL OR c2 IS NULL OR c1 = c2 THEN
        RAISE EXCEPTION '121 sequential claims must be distinct, got % and %', c1, c2;
    END IF;
    IF c1 <> 'cand-a' THEN RAISE EXCEPTION '121 oldest eligible should be claimed first, got %', c1; END IF;
    ...
    IF (SELECT count(*) FROM public.claim_next_research_lab_candidate('w3',120,900,300)) <> 0 THEN
        RAISE EXCEPTION '121 no eligible candidate should remain';
    END IF;

    -- 122: atomic run claim -- sequential claims yield DISTINCT runs, priority order.
    INSERT INTO public.research_loop_run_queue_current
        (run_id, ticket_id, queue_priority, current_event_hash, current_status_at, current_queue_status)
    VALUES
...
BEGIN
    -- 118: two processes, same human-readable name, DIFFERENT tokens -- only one holds.
    r := public.research_lab_acquire_maintenance_lease(lease, tok_a, 180);
    IF (r->>'acquired')::BOOL IS NOT TRUE THEN RAISE EXCEPTION '118 A should acquire'; END IF;
    r := public.research_lab_acquire_maintenance_lease(lease, tok_b, 180);
    IF (r->>'acquired')::BOOL IS NOT FALSE THEN RAISE EXCEPTION '118 B must NOT acquire while A holds'; END IF;
    r := public.research_lab_acquire_maintenance_lease(lease, tok_a, 180);
    IF (r->>'acquired')::BOOL IS NOT TRUE THEN RAISE EXCEPTION '118 A should renew'; END IF;
    UPDATE public.research_lab_maintenance_lease SET expires_at = now() - interval '1 s' WHERE lease_name = lease;
    r := public.research_lab_acquire_maintenance_lease(lease, tok_b, 180);
    IF (r->>'acquired')::BOOL IS NOT TRUE THEN RAISE EXCEPTION '118 B should take an expired lease'; END IF;
