...
gateway/db/client.py owns the fix (`create_http1_sync_client` builds the client
on an ``httpx.Client(http1=True, http2=False)``). Every other gateway module that
...
# and does not match; create_http1_sync_client does not match either.
...
def test_gateway_has_no_unpinned_supabase_clients():
    ...
    assert not offenders, (
        ...
        "threadpool use). Build them via "
        "gateway.db.client.create_http1_sync_client "
        "instead:\n" + "\n".join(offenders)
    )
...
def test_allowed_factory_still_exists_and_pins_http1():
    ...
    assert "create_http1_sync_client" in text
    assert "http1=True" in text and "http2=False" in text
    assert "SyncClientOptions().postgrest_client_timeout" in text
