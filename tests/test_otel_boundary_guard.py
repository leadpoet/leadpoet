...
def test_bootstrap_uses_raw_asgi_middleware() -> None:
    source = _read(BOOTSTRAP)
    tree = ast.parse(source)
    uses_http_decorator = False
    uses_base_http_middleware = False
    registers_raw_middleware = False

    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            uses_base_http_middleware |= any(
                alias.name == "BaseHTTPMiddleware" for alias in node.names
            )
        if isinstance(node, (ast.Name, ast.Attribute)):
            uses_base_http_middleware |= (
                getattr(node, "id", None) == "BaseHTTPMiddleware"
                or getattr(node, "attr", None) == "BaseHTTPMiddleware"
            )
        if not isinstance(node, ast.Call) or not isinstance(node.func, ast.Attribute):
            continue
        if node.func.attr == "middleware" and node.args:
            uses_http_decorator |= (
                isinstance(node.args[0], ast.Constant)
                and node.args[0].value == "http"
            )
        if node.func.attr == "add_middleware" and node.args:
            registers_raw_middleware |= (
                isinstance(node.args[0], ast.Name)
                and node.args[0].id == "_GatewayOtelMiddleware"
            )

    assert not uses_http_decorator, (
        "FastAPI's HTTP decorator installs BaseHTTPMiddleware and changes "
        "exception, cancellation, and streaming behavior"
    )
    assert not uses_base_http_middleware, (
        "gateway telemetry must remain a raw ASGI middleware"
    )
    assert registers_raw_middleware


def test_batch_processor_limits_are_explicit() -> None:
    source = _read(BOOTSTRAP)
    for argument in (
