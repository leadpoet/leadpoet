...
MIGRATION = (
    ROOT / "scripts" / "119-research-lab-provider-usage-batch-insert.sql"
...
def test_batch_inserted_count_handles_dict_list_and_garbage() -> None:
    ...
    assert tp._batch_inserted_count([]) is None
    assert tp._batch_inserted_count(None) is None
    assert tp._batch_inserted_count({}) is None
    assert tp._batch_inserted_count({"inserted": "bad"}) is None
    assert tp._batch_inserted_count({"inserted": -1}) is None


@pytest.mark.asyncio
@pytest.mark.parametrize(
    "response",
    (None, {}, [], [{"inserted": 0}, {"inserted": 0}], {"inserted": "bad"}),
)
async def test_batch_insert_rejects_malformed_rpc_success(response: Any) -> None:
    class _Malformed:
        async def call_rpc(self, *args: Any, **kwargs: Any) -> Any:
            return response

    assert (
        await tp._insert_provider_usage_ledger_rows_batch(
            _Malformed(), [_row("e" * 32)]
        )
        is None
    )
...
@pytest.mark.asyncio
async def test_batch_insert_rejects_impossible_insert_count() -> None:
    class _Impossible:
        async def call_rpc(self, *args: Any, **kwargs: Any) -> Any:
            return {"requested": 1, "inserted": 2}

    assert (
        await tp._insert_provider_usage_ledger_rows_batch(
            _Impossible(), [_row("e" * 32)]
        )
        is None
    )
