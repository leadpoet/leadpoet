...
MIGRATION = (
    ROOT / "scripts" / "121-research-lab-atomic-run-claim.sql"
...
async def test_next_queued_run_uses_atomic_claim_rpc(monkeypatch) -> None:
    ...
    assert seen["params"] == {"p_holder_ref": "research-lab-worker-1", "p_ttl_seconds": 120}
...
async def test_falls_back_to_scan_partition_when_rpc_unavailable(monkeypatch) -> None:
    # Migration 121 not applied yet -> RPC errors -> legacy trimmed scan + partition.
