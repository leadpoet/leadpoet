...
OPENCORPORATES_API_KEY = os.getenv(
...
class LRUCache:
    ...
    def cleanup_expired(self, ttl_hours: int):
        ...
        expired_keys = [
            key for key in list(self.cache.keys()) if self.is_expired(key, ttl_hours)
        ]
            ...
            try:
                ...
                if w.creation_date:
                    ...
                    if age_days >= min_age_days:
                        return (
                            True,
                            f"Domain age: {age_days} days (minimum: {min_age_days})",
                        )
                    else:
                        return (
                            False,
                            f"Domain too new: {age_days} days (minimum: {min_age_days})",
                        )
                else:
                        ...
                        if companies:
                            ...
                            for company in companies[:5]:  # Check first 5 results
                                if company.get("company", {}).get("inactive", False):
                                    result = (
                                        False,
                                        f"Company marked as inactive: {company.get('company', {}).get('name', 'Unknown')}",
                                    )
                                    validation_cache[cache_key] = result
                                    return result
                            ...
                            result = (
                                True,
                                "Company status verified via OpenCorporates",
                            )
                        else:
                            result = (
                                True,
                                "Company not found in OpenCorporates (assumed valid)",
                            )
                    else:
                        result = (
                            True,
                            "OpenCorporates API unavailable (assumed valid)",
                        )
            validation_cache[cache_key] = result
            return result
...
async def check_industry_location_match(lead: dict) -> tuple[bool, str]:
    ...
    try:
        ...
        industry_match = any(
            ...
            for keyword in tech_keywords + finance_keywords + healthcare_keywords
        )
...
async def check_apollo_company_match(lead: dict) -> tuple[bool, str]:
    ...
    # TODO: Uncomment when company matching is needed in the future
    ...
    #     return False, "Invalid website format"
    #
    # cache_key = f"apollo_company:{domain}"
    # if cache_key in validation_cache and not validation_cache.is_expired(cache_key, CACHE_TTLS["hunter_apollo_zerobounce"]):
    #     return validation_cache[cache_key]
    #
    # try:
    #     # Mock mode fallback - check if API key is placeholder or if we're on free plan
    #     if "YOUR_APOLLO_API_KEY" in APOLLO_API_KEY or "free" in APOLLO_API_KEY.lower():
    #         result = (True, "Mock pass - Apollo company match")
    #         validation_cache[cache_key] = result
    #         return result
    #
    #     # Implement actual Apollo companies search API
    #     # Use organizations search endpoint which is accessible on free plan
    #     url = "https://api.apollo.io/api/v1/organizations/search"
    #
    #     # Build Apollo search request body - use company name instead of domain
    #     company_name = lead.get("Company", "")
    #     body = {
    #         "page": 1,
    #         "per_page": 50,  # Increased to capture more results for better company matching
    #         "q_keywords": company_name if company_name else domain
    #     }
    #
    #     # Set up headers for Apollo API
    #     headers = {
    ...
    #
    #                                 found_match = False
    #                                 # Apollo API returns organizations in an 'organizations' array
    #                                 # If we find any organizations matching the company name, the company exists
    #                                 if isinstance(data, dict) and data.get("organizations"):
    #                                     organizations = data["organizations"]
    #                                     if organizations and len(organizations) > 0:
    #                                         result = (True, "Apollo company match found via contact API")
    #                                     else:
    #                                         result = (False, "Apollo company match not found")
    #                                 else:
    #                                     result = (False, "Apollo company match not found")
    #                         elif response.status == 403:
    #                             # API endpoint not accessible (likely due to plan restrictions)
    #                             # Fall back to mock mode for testing
    #                             print("⚠️ Apollo API endpoint not accessible with current plan - using mock mode")
    #                             result = (True, "Mock pass - Apollo company match (endpoint restricted)")
    #                         elif response.status == 422:
    #                             # Check if it's a credit limit error
    #                             response_text = await response.text()
    #                             if "insufficient credits" in response_text.lower():
    #                                 print("⚠️ Apollo API credit limit reached - using mock mode")
    #                                 result = (True, "Mock pass - Apollo company match (credit limit reached)")
    #                             else:
    #                                 result = (False, f"Apollo company match check failed: API returned status {response.status} - {response_text[:100]}")
    #                         else:
    #                             result = (False, f"Apollo company match check failed: API returned status {response.status}")
    #
    #                 validation_cache[cache_key] = result
    #                 return result
    #
    # except Exception as e:
    #     result = (False, f"Apollo company match failed: {str(e)}")
    #     validation_cache[cache_key] = result
    #     return result
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.head(
                company_domain, timeout=10, allow_redirects=True
            ) as response:
                if response.status == 200:
...
async def validate_lead_list(leads: list) -> list:
    ...
    if has_duplicates:
        ...
        for i, lead in enumerate(leads):
            email = lead.get("Email 1", lead.get("Owner(s) Email", lead.get("email", "")))
            domain = (
                urlparse(lead.get("Website", lead.get("website", ""))).netloc
                if lead.get("Website") or lead.get("website")
                else ""
            )
