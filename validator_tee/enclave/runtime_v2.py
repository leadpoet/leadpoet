...
VALIDATOR_RUNTIME_CONFIG_SCHEMA_VERSION = (
    "leadpoet.validator_runtime_config.v2.legacy_cutover_lineage"
)
...
def _validate_configuration(value: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    observed_fields = frozenset(value) if isinstance(value, Mapping) else frozenset()
    if not isinstance(value, Mapping) or observed_fields not in {
