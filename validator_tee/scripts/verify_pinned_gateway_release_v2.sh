...
COORDINATION_MAX_ATTEMPTS="${VALIDATOR_PINNED_GATEWAY_COORDINATION_MAX_ATTEMPTS:-1200}"
TOTAL_TIMEOUT_SECONDS="${VALIDATOR_PINNED_GATEWAY_TIMEOUT_SECONDS:-3900}"
...
if ! [[ "$MAX_ATTEMPTS" =~ ^[1-9][0-9]*$ ]] \
    || [ "$MAX_ATTEMPTS" -gt 1200 ]; then
  echo "ERROR: VALIDATOR_PINNED_GATEWAY_MAX_ATTEMPTS must be between 1 and 1200" >&2
  exit 2
fi
...
if ! [[ "$TOTAL_TIMEOUT_SECONDS" =~ ^[1-9][0-9]*$ ]] \
    || [ "$TOTAL_TIMEOUT_SECONDS" -gt 7200 ]; then
  echo "ERROR: VALIDATOR_PINNED_GATEWAY_TIMEOUT_SECONDS must be between 1 and 7200" >&2
  exit 2
fi
